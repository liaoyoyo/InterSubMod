...
public:
    ...
    static bool parse(int argc, char** argv, Config& config) {
        ...
        app.add_option("-j,--threads", config.threads, "Number of threads (Default: 1)")->check(CLI::PositiveNumber);
        ...
        static const std::map<std::string, DistanceMetricType> metric_map = {{"nhd", DistanceMetricType::NHD},
        ...
        static const std::map<std::string, NanDistanceStrategy> nan_map = {{"max_dist", NanDistanceStrategy::MAX_DIST},
