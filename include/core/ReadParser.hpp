...
public:
     ...
     */
    ReadInfo parse(const bam1_t* b, int read_id, bool is_tumor, const SomaticSnv& anchor_snv,
     ...
     */
    FilteredReadInfo create_filtered_info(const bam1_t* b, bool is_tumor, FilterReason reasons) const;
     ...
     */
    const ReadFilterConfig& get_config() const {
        return config_;
    }
...
private:
     ...
     */
    AltSupport determine_alt_support(const bam1_t* b, const SomaticSnv& snv, const std::string& ref_seq,
     ...
     */
    AltSupportResult determine_alt_support_with_reason(const bam1_t* b, const SomaticSnv& snv,
                                                       const std::string& ref_seq, int32_t ref_start_pos) const;
};
