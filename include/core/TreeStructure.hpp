...
struct TreeNode {
     ...
     * @brief 建立內部節點
     */
    static std::shared_ptr<TreeNode> create_internal(
        int node_id,
        ...
        if (right_child) {
            node->leaf_indices.insert(
                node->leaf_indices.end(),
                right_child->leaf_indices.begin(),
...
public:
    ...
    std::string to_newick(
        ...
        bool include_branch_length = true,
        int precision = 6
    ) const;
...
private:
     ...
     * @brief 遞迴深拷貝節點
     */
    std::shared_ptr<TreeNode> deep_copy_node(
        const std::shared_ptr<TreeNode>& node
    ) const;
};
