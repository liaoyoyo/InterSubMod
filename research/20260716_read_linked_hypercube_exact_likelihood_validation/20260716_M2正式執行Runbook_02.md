<!--
建立時間: 2026-07-16 18:05 +08:00
目標: 以可恢復、不可覆寫、source-bound 的方式執行 M2 全量 extraction/ranking、最終數字、教授版 HTML 與瀏覽器 QA
...
取代文件:
  - InterSubMod/research/20260716_read_linked_hypercube_exact_likelihood_validation/20260716_M2正式執行Runbook_01.md
-->

# M2 正式執行 Runbook v2（resource-attested / resumable at completed gates）

> **TL;DR**：流程固定為 `zero-conflict bootstrap → PRE → exact-11 freeze → 雙 pilot 6 gates → extraction 12 gates → ranking 12 gates → POST → independent verifier → final numeric summary → presentation freeze → FINAL HTML → frozen browser QA`。每個正式 gate 都是不可覆寫 JSON＋SHA sidecar；任何不符即停止。（影響：高；reader-test：GO，P0=0／open P1=0；formal actual-data：WAIT）
...
RUN_ROOT="$OUT_BASE/m2_frozen_release_v2"
ATTEMPT_PARENT="$OUT_BASE/m2_frozen_release_v2_attempts"
...
c82210f25870d1405cc070c18096fb7d1c2b908fb4d8a7858aece7ac4b151d28  research/20260716_read_linked_hypercube_exact_likelihood_validation/scripts/build_m2_patterns_and_rank.py
