<!--
建立時間: 2026-07-19 11:00 +08:00
目標: 以 exact-preserving prepared-base/cache source 身分，加上 all-sSNV cooccurrence audit 互斥閘門修補，先執行雙 pilot gates，再決定是否進入 M2 全量 extraction/ranking
...
# M2 exact-preserving與資源閘門修補正式執行 Runbook v4

> **TL;DR**：v4延續v3的prepared-base與complete-only process cache，並修正兩個runner漏判`audit_cooccurrence_task_contract_preflight*.py`的資源閘門假陰性；流程仍固定為 `zero-conflict bootstrap → PRE → exact-11 freeze → 雙 pilot 6 gates → extraction 12 gates → ranking 12 gates → POST → independent verifier → final numeric summary → presentation freeze → FINAL HTML → frozen browser QA`。目前scale verdict是`PROBE`；雙pilot未通過前禁止進full 154。每個正式gate仍是不可覆寫JSON＋SHA sidecar，任何不符即停止。（影響：高；resource-gate regression：PASS；formal actual-data：WAIT）
...
RUN_ROOT="$OUT_BASE/m2_frozen_release_v4"
ATTEMPT_PARENT="$OUT_BASE/m2_frozen_release_v4_attempts"
...
1cbdb3ba603de060a6ddb5143c89c796787378d305ee1a4ae9cc102c0765e0b5  research/20260716_read_linked_hypercube_exact_likelihood_validation/scripts/run_full_m2_extraction.py
...
d2b060a6798e9ae966bf2a46c8755ff73ac71c5216ad7507362c518e9ac793a0  research/20260716_read_linked_hypercube_exact_likelihood_validation/scripts/run_full_m2_ranking.py
