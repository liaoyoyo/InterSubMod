<!--
建立時間: 2026-07-18
目標: 獨立檢查 K=11 pre-axis summary totality 修補與 canonical 測試證據
...
# External Reviewer A v8: K=11 summary-totality and source review

Perform a fresh independent, read-only P0 review of the exact live source.
The prior full-scope cooccurrence run completed all worker computation but failed
closed during summary construction. Exactly one selected M1 site had 11 methyl
groups. The locked M2 gate correctly classified that site as not evaluable
before per-axis classification and returned an empty `m2_axis_statuses` object,
but the old summary unconditionally indexed every axis and raised `KeyError:
'hp_exact'`.
...
- Canonical JUnit XML SHA-256 is
  ...
  it records 448 tests and zero failures, errors, or skips.
...
- `summarize_m2_axis_status_counts` derives the exact axis set from
  `M2_GATE.AXIS_SPECS`, accepts an empty axis object only with an explicit
  pre-axis `NOT_EVALUABLE_M2_*` gate, does not invent an effect or p-value,
  rejects partial/extra axis schemas, and conserves all selected site rows for
  every axis.
- Positive and negative tests cover the 11-group pre-axis case and partial-axis
  schema drift.
- New formal paths do not overwrite the failed v6 evidence: cooccurrence output
  ...
  dataset/report paths are v5.
- Both runners bind only v5 authority and retain source checks before/during/
  after computation. Existing v4 artifacts remain historical evidence only.
