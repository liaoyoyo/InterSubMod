#!/usr/bin/env python3
"""Archive signed v16 after C rejected a legacy eight-key stat relation."""
...
CONTINUATION = AUDIT_ROOT / "continue_m2v5_after_tumor_ref_promotion_recovery_v16.py"
FORMAL_ENV = {
    "PATH": "/usr/bin:/bin",
    "HOME": "/tmp",
    "LANG": "C.UTF-8",
...
AUTHORITY_BUNDLE = RESULT_ROOT / "tumor_ref_promotion_schema_recovery_authority.v16.bundle"
VERIFICATION_RECEIPT = (
    RESULT_ROOT / "tumor_ref_source_receipt_promotion_verification.recovery.v16.json"
)
R_OUTPUTS = {
    "replay_receipt": RESULT_ROOT / "m2v5_runner_only_gate_replay.recovery.v16.json",
    "replay_log": RESULT_ROOT / "m2v5_runner_only_gate_replay.recovery.v16.log",
...
SOURCES = {
    "authority_validator": AUDIT_ROOT / "validate_tumor_ref_schema_recovery_authority_v16.py",
    "continuation_verifier": AUDIT_ROOT / "verify_tumor_ref_receipt_promotion_recovery_v16.py",
    "runner_gate_replay": AUDIT_ROOT / "replay_m2v5_runner_only_gates_recovery_v16.py",
    "downstream_continuation": CONTINUATION,
    "readonly_probe": AUDIT_ROOT / "probe_tumor_ref_schema_recovery_sources_v16.py",
    "regression_tests": AUDIT_ROOT / "schema_recovery_tests/test_tumor_ref_schema_recovery_v16.py",
    "ceremony_builder": AUDIT_ROOT / "build_tumor_ref_schema_recovery_authority_v16.py",
    "archive_script": Path(__file__).resolve(strict=True),
}
REVIEW_TRANSPORT = {
    "external_prompt": AUDIT_ROOT / "20260723_external_claude_schema_recovery_review_v16_prompt.md",
    "external_schema": AUDIT_ROOT / "20260723_external_claude_schema_recovery_review_v16.schema.json",
    "external_attempt1_envelope": (
        AUDIT_ROOT / "20260723_external_claude_schema_recovery_review_v16.claude_cli.envelope.json"
    ),
    "external_attempt1_stderr": (
        AUDIT_ROOT / "20260723_external_claude_schema_recovery_review_v16.claude_cli.stderr.txt"
    ),
...
LEGACY_RELATION_PATH = (
    REPO_ROOT
    / "research/20260715_single_fp_alt_multicluster_subclone_validation/scripts/"
...
def find_relation(value: Any) -> list[dict[str, Any]]:
    found: list[dict[str, Any]] = []
    if isinstance(value, dict):
        if (
            value.get("path") == str(LEGACY_RELATION_PATH)
            and sorted(value) == LEGACY_RELATION_KEYS
        ):
            found.append(value)
        for child in value.values():
            found.extend(find_relation(child))
...
def main() -> int:
    ...
    require_absent(C_OUTPUTS, "v16 C outputs before diagnostic replay")
    ...
    if not AUTHORITY_BUNDLE.is_dir() or not VERIFICATION_RECEIPT.is_file():
        raise ArchiveError("v16 signed authority or V receipt is missing")
    if any(not path.is_file() for path in R_OUTPUTS.values()):
        raise ArchiveError("One or more v16 R outputs are missing")
    if any(not path.is_file() for path in REVIEWS.values()):
        raise ArchiveError("One or more v16 formal reviews are missing")
    ...
    if completed.returncode != 1 or completed.stdout != b"":
        raise ArchiveError("Formal v16 C diagnostic did not fail closed as expected")
    ...
    if (
        ...
        raise ArchiveError("Formal v16 C diagnostic error contract drift")
    require_absent(C_OUTPUTS, "v16 C outputs after diagnostic replay")
    require_absent(CANONICAL_DOWNSTREAM_OUTPUTS, "canonical downstream outputs")
    ...
    if (
        ...
        raise ArchiveError("v16 authority/V/R evidence drift")
    if any(
        payload.get("verdict") != "APPROVE" or payload.get("pass") is not True
        for payload in review_payloads.values()
    ):
        raise ArchiveError("v16 formal review verdict drift")
    ...
    completion_payload = load_json(
        RESULT_ROOT / "tumor_ref_source_receipt_promotion.v3.json"
    )
    legacy_records = find_relation(authorization_payload) + find_relation(
        completion_payload
    )
    if (
        len(legacy_records) != 2
        or any(sorted(record) != LEGACY_RELATION_KEYS for record in legacy_records)
        or legacy_records[0] != legacy_records[1]
        or "link_count" in legacy_records[0]
    ):
        raise ArchiveError("Legacy eight-key relation diagnostic drift")
    ...
    if authority_private["mode"] != "0o0":
        raise ArchiveError("v16 authority private key was not retired")
    if continuation_private["mode"] != "0o400":
        raise ArchiveError("v16 continuation private key is not unused mode 0400")
    ...
    evidence = {
        ...
        "generation": "v16",
        "status": "SIGNED_AUTHORITY_V_AND_R_PASS_C_FAILED_BEFORE_CHILD_START",
        "review_contract": {
            key: reviewed[key]
            for key in (
        ...
        "runtime": {
            ...
            "error_message": "Ambiguous identity-like relation schema",
            "failure_location": (
                "downstream_continuation.validate_promotion_chain."
                "bind_declared_relations.authorization_legacy_stat_record"
            ),
        },
        "diagnostic": {
            "legacy_relation_path": str(LEGACY_RELATION_PATH),
            "record_shape": LEGACY_RELATION_KEYS,
            "record": legacy_records[0],
            "link_count_present_in_historical_schema": False,
            "root_cause": (
                "The v16 C relation classifier accepted exact current nine-key stat "
                "records but omitted the exact historical eight-key stat schema used "
                "by both immutable promotion payloads. It therefore failed closed "
                "before starting the supervised child."
            ),
            "required_fix": (
                "Add an exact legacy eight-key stat relation type, project and compare "
                "only its declared fields without inventing link_count, reject every "
                "near-miss shape, and cover the two live promotion payloads in tests."
            ),
            "failure_before_c_child_start": True,
        ...
        "required_resolution": (
            "Create append-only v17 with fresh authority and terminal keys, exact "
            "legacy eight-key stat handling, live promotion-payload regressions, "
    ...
    require_absent(original_slots, "v16 original generation slots after archive")
    require_absent(CANONICAL_DOWNSTREAM_OUTPUTS, "canonical downstream outputs")
    print(
        json.dumps(
            {
                ...
                "v16_original_slots_absent": True,
