#!/usr/bin/env python3
"""Archive signed v17 after C rejected metadata relation enrichment."""
...
CONTINUATION = AUDIT_ROOT / "continue_m2v5_after_tumor_ref_promotion_recovery_v17.py"
FORMAL_ENV = {
    "PATH": "/usr/bin:/bin",
    "HOME": "/tmp",
    "LANG": "C.UTF-8",
...
AUTHORITY_BUNDLE = RESULT_ROOT / "tumor_ref_promotion_schema_recovery_authority.v17.bundle"
VERIFICATION_RECEIPT = (
    RESULT_ROOT / "tumor_ref_source_receipt_promotion_verification.recovery.v17.json"
)
R_OUTPUTS = {
    "replay_receipt": RESULT_ROOT / "m2v5_runner_only_gate_replay.recovery.v17.json",
    "replay_log": RESULT_ROOT / "m2v5_runner_only_gate_replay.recovery.v17.log",
...
SOURCES = {
    "authority_validator": AUDIT_ROOT / "validate_tumor_ref_schema_recovery_authority_v17.py",
    "continuation_verifier": AUDIT_ROOT / "verify_tumor_ref_receipt_promotion_recovery_v17.py",
    "runner_gate_replay": AUDIT_ROOT / "replay_m2v5_runner_only_gates_recovery_v17.py",
    "downstream_continuation": CONTINUATION,
    "readonly_probe": AUDIT_ROOT / "probe_tumor_ref_schema_recovery_sources_v17.py",
    "regression_tests": AUDIT_ROOT / "schema_recovery_tests/test_tumor_ref_schema_recovery_v17.py",
    "ceremony_builder": AUDIT_ROOT / "build_tumor_ref_schema_recovery_authority_v17.py",
    "archive_script": Path(__file__).resolve(strict=True),
}
REVIEW_TRANSPORT = {
    "external_prompt": AUDIT_ROOT / "20260723_external_claude_schema_recovery_review_v17_prompt.md",
    "external_schema": AUDIT_ROOT / "20260723_external_claude_schema_recovery_review_v17.schema.json",
    "external_runner": (
        AUDIT_ROOT / "run_external_claude_schema_recovery_review_v17.py"
    ),
    "external_envelope": (
        AUDIT_ROOT / "20260723_external_claude_schema_recovery_review_v17.claude_cli.envelope.json"
    ),
    "external_stderr": (
        AUDIT_ROOT / "20260723_external_claude_schema_recovery_review_v17.claude_cli.stderr.txt"
    ),
}
...
METADATA_RELATION_KEYS = ["mode", "path"]
METADATA_PLUS_SIZE_STATE_KEYS = ["mode", "path", "size_bytes", "state"]
LEGACY_KEY_POLICY_KEYS = ["must_remain_unretired", "path", "required_mode"]
TRACE_ROOT = Path("/tmp")
TRACE_GLOB = "intersubmod_v17_c_child_write_trace_20260723T0945Z.*"
EXPECTED_TRACE_COUNT = 49
CHILD_ERROR_MESSAGE = (
    "Conflicting declared relation schema or value: "
    + str(LEGACY_PRIVATE_KEY_PATH)
)
CHILD_STDERR_SHA256 = (
...
def find_legacy_private_key_relations(value: Any) -> list[dict[str, Any]]:
    found: list[dict[str, Any]] = []
    if isinstance(value, dict):
        if value.get("path") == str(LEGACY_PRIVATE_KEY_PATH):
            found.append(value)
        for child in value.values():
            found.extend(find_legacy_private_key_relations(child))
...
def main() -> int:
    ...
    require_absent(C_OUTPUTS, "v17 C outputs before diagnostic replay")
    ...
    if not AUTHORITY_BUNDLE.is_dir() or not VERIFICATION_RECEIPT.is_file():
        raise ArchiveError("v17 signed authority or V receipt is missing")
    if any(not path.is_file() for path in R_OUTPUTS.values()):
        raise ArchiveError("One or more v17 R outputs are missing")
    if any(not path.is_file() for path in REVIEWS.values()):
        raise ArchiveError("One or more v17 formal reviews are missing")
    ...
    if (
        ...
        raise ArchiveError("v17 syscall trace evidence is missing or ambiguous")
    ...
    if completed.returncode != 1 or completed.stdout != b"":
        raise ArchiveError("Formal v17 C diagnostic did not fail closed as expected")
    ...
    if (
        ...
        raise ArchiveError("Formal v17 C diagnostic error contract drift")
    require_absent(C_OUTPUTS, "v17 C outputs after diagnostic replay")
    require_absent(CANONICAL_DOWNSTREAM_OUTPUTS, "canonical downstream outputs")
    ...
    if (
        ...
        raise ArchiveError("v17 authority/V/R evidence drift")
    if any(
        payload.get("verdict") != "APPROVE" or payload.get("pass") is not True
        for payload in review_payloads.values()
    ):
        raise ArchiveError("v17 formal review verdict drift")

    legacy_records = find_legacy_private_key_relations(replay_payload)
    observed_shapes = sorted(sorted(record) for record in legacy_records)
    if (
        len(legacy_records) != 3
        or observed_shapes
        != sorted(
            [
                METADATA_RELATION_KEYS,
                METADATA_PLUS_SIZE_STATE_KEYS,
                LEGACY_KEY_POLICY_KEYS,
            ]
        )
        or next(
            record
            for record in legacy_records
            if sorted(record) == METADATA_RELATION_KEYS
        )
        != {"mode": "0o400", "path": str(LEGACY_PRIVATE_KEY_PATH)}
        or next(
            record
            for record in legacy_records
            if sorted(record) == METADATA_PLUS_SIZE_STATE_KEYS
        )
        != {
            "mode": "0o400",
            "path": str(LEGACY_PRIVATE_KEY_PATH),
            "size_bytes": 119,
            "state": "LEGACY_SIGNED_CONTRACT_UNUSED_NOT_RETIRED",
        }
    ):
        raise ArchiveError("Legacy private-key metadata relation diagnostic drift")
    ...
    if authority_private["mode"] != "0o0":
        raise ArchiveError("v17 authority private key was not retired")
    if continuation_private["mode"] != "0o400":
        raise ArchiveError("v17 continuation private key is not unused mode 0400")
    ...
    evidence = {
        ...
        "runtime": {
            ...
            "failure_location": (
                "downstream_continuation.validate_replay_evidence."
                "bind_declared_relations.replay_terminal_key_records"
            ),
        },
        "diagnostic": {
            "legacy_private_key_path": str(LEGACY_PRIVATE_KEY_PATH),
            "relation_records": legacy_records,
            "relation_shapes": observed_shapes,
            "syscall_trace_count": len(trace_archive_records),
            "syscall_trace_captured_exact_child_error": True,
            "root_cause": (
                "The v17 replay embeds the same legacy terminal-v2 private key once "
                "as exact metadata {path,mode} and once as exact metadata-plus-size "
                "{path,mode,size_bytes,state}. Both declarations agree on path and "
                "mode, but the registry treated every cross-kind metadata enrichment "
                "as a conflict. It failed after fresh V and before downstream launch."
            ),
            "required_fix": (
                "Permit only exact metadata-to-metadata-plus-size enrichment when "
                "path and mode agree; upgrade the stored registry record to the richer "
                "schema so later size/state conflicts still fail. Preserve strict "
                "conflict rejection across unrelated kinds and add live replay tests "
                "for terminal-v2, failed-v6, failed-v7, and fresh-v8 key states."
            ),
        ...
        "required_resolution": (
            "Create append-only v18 with fresh authority and terminal-v8 keys, "
            "strict metadata enrichment handling, live replay regressions, fresh "
    ...
    require_absent(original_slots, "v17 original generation slots after archive")
    require_absent(CANONICAL_DOWNSTREAM_OUTPUTS, "canonical downstream outputs")
    print(
        json.dumps(
            {
                ...
                "v17_original_slots_absent": True,
