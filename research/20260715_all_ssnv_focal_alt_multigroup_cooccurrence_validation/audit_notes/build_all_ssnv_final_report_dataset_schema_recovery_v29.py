#!/usr/bin/env python3
"""Build the fail-closed all-sSNV dataset with tumor-REF audit-lineage recovery.
...
normal controls. The only recovery semantic is an exact, independently checked
transition from the immutable tumor-REF v1 pre-audit to the source-authorized
v6 re-audit of the same data plane. It emits data products only.
"""
...
SCRIPT_DIR = RECOVERY_SOURCE.parent.parent / "scripts"
...
LEGACY_PRIMARY_ARTIFACT_AUDIT_SCHEMA_VERSION = "1.0.0"
LEGACY_TUMOR_REF_PRIMARY_AUDIT = (
    SCRIPT_DIR.parent / "results" / "stable_primary_artifact_audit.v1_pre_downstream.json"
)
...
def load_bound_json_artifact(
    ...
    *,
    ...
    """Read, hash, and parse one stable regular-file snapshot from a single FD."""
    candidate = path.expanduser()
    if not candidate.is_absolute() or candidate.resolve(strict=True) != candidate:
        raise ContractError(f"Non-canonical {label} path: {candidate}")
    ...
    if (
        ...
        raise ContractError(f"Unusable {label} artifact: {candidate}")
    ...
    try:
        ...
        if (
            ...
            raise ContractError(f"{label} identity changed during bound read")
    finally:
        os.close(descriptor)
    record = {
        "path": str(candidate),
    ...
    if declared is not None:
        if (
            ...
            or set(declared) != set(record)
            or dict(declared) != record
        ):
            raise ContractError(f"{label} declared identity drift")
    try:
        payload = json.loads(data.decode("utf-8"))
    except (json.JSONDecodeError, UnicodeDecodeError) as error:
        raise ContractError(f"Invalid JSON in {label}: {candidate}") from error
    if not isinstance(payload, dict):
        raise ContractError(f"{label} is not a JSON object: {candidate}")
...
def validate_recovered_primary_artifact_audit_window(
    legacy_pre_audit: Mapping[str, Any],
    current_pre_audit: Mapping[str, Any],
    post_audit: Mapping[str, Any],
    producer_receipts: Mapping[str, Mapping[str, Any]],
) -> dict[str, Any]:
    """Validate tumor-REF in the v1 epoch and every later consumer in the v6 epoch."""
    ...
    def audit_window(
        ...
        if started > finished or created != finished:
            raise ContractError(f"{label} execution time window drift")
        return started, finished

    legacy_started, legacy_finished = audit_window(
        legacy_pre_audit, label="legacy v1 primary artifact audit"
    )
    current_started, current_finished = audit_window(
        current_pre_audit, label="current v6 primary artifact audit"
    )
    post_started, post_finished = audit_window(
        post_audit, label="post-downstream primary artifact audit"
    )
    if not legacy_finished <= current_started <= current_finished <= post_started:
        raise ContractError("Recovered primary audit epoch ordering drift")
    required_producers = {"tumor-REF", "cooccurrence-preflight", "cooccurrence"}
    if not required_producers.issubset(producer_receipts):
        raise ContractError("Recovered primary audit window lacks required producers")
    ...
    for producer, receipt in producer_receipts.items():
        ...
        if started > finished:
            raise ContractError(f"{producer} execution time window is reversed")
        if producer == "tumor-REF":
            if started < legacy_finished or finished > current_started:
                raise ContractError(
                    "tumor-REF did not execute between the legacy v1 audit and current v6 re-audit"
                )
            epoch = "legacy_v1_to_current_v6_reaudit"
        else:
            if started < current_finished or finished > post_started:
                raise ContractError(
                    f"{producer} did not execute inside the current v6/post audit window"
                )
            epoch = "current_v6_to_post_downstream"
        intervals[producer] = {
            "epoch": epoch,
    ...
    return {
        "contract": "tumor_ref_legacy_epoch_plus_current_downstream_epoch_v1",
        "legacy_v1_audit_started_at_utc": legacy_started.isoformat(),
        "legacy_v1_audit_finished_at_utc": legacy_finished.isoformat(),
        "current_v6_audit_started_at_utc": current_started.isoformat(),
        "current_v6_audit_finished_at_utc": current_finished.isoformat(),
        "post_audit_started_at_utc": post_started.isoformat(),
        "post_audit_finished_at_utc": post_finished.isoformat(),
        "producer_intervals": intervals,
        "tumor_ref_uses_legacy_epoch_only": True,
        "all_other_consumers_use_current_v6_epoch": True,
        "pass": True,
    }
...
def _audit_data_artifact(reference: Any, *, label: str) -> dict[str, Any]:
    if not isinstance(reference, Mapping):
        raise ContractError(f"{label} is not an artifact object")
    required = {"path", "size_bytes", "sha256", "mtime_ns"}
    if set(reference) != required:
        raise ContractError(
            f"{label} identity schema drift: observed={sorted(reference)} "
            f"expected={sorted(required)}"
        )
    path = Path(str(reference["path"])).expanduser().resolve()
    require_nonempty_file(path, label)
    if (
        int(reference["size_bytes"]) != path.stat().st_size
        or str(reference["sha256"]) != sha256(path)
        or int(reference["mtime_ns"]) != path.stat().st_mtime_ns
    ):
        raise ContractError(f"{label} live identity drift")
    return dict(reference)


def validate_tumor_ref_primary_audit_lineage(
    reference: Any,
    current_path: Path,
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Validate the one authorized legacy-v1 to current-v6 audit transition."""
    if not isinstance(reference, Mapping):
        raise ContractError("tumor-REF pre-audit reference is not an artifact object")
    declared_value = reference.get("path")
    if not declared_value:
        raise ContractError("tumor-REF pre-audit reference lacks path")
    declared_path = Path(str(declared_value)).expanduser().resolve()
    expected_legacy = LEGACY_TUMOR_REF_PRIMARY_AUDIT.resolve()
    if declared_path != expected_legacy:
        raise ContractError(
            "tumor-REF legacy pre-audit path is not the sole authorized lineage anchor: "
            f"observed={declared_path} expected={expected_legacy}"
        )
    current_path = current_path.expanduser().resolve()
    if current_path != CANONICAL_TASK_B_PATHS["primary_artifact_audit_pre"].resolve():
        raise ContractError("tumor-REF current pre-audit is not the canonical v6 audit")

    legacy, legacy_record = load_bound_json_artifact(
        expected_legacy,
        "tumor-REF legacy v1 primary artifact audit",
        declared=reference,
    )
    current, current_record = load_bound_json_artifact(
        current_path,
        "tumor-REF current v6 primary artifact audit",
    )
    require_schema_pass(
        legacy,
        "intersubmod.stable_primary_artifact_audit",
        "tumor-REF legacy v1 primary artifact audit",
        schema_version=LEGACY_PRIMARY_ARTIFACT_AUDIT_SCHEMA_VERSION,
    )
    require_schema_pass(
        current,
        "intersubmod.stable_primary_artifact_audit",
    ...
    for field_name, expected in shared_scalar_contract.items():
        if legacy.get(field_name) != expected or current.get(field_name) != expected:
            raise ContractError(f"tumor-REF primary audit lineage drift: {field_name}")
    if current.get("formal_task_b_release_eligible") is not True:
        raise ContractError("tumor-REF current v6 audit is not formal-release eligible")

    legacy_inputs = legacy.get("inputs")
    current_inputs = current.get("inputs")
    if not isinstance(legacy_inputs, Mapping) or not isinstance(current_inputs, Mapping):
        raise ContractError("tumor-REF primary audit lineage lacks inputs")
    if legacy_inputs.get("consumer_receipts") != [] or current_inputs.get(
        "consumer_receipts"
    ) != []:
        raise ContractError("tumor-REF primary audit lineage pre-audits must have no consumers")
    legacy_site = _audit_data_artifact(
        legacy_inputs.get("site_results"), label="legacy v1 audit site results"
    )
    current_site = _audit_data_artifact(
        current_inputs.get("site_results"), label="current v6 audit site results"
    )
    legacy_assignments = _audit_data_artifact(
        legacy_inputs.get("stable_assignments"),
        label="legacy v1 audit stable assignments",
    )
    current_assignments = _audit_data_artifact(
        current_inputs.get("stable_assignments"),
        label="current v6 audit stable assignments",
    )
    if legacy_site != current_site or legacy_assignments != current_assignments:
        raise ContractError("tumor-REF v1/v6 audited data-plane identity differs")
    ...
    if legacy.get("counts") != expected_counts or current.get("counts") != expected_counts:
        raise ContractError("tumor-REF v1/v6 primary audit counts differ")
    verification_fields = {
    ...
    legacy_verification = legacy.get("verification")
    current_verification = current.get("verification")
    if not isinstance(legacy_verification, Mapping) or not isinstance(
        current_verification, Mapping
    ):
        raise ContractError("tumor-REF v1/v6 primary audit lacks verification")
    for field_name, expected in verification_fields.items():
        if (
            legacy_verification.get(field_name) != expected
            or current_verification.get(field_name) != expected
        ):
            raise ContractError(
                f"tumor-REF v1/v6 primary audit verification drift: {field_name}"
            )
    artifact_set_sha256 = legacy_verification.get("artifact_set_sha256")
    if (
        not isinstance(artifact_set_sha256, str)
        or len(artifact_set_sha256) != 64
        or artifact_set_sha256 != current_verification.get("artifact_set_sha256")
    ):
        raise ContractError("tumor-REF v1/v6 artifact-set SHA-256 differs")

    lineage = {
        "contract": "immutable_tumor_ref_v1_to_source_authorized_v6_reaudit_lineage_v1",
        "legacy_audit": legacy_record,
        "current_audit": current_record,
        "audited_site_results": legacy_site,
        "audited_stable_assignments": legacy_assignments,
        "counts": expected_counts,
        "artifact_set_sha256": artifact_set_sha256,
        "same_data_plane_identity": True,
        "legacy_receipt_not_rewritten": True,
        "current_v6_is_formal_release_gate": True,
        "pass": True,
        "pass_semantics": (
            "audit_lineage_and_chronology_only_not_scientific_confirmation"
        ),
    }
    return legacy, lineage
...
def load_tumor_ref(
    ...
    legacy_pre_audit, audit_lineage = validate_tumor_ref_primary_audit_lineage(
        inputs.get("primary_artifact_audit_pre"),
        bundle.primary_artifact_audit_pre,
    )
    return rows, summary, receipt, legacy_pre_audit, audit_lineage
...
def integrate_inputs(
    ...
    *,
    ...
    (
        ...
        tumor_ref_summary,
        tumor_ref_receipt,
        legacy_tumor_ref_pre_audit,
        tumor_ref_primary_audit_lineage,
    ) = load_tumor_ref(bundle, assignment_keys)
    ...
    recovered_audit_window = validate_recovered_primary_artifact_audit_window(
        legacy_tumor_ref_pre_audit,
        primary_artifact_audit_pre,
        primary_artifact_audit_post,
    ...
    validations = {
        ...
        "tumor_ref_primary_audit_lineage": tumor_ref_primary_audit_lineage,
        "recovered_primary_artifact_audit_split_window": recovered_audit_window,
