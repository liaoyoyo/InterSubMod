#!/usr/bin/env python3
"""Archive the unused v15 authority and terminal keys after pre-authority rejection."""
...
ENTRIES = (
    {
        "role": "schema_recovery_authority_v15",
        "source": Path(
...
def preflight(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source": str(source),
        "archive": str(archive),
        "reason": "REJECTED_PRE_AUTHORITY_BY_INDEPENDENT_HIGH_AND_MEDIUM_FINDINGS",
...
def main() -> None:
    ...
    occupied = [str(path) for path in FORMAL_V15_SLOTS if os.path.lexists(path)]
    if occupied:
        raise ArchiveError(f"Formal v15 slots are occupied: {occupied}")
    ...
    try:
        append_event(
            ledger_fd,
            {
                ...
                "formal_v15_slot_count": len(FORMAL_V15_SLOTS),
                "formal_v15_slots_all_absent": True,
