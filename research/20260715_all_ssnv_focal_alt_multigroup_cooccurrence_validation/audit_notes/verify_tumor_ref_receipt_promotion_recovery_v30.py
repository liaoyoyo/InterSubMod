#!/usr/bin/env python3
"""Verify the signed tumor-REF promotion through the scoped schema recovery."""
...
AUTHORIZED_VERIFIER = AUDIT_ROOT / "verify_tumor_ref_receipt_promotion_v2.py"
RECOVERY_AUTHORITY_VALIDATOR = (
    AUDIT_ROOT / "validate_tumor_ref_schema_recovery_authority_v30.py"
)
EXPECTED_RECOVERY_AUTHORITY_VALIDATOR_SHA256 = (
...
def _load_recovery_authority_validator() -> tuple[ModuleType, dict[str, Any]]:
    data, record = _bootstrap_open(RECOVERY_AUTHORITY_VALIDATOR)
    if not _strict_json_equal(
        record,
        {
            "path": str(RECOVERY_AUTHORITY_VALIDATOR),
            "size_bytes": len(data),
            "sha256": EXPECTED_RECOVERY_AUTHORITY_VALIDATOR_SHA256,
            "mode": "0o444",
        },
    ):
        raise VerificationError("Recovery authority validator identity drift")
    module = ModuleType("bound_tumor_ref_schema_recovery_authority_validator_v1")
    module.__file__ = str(RECOVERY_AUTHORITY_VALIDATOR)
    module.__package__ = ""
    exec(compile(data, str(RECOVERY_AUTHORITY_VALIDATOR), "exec"), module.__dict__)
    if not hasattr(module, "validate_recovery_authority"):
        raise VerificationError("Recovery authority validator API is incomplete")
    return module, record


def _acquire_recovery_output_writer_lease(module: ModuleType) -> int:
    _, record, descriptor = module._open_bound(module.PUBLIC_KEY)
    if record.get("sha256") != module.EXPECTED_PUBLIC_KEY_SHA256:
        raise VerificationError("Recovery writer-lock public-key identity drift")
    try:
...
def _authorized_verifier_command(mode: str) -> list[str]:
    return [str(PYTHON), "-I", "-B", str(AUTHORIZED_VERIFIER), mode]
...
def _require_downstream_output_state(mode: str) -> None:
    if mode not in {"--verify", "--verify-and-record"}:
        raise VerificationError("Unknown downstream-output verification stage")
    ...
    if mode == "--verify-and-record":
        required_present: tuple[Path, ...] = ()
    else:
        replay_presence = tuple(presence[path] for path in RECOVERY_REPLAY_OUTPUTS)
        if any(replay_presence) and not all(replay_presence):
            raise VerificationError(
                "Downstream output stage contract drift: "
                "mode=--verify, partial_replay_evidence=true"
            )
        required_present = RECOVERY_REPLAY_OUTPUTS if all(replay_presence) else ()
    forbidden = tuple(path for path in DOWNSTREAM_OUTPUTS if path not in required_present)
    missing = [str(path) for path in required_present if not presence[path]]
    occupied = [str(path) for path in forbidden if presence[path]]
    if missing or occupied:
        raise VerificationError(
            "Downstream output stage contract drift: "
            f"mode={mode}, missing_required={missing}, occupied_forbidden={occupied}"
        )
...
def _require_historical_execution_artifact(value: Any, label: str) -> None:
    _require_exact_keys(value, HISTORICAL_EXECUTION_ARTIFACT_KEYS, label)
    if not isinstance(value.get("path"), str) or not value["path"].startswith("/"):
        raise VerificationError(f"{label} path is not an absolute string")
    if type(value.get("size_bytes")) is not int or value["size_bytes"] < 0:
        raise VerificationError(f"{label} size_bytes is not a non-negative JSON integer")
...
def _validate_authorization_evidence(
    ...
    *,
    ...
    _require_historical_execution_artifact(
        transition["during_execution"], "focal source during-execution identity"
    )
...
def _verification_payload(
    *,
    ...
    recovery_authority_evidence: Mapping[str, Any],
    retained_descriptor_count: int,
) -> dict[str, Any]:
    return {
        "schema_name": RECOVERY_VERIFICATION_SCHEMA,
        ...
        "schema_recovery_authority": dict(recovery_authority_evidence),
        "historical_identity_contract": {
            "during_execution_keys": sorted(HISTORICAL_EXECUTION_ARTIFACT_KEYS),
            "current_identity_keys": sorted(STAT_ARTIFACT_KEYS),
...
def _verify(mode: str) -> tuple[dict[str, Any], dict[str, Any] | None]:
    ...
    _require_downstream_output_state(mode)
    authority_module, bootstrap_authority_record = _load_authority_module()
    recovery_validator, recovery_validator_record = (
        _load_recovery_authority_validator()
    )
    recovery_authority_evidence = recovery_validator.validate_recovery_authority(
        expected_runtime_role="continuation_verifier",
        expected_recovery_continuation_private_mode="0o400",
    )
    writer_lease_fd = (
        _acquire_recovery_output_writer_lease(recovery_validator)
        if mode == "--verify-and-record"
        else -1
    )
    del writer_lease_fd
    ...
    with authority_module.BoundArtifactReader() as reader:
        ...
        if (
            not _strict_json_equal(
                recovery_authority_evidence.get("runtime_source"), self_record
            )
            or recovery_authority_evidence.get("authority_id")
            != "20260724_tumor_ref_schema_recovery_v30"
            or recovery_authority_evidence.get("pass") is not True
        ):
            raise VerificationError("Recovery authority does not bind this verifier")
        if (
        ...
        _, _, authorized_verifier_record, authorized_verifier_stat = _open_regular(
            reader, AUTHORIZED_VERIFIER
        )
        _require_mode_and_link_count(
            authorized_verifier_record,
            authorized_verifier_stat,
            "signed-authorization verifier source",
        )
        ...
        if recovery_validator_record.get("sha256") != (
            ...
            raise VerificationError("Recovery authority validator binding drift")
        ...
        expected_commands = {
            ...
            "continuation_verify": _authorized_verifier_command("--verify"),
        ...
        if (
            ...
            or not _strict_json_equal(
                ...
                [str(path) for path in AUTHORIZED_DOWNSTREAM_OUTPUTS],
            )
            or not _strict_json_equal(
        ...
        retained_count = (
            reader.opened_descriptor_count
            + len(_BOOTSTRAP_FD_LEASES)
            + int(recovery_authority_evidence["descriptor_lease_count"])
        )
        payload = _verification_payload(
            ...
            recovery_authority_evidence=recovery_authority_evidence,
...
def main() -> int:
    ...
    try:
        ...
        failure = {
            "schema_name": f"{RECOVERY_VERIFICATION_SCHEMA}.error",
            "schema_version": RECOVERY_SCHEMA_VERSION,
            "error": {
