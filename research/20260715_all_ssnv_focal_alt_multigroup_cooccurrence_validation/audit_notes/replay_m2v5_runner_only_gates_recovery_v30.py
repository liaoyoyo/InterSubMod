...
promotion first, binds the exact original physical lines 1-358 of the
v7-authorized completion runner, executes a deterministic three-path
archive-rebased derivative through a bound Bash descriptor, and publishes
immutable evidence only when every check passes. It never executes line 359 or
later and does not itself authorize downstream execution.
"""
...
AUTHORIZED_REPLAYER = AUDIT_ROOT / "replay_m2v5_runner_only_gates_v1.py"
...
AUTHORIZED_DOWNSTREAM_CONTINUATION = (
...
OUTPUT_LOG = RESULT_ROOT / "m2v5_runner_only_gate_replay.recovery.v30.log"
OUTPUT_RECEIPT = RESULT_ROOT / "m2v5_runner_only_gate_replay.recovery.v30.json"
OUTPUT_SUCCESS_WITNESS = (
    RESULT_ROOT / "m2v5_runner_only_gate_replay.recovery.v30.success_witness.json"
)
OUTPUT_SUCCESS_WITNESS_SCHEMA = (
    "intersubmod.m2v5_runner_only_gate_replay.recovery.success_witness"
)
OUTPUT_SUCCESS_WITNESS_KEYS = frozenset(
    {
        "schema_name",
        "schema_version",
        "created_at_utc",
...
AUTHORIZED_OUTPUT_RECEIPT = RESULT_ROOT / "m2v5_runner_only_gate_replay.v1.json"
...
RUNNER_ARCHIVE_REBASE = (
    (
        b"/bip7_disk/liaoyoyo2001/.config/intersubmod_result_release_authority/"
        b"20260719_all_ssnv_result_v5_post_reboot_bootstrap/ed25519_public.pem",
        os.fsencode(FAILED_V28_RESULT_PUBLIC_KEY),
        "failed_v28_result_public_key",
    ),
    (
        b"/bip7_disk/liaoyoyo2001/.config/intersubmod_report_release_authority/"
        b"20260719_all_ssnv_report_v5_post_reboot_bootstrap/ed25519_public.pem",
        os.fsencode(FAILED_V28_REPORT_PUBLIC_KEY),
        "failed_v28_report_public_key",
    ),
    (
        b"/bip7_disk/liaoyoyo2001/.config/intersubmod_report_release_authority/"
        b"20260719_all_ssnv_report_v5_post_reboot_bootstrap/"
        b"ed25519_private_encrypted_unrecoverable_after_signing.pem",
        os.fsencode(FAILED_V28_REPORT_PRIVATE_KEY),
        "failed_v28_report_private_key_metadata_only",
    ),
)
FAILED_V28_LIVE_KEY_PATHS = tuple(
    Path(os.fsdecode(original_path))
    for original_path, _, _ in RUNNER_ARCHIVE_REBASE
)
...
VERIFICATION_CHECK_KEYS = frozenset(
    {
        ...
        "signed_schema_recovery_authority_verified",
        "historical_eight_key_identity_validated_without_inferred_link_count",
        "current_nine_key_identity_and_link_count_one_verified",
    }
)
VERIFICATION_GOVERNANCE_KEYS = frozenset(
    {
        ...
        "verification_scope",
        "schema_recovery_scope",
        "biological_claim_ceiling_changed",
    }
)

BASIC_ARTIFACT_KEYS = frozenset({"path", "size_bytes", "sha256", "mode"})
METADATA_PLUS_SIZE_STATE_KEYS = frozenset({"path", "size_bytes", "mode", "state"})
...
HISTORICAL_EXECUTION_ARTIFACT_KEYS = frozenset(
    {
        "path",
        "size_bytes",
        "sha256",
...
FOCAL_TRANSITION_KEYS = frozenset(
    {
        "during_execution",
        "current",
        "unchanged_fields",
...
FOCAL_TRANSITION_LOCATION = ("evidence", "focal_source_identity_transition")
...
DOWNSTREAM_OUTPUT_SLOTS = CURRENT_DOWNSTREAM_OUTPUT_SLOTS + (
    ...
    RESULT_ROOT
    / "m2v5_downstream_continuation_supervisor_success.recovery.v30.json",
...
def critical_globals_equal(
    ...
    names = (
        ...
        "OUTPUT_SUCCESS_WITNESS",
        "OUTPUT_SUCCESS_WITNESS_SCHEMA",
        "OUTPUT_SUCCESS_WITNESS_KEYS",
        "OUTPUT_SUCCESS_WITNESS_CHECKS",
        "AUTHORIZED_OUTPUT_RECEIPT",
        "AUTHORIZED_DOWNSTREAM_OUTPUT_SLOTS",
        "CURRENT_DOWNSTREAM_OUTPUT_SLOTS",
        "DOWNSTREAM_OUTPUT_SLOTS",
        "EXPECTED_ENVIRONMENT",
        "EXPECTED_RUNNER_LINE_COUNT_MINIMUM",
        "RUNNER_INCLUDED_LINE_START",
        "RUNNER_INCLUDED_LINE_END",
        "RUNNER_FIRST_EXCLUDED_LINE",
        "EXPECTED_RUNNER_PREFIX_SHA256",
        "EXPECTED_ARCHIVE_REBASED_RUNNER_PREFIX_SHA256",
        ...
        "HISTORICAL_EXECUTION_ARTIFACT_KEYS",
        "FOCAL_TRANSITION_KEYS",
        "FOCAL_TRANSITION_LOCATION",
...
class ExecutableAliasMutationWatch:
    """Permanently fail after any observed mutation of an alias or target."""

    _EVENT_HEADER_SIZE = 16
    _TERMINAL_MASK = 0x00000400 | 0x00000800 | 0x00004000 | 0x00008000
    ...
    def __init__(self, alias_path: Path, target_path: Path) -> None:
        ...
        if init is None or add_watch is None:
            raise GateError("Linux inotify is required for executable alias monitoring")
        ...
        if self._fd < 0:
            ...
            raise GateError(
                "inotify_init1 failed for executable alias: "
        ...
        try:
            for parent, name in (
                ...
                if watch < 0:
                    error_number = ctypes.get_errno()
                    raise GateError(
                        f"inotify parent watch failed for {parent}: "
                        + os.strerror(error_number)
                    )
            ...
            if target_watch < 0:
                error_number = ctypes.get_errno()
                raise GateError(
                    f"inotify target watch failed for {target_path}: "
                    + os.strerror(error_number)
                )
    ...
    def _fail(self, reason: str) -> None:
        ...
        raise GateError(self._violation)
    ...
    def assert_clean(self) -> None:
        ...
        while True:
            try:
                ...
                self._fail(f"Executable alias inotify read failed: {error}")
            if not data:
                self._fail("Executable alias inotify descriptor closed unexpectedly")
            offset = 0
            while offset < len(data):
                if len(data) - offset < self._EVENT_HEADER_SIZE:
                    self._fail("Truncated executable alias inotify event")
                ...
                if end > len(data):
                    self._fail("Malformed executable alias inotify event")
                raw_name = data[offset + self._EVENT_HEADER_SIZE : end]
                name = os.fsdecode(raw_name.split(b"\0", 1)[0])
                if mask & self._TERMINAL_MASK:
                    self._fail(
                        f"Executable alias terminal mutation event observed: mask={mask:#x}"
                    )
                if watch in self._file_watches:
                    self._fail(
                        f"Executable alias target mutation event observed: mask={mask:#x}"
                    )
                if name in self._parent_watches.get(watch, set()):
                    self._fail(
                        "Executable alias pathname mutation event observed: "
                        f"name={name!r} mask={mask:#x}"
                    )
                offset = end
...
class BoundArtifactReader:
    ...
    def __init__(self) -> None:
        ...
        self._aliases: dict[
            ...
            tuple[
                ...
                str,
                Path,
                "ExecutableAliasMutationWatch",
            ],
        ] = {}
        self._declared_relations: dict[
    ...
    def open_metadata_with_size(self, path: Path) -> tuple[int, dict[str, Any]]:
        """Bind non-readable key state and expose only descriptor metadata."""
        descriptor, record = self.open_metadata(path)
        opened = self.stat_for(path)
        return descriptor, {**record, "size_bytes": opened.st_size}

    def register_declared_relation(
        self,
        path: Path,
        *,
        kind: str,
        ...
        """Reject traversal-order-dependent relation reclassification."""
        resolved, _ = self._canonical_regular_path(path)
        normalized = dict(declared)
        existing = self._declared_relations.get(resolved)
        if existing is not None:
            existing_kind, existing_declared = existing
            if existing_kind != kind or not strict_json_equal(
                existing_declared, normalized
            ):
                raise GateError(
                    f"Conflicting declared relation schema or value: {resolved}"
                )
            return
        self._declared_relations[resolved] = (kind, normalized)
    ...
    def open_executable_alias(
        ...
        *,
        ...
        expected_target_size_bytes: int,
        ...
        if (
            ...
            or expected_resolved_path.resolve(strict=True) != expected_resolved_path
        ):
            raise GateError("Executable alias or target path is not canonical")
        if not hasattr(os, "O_PATH"):
            raise GateError("O_PATH is required for executable alias binding")
        mutation_watch = ExecutableAliasMutationWatch(path, expected_resolved_path)
        alias_fd = -1
        try:
            ...
            descriptor_resolved = (
                ...
                else path.parent / descriptor_link_text
            ).resolve(strict=True)
            if (
                not stat.S_ISLNK(opened_alias.st_mode)
                or not _same_stat(alias_stat, opened_alias)
                or descriptor_link_text != expected_link_text
                or descriptor_resolved != expected_resolved_path
            ):
            ...
            expected_target_record = {
                ...
                "size_bytes": expected_target_size_bytes,
                "sha256": expected_target_sha256,
                "mode": expected_target_mode,
            }
            if not strict_json_equal(target_record, expected_target_record):
                raise GateError(f"Executable alias target identity drift: {path}")
            target_stat = self.stat_for(expected_resolved_path)
            live_alias = os.lstat(path)
            if (
                not _same_stat(opened_alias, live_alias)
                or os.readlink(path) != expected_link_text
                or os.readlink("", dir_fd=alias_fd) != expected_link_text
                or path.resolve(strict=True) != expected_resolved_path
                or not _same_stat(os.stat(path), target_stat)
            ):
                raise GateError(f"Executable alias target binding drift: {path}")
    ...
    def require_paths_still_bound(self) -> None:
        for _, (_, _, _, _, mutation_watch) in self._aliases.items():
            mutation_watch.assert_clean()
        ...
        for path, (
            ...
            try:
                descriptor_stat = os.fstat(descriptor)
                live = os.lstat(path)
                live_link_text = os.readlink(path)
                descriptor_link_text = os.readlink("", dir_fd=descriptor)
                live_resolved = path.resolve(strict=True)
            except OSError as error:
                raise GateError(f"Bound executable alias disappeared: {path}") from error
            if (
                not stat.S_ISLNK(live.st_mode)
                or not _same_stat(descriptor_stat, opened)
                or not _same_stat(live, opened)
                or live_link_text != link_text
                or descriptor_link_text != link_text
                or live_resolved != resolved
            ):
    ...
    def close(self) -> None:
        if self._retained:
            return
        while self._aliases:
            _, (descriptor, _, _, _, mutation_watch) = self._aliases.popitem()
            mutation_watch.close()
            os.close(descriptor)
        while self._opened:
            _, (descriptor, _, _, _) = self._opened.popitem()
            os.close(descriptor)
        self._declared_relations.clear()
...
def authorized_replayer_command() -> list[str]:
    return [str(PYTHON), "-I", "-B", str(AUTHORIZED_REPLAYER)]
...
def authorized_downstream_continuation_command() -> list[str]:
    return [
        str(PYTHON),
        "-I",
        "-B",
        str(AUTHORIZED_DOWNSTREAM_CONTINUATION),
        "--supervise-and-sign",
    ]
...
def require_historical_execution_artifact_types(value: Any, label: str) -> None:
    require_exact_keys(value, HISTORICAL_EXECUTION_ARTIFACT_KEYS, label)
    require_basic_artifact_types(value, label)
    for field in ("device", "inode", "mtime_ns", "ctime_ns"):
        ...
        if type(observed) is not int or observed < 0:
            raise GateError(f"{label} {field} is not a non-negative JSON integer")
    if value["inode"] <= 0:
        raise GateError(f"{label} inode is not positive")


def validate_focal_source_identity_transition(
    value: Any,
    reader: BoundArtifactReader,
) -> dict[str, Any]:
    require_exact_keys(value, FOCAL_TRANSITION_KEYS, "focal source identity transition")
    during = value["during_execution"]
    current = value["current"]
    require_historical_execution_artifact_types(
        during, "focal source historical during-execution identity"
    )
    require_full_stat_artifact_types(current, "focal source current identity")

    current_path = Path(current["path"])
    _, _, live = reader.open(current_path)
    live_current = artifact_with_stat(live, reader.stat_for(current_path))
    unchanged_fields = ["path", "size_bytes", "sha256", "device", "inode", "mtime_ns"]
    expected_differences = {
        "mode": {"during": "0o664", "current": "0o444"},
        "ctime_ns": {"during": during["ctime_ns"], "current": current["ctime_ns"]},
    }
    if (
        not strict_json_equal(current, live_current)
        or not strict_json_equal(value["unchanged_fields"], unchanged_fields)
        or any(
            not strict_json_equal(during[field], current[field])
            for field in unchanged_fields
        )
        or during["mode"] != "0o664"
        or current["mode"] != "0o444"
        or current["link_count"] != 1
        or during["ctime_ns"] >= current["ctime_ns"]
        or not strict_json_equal(value["observed_differences"], expected_differences)
        ...
        raise GateError("Focal source historical/current transition contract drift")
    return live_current


def require_signed_focal_transitions_equal(
    authorization: Mapping[str, Any],
    promotion: Mapping[str, Any],
) -> None:
    try:
        authorization_transition = authorization["evidence"][
            "focal_source_identity_transition"
        ]
        promotion_transition = promotion["evidence"][
            "focal_source_identity_transition"
        ]
    except (KeyError, TypeError) as error:
        raise GateError("Signed focal source transition is missing") from error
    if not strict_json_equal(authorization_transition, promotion_transition):
        raise GateError("Authorization/promotion focal source transitions differ")
...
def acquire_recovery_output_writer_lease(module: types.ModuleType) -> int:
    _, record, descriptor = module._open_bound(module.PUBLIC_KEY)
    if record.get("sha256") != module.EXPECTED_PUBLIC_KEY_SHA256:
        raise GateError("Recovery writer-lock public-key identity drift")
    try:
...
def recursively_validate_artifact_relations(
    ...
    reader: BoundArtifactReader,
    *,
    require_focal_transition: bool = False,
) -> dict[str, dict[str, Any]]:
    """Bind live artifacts while preserving the one signed historical observation."""
    relations: dict[str, dict[str, Any]] = {}
    focal_transition_count = 0

    def visit(value: Any, location: tuple[str, ...]) -> None:
        nonlocal focal_transition_count
        if isinstance(value, Mapping):
            if location == FOCAL_TRANSITION_LOCATION:
                focal_transition_count += 1
                validate_focal_source_identity_transition(value, reader)
            path_value = value.get("path")
            mode_value = value.get("mode")
            is_historical_observation = location == (
                *FOCAL_TRANSITION_LOCATION,
                "during_execution",
            )
            if (
                ...
                and not is_historical_observation
            ):
                path = Path(path_value)
                if "sha256" in value:
                    if "size_bytes" not in value:
                        raise GateError(
                            f"Verification receipt content relation lacks size_bytes: {path}"
                        )
                    declared = {
                        key: value[key]
                        for key in ("path", "size_bytes", "sha256", "mode")
                    }
                    reader.register_declared_relation(
                        path, kind="content", declared=declared
                    )
                ...
                elif "size_bytes" in value:
                    if (
                        set(value) != set(METADATA_PLUS_SIZE_STATE_KEYS)
                        or mode_value not in {"0o0", "0o400"}
                        ...
                        raise GateError(
                            f"Verification receipt metadata-plus-size schema drift: {path}"
                        )
                    reader.register_declared_relation(
                        path,
                        kind="metadata_plus_size",
                        declared={key: value[key] for key in METADATA_PLUS_SIZE_STATE_KEYS},
                    )
                    _, live = reader.open_metadata_with_size(path)
                    for key in ("path", "size_bytes", "mode"):
                        if not strict_json_equal(value.get(key), live.get(key)):
                            raise GateError(
                                f"Verification receipt metadata-plus-size relation drift: "
                                f"{path} field={key}"
                            )
                    relations[str(path)] = live
                elif mode_value == "0o0":
                    reader.register_declared_relation(
                        path,
                        kind="metadata",
                        declared={"path": path_value, "mode": mode_value},
                    )
    ...
    visit(payload, ())
    if require_focal_transition and focal_transition_count != 1:
        raise GateError(
            "Signed payload does not contain exactly one focal source identity transition"
        )
    if not require_focal_transition and focal_transition_count != 0:
        raise GateError("Unexpected focal source identity transition in relation payload")
    return relations
...
def validate_verification_receipt(
    ...
    *,
    ...
    if (
        ...
        or governance.get("schema_recovery_scope")
        != "historical_identity_schema_compatibility_only"
        or governance.get("biological_claim_ceiling_changed") is not False
...
def validate_authorized_gate_contract(
    ...
    *,
    ...
    expected_terminal_signature_contract = {
        ...
        "supervisor_command": authorized_downstream_continuation_command(),
    ...
    if (
        ...
        or not strict_json_equal(
            commands.get("runner_gate_replay"), authorized_replayer_command()
        )
        or not strict_json_equal(
            commands.get("continuation_verify"), authorized_verifier_command()
        )
        or not strict_json_equal(
            commands.get("downstream_continuation"),
            authorized_downstream_continuation_command(),
        )
        ...
        or not strict_json_equal(
            continuation.get("supervised_child_command"),
        ...
        or continuation.get("verification_receipt")
        != str(AUTHORIZED_PROMOTION_VERIFICATION_RECEIPT)
        or continuation.get("runner_gate_receipt") != str(AUTHORIZED_OUTPUT_RECEIPT)
        or not strict_json_equal(
    ...
    return {
        ...
        "authorization_commands": {
            ...
            "runner_gate_replay": authorized_replayer_command(),
            "downstream_continuation": authorized_downstream_continuation_command(),
        },
        "runtime_artifacts": {
...
def catchable_signal_set() -> set[signal.Signals]:
    ...
    if not blocked:
        raise GateError("No catchable signals are available for terminal publication")
    return blocked
    ...
    try:
        ...
        if terminal_commit is not None:
            if not hasattr(signal, "pthread_sigmask"):
                raise GateError("pthread_sigmask is required for terminal publication")
            previous_signal_mask = signal.pthread_sigmask(
                signal.SIG_BLOCK, catchable_signal_set()
            )
            try:
                terminal_commit()
                terminal_staged = os.fstat(descriptor)
                terminal_parent = os.stat(path.parent, follow_symlinks=False)
                if (
                    not _same_stat(opened, terminal_staged)
                    or terminal_staged.st_nlink != 0
                    or os.pread(descriptor, len(data), 0) != data
                    or (
                        parent_opened.st_dev,
                        parent_opened.st_ino,
                    ...
                    != (
                        ...
                        terminal_parent.st_mode,
                    )
                ):
                    raise GateError(
                        f"Terminal publication input changed before commit: {path}"
                    )
                require_absent(path, "terminal publication target")
            except BaseException:
...
def validate_archive_rebase_inputs(
    *,
    result_archive_data: bytes,
    result_archive_record: Mapping[str, Any],
    result_public_record: Mapping[str, Any],
    report_archive_data: bytes,
    report_archive_record: Mapping[str, Any],
    report_public_record: Mapping[str, Any],
    report_private_record: Mapping[str, Any],
    report_private_link_count: int,
) -> dict[str, Any]:
    result_payload = parse_json(result_archive_data, "failed-v28 result key archive")
    report_payload = parse_json(report_archive_data, "failed-v28 report key archive")
    if (
        ...
        != EXPECTED_FAILED_V28_REPORT_PUBLIC_KEY_SHA256
        or result_public_record.get("mode") != "0o444"
        or report_public_record.get("mode") != "0o444"
        or report_private_record
        != {
            "path": str(FAILED_V28_REPORT_PRIVATE_KEY),
            "mode": "0o400",
            "size_bytes": 290,
        }
        or report_private_link_count != 1
    ):
        raise GateError("Failed-v28 archive rebase artifact identity drift")
    ...
    for role, payload, archive_root, public_record, state in (
        ...
        if (
            ...
            raise GateError(f"Failed-v28 {role} archive record contract drift")
    report_private_declared = report_payload.get("private_key_metadata_only")
    if (
        ...
        raise GateError("Failed-v28 report private-key metadata contract drift")
    return {
        ...
        "report_private_key_metadata_only": report_private_record,
        "report_private_key_link_count": report_private_link_count,
        "private_key_bytes_read": False,
...
def archive_rebase_runner_prefix(
    runner_prefix: bytes,
) -> tuple[bytes, dict[str, Any]]:
    if sha256_bytes(runner_prefix) != EXPECTED_RUNNER_PREFIX_SHA256:
        raise GateError("Original runner prefix identity drift before archive rebase")
    transformed = runner_prefix
    mappings: list[dict[str, Any]] = []
    for old_path, archive_path, role in RUNNER_ARCHIVE_REBASE:
        if transformed.count(old_path) != 1 or transformed.count(archive_path) != 0:
            raise GateError(f"Runner archive-rebase cardinality drift: {role}")
        transformed = transformed.replace(old_path, archive_path, 1)
        mappings.append(
            {
                "role": role,
    ...
    if transformed_sha256 != EXPECTED_ARCHIVE_REBASED_RUNNER_PREFIX_SHA256:
        raise GateError("Archive-rebased runner prefix identity drift")
    return transformed, {
        "contract": "deterministic_three_path_archive_rebase_v1",
        "original_prefix": {
            "size_bytes": len(runner_prefix),
            "sha256": sha256_bytes(runner_prefix),
        },
...
def build_log(
    ...
    runner_prefix_sha256: str,
    archive_rebased_prefix_sha256: str,
) -> bytes:
    def decoded(data: bytes) -> str:
        return data.decode("utf-8", errors="backslashreplace")

    sections = [
        "InterSubMod M2v5 runner-only gate replay transition-recovery v30\n",
        ...
        "CLAIM: This archive-rebased pathname replay is non-authoritative and does not authorize downstream execution.\n",
        f"Legacy signed verifier command: {json.dumps(authorized_verifier_command())}\n",
        f"Recovery-authorized verifier command: {json.dumps(verifier_command())}\n",
        ...
        f"Archive-rebased runner prefix SHA-256: {archive_rebased_prefix_sha256}\n",
        ...
        "\n--- terminal status ---\nPASS EVIDENCE ONLY: archive-rebased runner lines 1-358 executed; original bytes were bound but not executed; downstream not executed or authorized.\n",
    ]
...
def worker_main() -> int:
    ...
    require_absent(OUTPUT_SUCCESS_WITNESS, "runner gate success witness")
    ...
    failed_v28_live_keys_before = absence_snapshot(
        FAILED_V28_LIVE_KEY_PATHS, "pre-verifier failed-v28 live key paths"
    )
    ...
    try:
        ...
        _, _, authorized_replayer_record = reader.open(AUTHORIZED_REPLAYER)
        verifier_fd, verifier_data, verifier_record = reader.open(CONTINUATION_VERIFIER)
        _, _, authorized_verifier_record = reader.open(
            AUTHORIZED_CONTINUATION_VERIFIER
        )
        continuation_fd, continuation_data, continuation_record = reader.open(
            DOWNSTREAM_CONTINUATION
        )
        _, _, authorized_continuation_record = reader.open(
            AUTHORIZED_DOWNSTREAM_CONTINUATION
        )
        ...
        python_fd, _, python_record = reader.open_executable_alias(
            ...
            expected_target_size_bytes=EXPECTED_RUNTIME_ARTIFACTS["python"][
                "size_bytes"
            ],
            expected_target_sha256=EXPECTED_RUNTIME_ARTIFACTS["python"]["sha256"],
            expected_target_mode=EXPECTED_RUNTIME_ARTIFACTS["python"]["mode"],
        )
        ...
        _, recovery_validator_data, recovery_validator_record = reader.open(
            SCHEMA_RECOVERY_VALIDATOR
        )
        runner_fd, runner_data, runner_record = reader.open(COMPLETION_RUNNER)
        (
            _,
            failed_v28_result_archive_data,
            failed_v28_result_archive_record,
        ) = reader.open(FAILED_V28_RESULT_KEY_ARCHIVE_RECORD)
        _, _, failed_v28_result_public_record = reader.open(
            FAILED_V28_RESULT_PUBLIC_KEY
        )
        (
            _,
            failed_v28_report_archive_data,
            failed_v28_report_archive_record,
        ) = reader.open(FAILED_V28_REPORT_KEY_ARCHIVE_RECORD)
        _, _, failed_v28_report_public_record = reader.open(
            FAILED_V28_REPORT_PUBLIC_KEY
        )
        _, failed_v28_report_private_record = reader.open_metadata_with_size(
            FAILED_V28_REPORT_PRIVATE_KEY
        )
        failed_v28_report_private_stat = reader.stat_for(
            FAILED_V28_REPORT_PRIVATE_KEY
        )
        archive_rebase_inputs = validate_archive_rebase_inputs(
            result_archive_data=failed_v28_result_archive_data,
            result_archive_record=failed_v28_result_archive_record,
            result_public_record=failed_v28_result_public_record,
            report_archive_data=failed_v28_report_archive_data,
            report_archive_record=failed_v28_report_archive_record,
            report_public_record=failed_v28_report_public_record,
            report_private_record=failed_v28_report_private_record,
            report_private_link_count=failed_v28_report_private_stat.st_nlink,
        )
        ...
        if any(
            ...
            for record in (
                self_record,
                authorized_replayer_record,
                verifier_record,
                authorized_verifier_record,
                continuation_record,
                authorized_continuation_record,
                ...
                recovery_validator_record,
                runner_record,
                failed_v28_result_archive_record,
                failed_v28_result_public_record,
                failed_v28_report_archive_record,
                failed_v28_report_public_record,
            )
        ):
        ...
        if recovery_validator_record.get("sha256") != (
            ...
            raise GateError("Schema-recovery authority validator identity drift")
        recovery_module = load_bound_authority_module(
            recovery_validator_data, SCHEMA_RECOVERY_VALIDATOR
        )
        if not hasattr(recovery_module, "validate_recovery_authority"):
            raise GateError("Schema-recovery authority validator API is incomplete")
        recovery_authority = recovery_module.validate_recovery_authority(
            expected_runtime_role="runner_gate_replay",
            expected_recovery_continuation_private_mode="0o400",
        )
        recovery_writer_lease_fd = acquire_recovery_output_writer_lease(
            recovery_module
        )
        del recovery_writer_lease_fd
        if (
            ...
            raise GateError("Schema-recovery authority does not bind this replayer")
        ...
        expected_reviewed_sources = {
            ...
            "runner_gate_replay": authorized_replayer_record,
            "downstream_continuation": authorized_continuation_record,
        ...
        source_full_record = artifact_with_stat(
            source_record, reader.stat_for(HISTORICAL_SOURCE_RECEIPT)
        )
        canonical_full_record = artifact_with_stat(
            canonical_record, reader.stat_for(CANONICAL_SOURCE_RECEIPT)
        )
        require_full_stat_artifact_types(
            source_full_record, "historical source receipt replay record"
        )
        require_full_stat_artifact_types(
            canonical_full_record, "canonical source receipt replay record"
        )
        ...
        require_signed_focal_transitions_equal(
            authorization_payload, promotion_payload
        )
        authorization_relations = recursively_validate_artifact_relations(
            authorization_payload, reader, require_focal_transition=True
        )
        promotion_relations = recursively_validate_artifact_relations(
            promotion_payload, reader, require_focal_transition=True
        )
        ...
        runtime_contract = validate_authorized_gate_contract(
            ...
            self_record=authorized_replayer_record,
            verifier_record=authorized_verifier_record,
            continuation_record=authorized_continuation_record,
        ...
        archive_rebased_runner_prefix, archive_rebase_contract = (
            archive_rebase_runner_prefix(runner_prefix)
        )
        require_artifact_equal(
        ...
        failed_v28_live_keys_after_verifier = absence_snapshot(
            FAILED_V28_LIVE_KEY_PATHS,
            "post-verifier failed-v28 live key paths",
        )
        reader.require_paths_still_bound()
        replay_result = subprocess.run(
            [f"/proc/self/fd/{bash_fd}", "-s"],
            input=archive_rebased_runner_prefix,
            cwd=REPO_ROOT,
            env=EXPECTED_ENVIRONMENT,
            pass_fds=(bash_fd,),
            capture_output=True,
        ...
        failed_v28_live_keys_after_replay = absence_snapshot(
            FAILED_V28_LIVE_KEY_PATHS,
            "post-replay failed-v28 live key paths",
        )
        ...
        log_data = build_log(
            verifier_result,
            replay_result,
            runner_prefix_sha256,
            archive_rebase_contract["archive_rebased_prefix"]["sha256"],
        )
        reader.require_paths_still_bound()
        def log_precommit() -> None:
            ...
            absence_snapshot(
                FAILED_V28_LIVE_KEY_PATHS,
                "pre-runner-log failed-v28 live key paths",
            )
            reader.require_paths_still_bound()
        ...
        receipt_payload = {
            "schema_name": "intersubmod.m2v5_runner_only_gate_replay.recovery",
            "schema_version": "1.0.0",
            "created_at_utc": replay_created_at,
            "task_type": "B_comprehensive_validation",
            "status": "ARCHIVE_REBASED_RUNNER_PREFIX_REPLAY_EVIDENCE_PASS_NON_AUTHORITATIVE",
            ...
            "code": {
                ...
                "schema_recovery_authority_validator": recovery_validator_record,
            },
            "legacy_signed_code": {
                "runner_gate_replay": authorized_replayer_record,
                "continuation_verifier": authorized_verifier_record,
            ...
            "failed_v28_key_archive_rebase_inputs": archive_rebase_inputs,
            "runner_archive_rebase_contract": archive_rebase_contract,
            "runtime": {
            ...
            "promotion_verifier": {
                "legacy_signed_command": authorized_verifier_command(),
                "recovery_authorized_command": verifier_command(),
            ...
            "promotion_trust_chain": {
                ...
                "historical_source_receipt": source_full_record,
                "canonical_source_receipt": canonical_full_record,
            ...
            "completion_runner": {
                ...
                "executed_stdin_size_bytes": len(archive_rebased_runner_prefix),
                "executed_stdin_sha256": archive_rebase_contract[
                    "archive_rebased_prefix"
                ]["sha256"],
            ...
            "runner_only_replay": {
                ...
                "stdin": {
                    ...
                    "original_size_bytes": len(runner_prefix),
                    "original_sha256": runner_prefix_sha256,
                    "archive_rebased_size_bytes": len(
                        archive_rebased_runner_prefix
                    ),
                    "archive_rebased_sha256": archive_rebase_contract[
                        "archive_rebased_prefix"
                    ]["sha256"],
                    "transform_contract": archive_rebase_contract["contract"],
                },
            ...
            "failed_v28_live_key_path_checks": {
                "before_verifier": failed_v28_live_keys_before,
                "after_verifier": failed_v28_live_keys_after_verifier,
                "after_runner_replay": failed_v28_live_keys_after_replay,
                "all_absent": True,
            },
            "log": log_record,
            "claims": {
                ...
                "archive_rebased_runner_lines_1_358_executed_successfully": True,
                "runner_prefix_archive_rebased_three_paths": True,
                "failed_v28_private_key_bytes_read": False,
                "failed_v28_live_key_paths_created": False,
        ...
        absence_snapshot(
            FAILED_V28_LIVE_KEY_PATHS,
            "pre-replay-receipt failed-v28 live key paths",
        )
        reader.require_paths_still_bound()
        ...
        def receipt_precommit() -> None:
            ...
            absence_snapshot(
                FAILED_V28_LIVE_KEY_PATHS,
                "terminal receipt failed-v28 live key paths",
            )
            reader.require_paths_still_bound()

        def receipt_terminal_commit() -> None:
            reader.require_paths_still_bound()
        ...
        write_exclusive_readonly(
            ...
            terminal_commit=receipt_terminal_commit,
        )
    except Exception:
        reader.close()
        raise
...
def require_worker_normal_exit(
    child_pid: int, waited_pid: int, wait_status: int
) -> dict[str, Any]:
    if (
        type(child_pid) is not int
        or child_pid <= 0
        or type(waited_pid) is not int
        or waited_pid != child_pid
        or type(wait_status) is not int
        or not os.WIFEXITED(wait_status)
        or os.WEXITSTATUS(wait_status) != 0
    ):
        raise GateError(
            "Runner-gate worker did not produce waitpid normal exit zero: "
            f"pid={child_pid} waited_pid={waited_pid} status={wait_status}"
        )
    return {
        "pid": child_pid,
...
def wait_for_worker(child_pid: int) -> tuple[int, int]:
    while True:
        try:
...
def supervise_worker() -> int:
    require_clean_runtime()
    require_absent(OUTPUT_LOG, "runner gate log before supervisor fork")
    require_absent(OUTPUT_RECEIPT, "runner gate receipt before supervisor fork")
    require_absent(
        OUTPUT_SUCCESS_WITNESS,
        "runner gate success witness before supervisor fork",
    )

    reader = BoundArtifactReader()
    try:
        _, self_data, self_record = reader.open(Path(__file__).resolve(strict=True))
        require_executing_replayer_matches_bound_source(self_data)
        if self_record.get("mode") != "0o444":
            raise GateError("Runner-gate supervisor source is not immutable mode 0444")
        child_pid = os.fork()
        if child_pid == 0:
            reader.close()
            try:
                ...
                message = (
                    f"runner-gate worker failure: {type(error).__name__}: {error}\n"
        ...
        waited_pid, wait_status = wait_for_worker(child_pid)
        worker_wait = require_worker_normal_exit(child_pid, waited_pid, wait_status)
        _, log_data, log_record = reader.open(OUTPUT_LOG)
        _, receipt_data, receipt_record = reader.open(OUTPUT_RECEIPT)
        log_stat = reader.stat_for(OUTPUT_LOG)
        receipt_stat = reader.stat_for(OUTPUT_RECEIPT)
        receipt_payload = parse_json(receipt_data, "runner-gate worker receipt")
        receipt_code = receipt_payload.get("code")
        if (
            log_stat.st_nlink != 1
            or receipt_stat.st_nlink != 1
            or log_record.get("mode") != "0o444"
            or receipt_record.get("mode") != "0o444"
            or not isinstance(receipt_code, Mapping)
            or not strict_json_equal(
                receipt_code.get("runner_gate_replay"), self_record
            )
            or not strict_json_equal(receipt_payload.get("log"), log_record)
            or not strict_json_equal(
                receipt_payload.get("canonical_command"), expected_command()
            )
            or receipt_payload.get("pass") is not True
            or receipt_payload.get("downstream_not_executed") is not True
            or receipt_payload.get("downstream_authorized_after_this_gate") is not False
            or not log_data
        ):
            raise GateError("Runner-gate worker output/source binding drift")
        reader.require_paths_still_bound()

        witness = {
            "schema_name": OUTPUT_SUCCESS_WITNESS_SCHEMA,
            "schema_version": "1.0.0",
            ...
            "canonical_command": expected_command(),
            "clean_environment": EXPECTED_ENVIRONMENT,
            "supervisor_source": self_record,
            "worker_wait": worker_wait,
            "worker_outputs": {
                "log": log_record,
                "receipt": receipt_record,
            },
            "checks": OUTPUT_SUCCESS_WITNESS_CHECKS,
            "pass": True,
            "pass_semantics": (
        ...
        if set(witness) != set(OUTPUT_SUCCESS_WITNESS_KEYS):
            raise GateError("Runner-gate success witness schema drift")
        witness_data = encode_json(witness)
        reader.require_paths_still_bound()
        reader.retain_until_process_exit()

        def witness_precommit() -> None:
            reader.require_paths_still_bound()
            require_absent(
                OUTPUT_SUCCESS_WITNESS,
                "runner gate success witness during supervisor commit",
            )

        write_exclusive_readonly(
            OUTPUT_SUCCESS_WITNESS,
            witness_data,
            precommit=witness_precommit,
            terminal_commit=witness_precommit,
        )
    except BaseException:
        reader.close()
        raise
