#!/usr/bin/env python3
"""Build the all-sSNV Traditional Chinese terminal report and canonical artifact."""
...
def validate_detached_final_release(inputs: ReportInputs) -> dict[str, Any]:
    ...
    script = Path(__file__).resolve().parent / "finalize_task_b_result_release.py"
    command = [
...
def validate_metrics(
    ...
    if not isinstance(source_per_sample, Mapping) or tuple(source_per_sample) != EXPECTED_DATASETS:
    ...
    if not isinstance(source_truth, Mapping) or tuple(source_truth) != TRUTH_LABELS:
        raise ReportContractError("Final truth_strata order/content drift")
    ...
    if not isinstance(source_sample_truth, Mapping) or tuple(source_sample_truth) != expected_keys:
        raise ReportContractError("Final sample_by_truth order/content drift")
