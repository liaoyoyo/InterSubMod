...
MINIMUM_CANONICAL_TEST_COUNT = 489
REQUIRED_SUPPLEMENTAL_TESTS = frozenset(
    {
        ...
        "test_reader_facing_report_uses_official_claim_ladder_and_determinate_language",
    }
)
COLORS = {
    "ink": "#17202a",
...
SOURCE_AUTHORITY_ID = "20260722_all_ssnv_focal_alt_task_b_release_v7_strict_command_parity"
CANONICAL_PYTEST_XML = (
    TOPIC_ROOT
    / "logs"
    / "pytest_full_supplemental_v30_print_qa_alignment_canonical.xml"
)
TEST_EVIDENCE_MANIFEST = (
    TOPIC_ROOT / "logs" / "supplemental_report_test_evidence.v6.json"
)
...
TEST_EVIDENCE_PUBLIC_KEY = Path(
    ...
    "20260722_positional_singleton_report_v6_print_qa_alignment/"
    "ed25519_public.pem"
)
...
TEST_EVIDENCE_PUBLIC_KEY_SHA256 = (
    "4d1e472229202065fe253ac92276a218b134fefa378345c6d90793ea17cfadc1"
)
TEST_EVIDENCE_AUTHORITY_ID = (
    "20260722_positional_singleton_supplemental_test_evidence_v6_"
    "print_qa_alignment"
)
    ...
    try:
        with authority.BoundArtifactReader() as reader:
            _, private_key_artifact = reader.open_metadata(
                TEST_EVIDENCE_PRIVATE_KEY, include_mode=True
            )
            ...
            openssl_fd, _, openssl_artifact = reader.open(
                authority.OPENSSL_PATH, include_mode=True
            )
            if openssl_artifact["sha256"] != authority.OPENSSL_SHA256:
                raise ReportBuildError(
                    "Supplemental test-evidence OpenSSL verifier drift"
                )
            ...
            expected_signature_contract = {
                ...
                "private_key_path": str(private_key_artifact["path"]),
            }
            if (
                ...
                != expected_signature_contract
                or manifest.get("signature_state_at_manifest_creation")
                != {
    ...
    *,
    ...
    try:
        with authority.BoundArtifactReader() as reader:
            ...
            if openssl_artifact["sha256"] != authority.OPENSSL_SHA256:
                raise ReportBuildError(f"{label} OpenSSL verifier drift")
            if any(
                record["mode"] != "0o444"
                for record in (
                    receipt_artifact,
                ...
                raise ReportBuildError(f"{label} protected mode drift")
            ...
            if not authority.verify_ed25519_signature_fds(
                ...
                raise ReportBuildError(f"{label} detached signature failed")
            reader.retain_until_process_exit()
    except authority.SourceAuthorityError as error:
        raise ReportBuildError(
...
def resolve_source_authority_input_paths(
    ...
    """Resolve every transitive artifact in the signed main Task-B audit chain."""

    source_chain = summary.get("source_chain")
    if not isinstance(source_chain, dict):
        raise ReportBuildError("Source chain is missing")
    authority_chain = source_chain.get("main_task_b_source_authority")
    if (
    ...
    records = {
        "main_authority_manifest": authority_chain.get("authority_manifest"),
        "main_authority_approval": authority_chain.get("detached_approval"),
        "main_authority_signature": authority_chain.get(
            "detached_approval_signature"
        ),
        "main_authority_public_key": authority_chain.get("public_key"),
    }
    paths = {}
...
def validate_source_authority_chain(summary: Mapping[str, Any]) -> None:
    ...
    verify_signature(
        receipt_path=paths["main_authority_approval"],
        signature_path=paths["main_authority_signature"],
        public_key_path=paths["main_authority_public_key"],
        expected_public_key_sha256=SOURCE_AUTHORITY_PUBLIC_KEY_SHA256,
        label="source authority",
    )
    authority = load_json(
        paths["main_authority_manifest"], label="source authority manifest"
    )
    approval = load_json(
        paths["main_authority_approval"], label="source authority approval"
    )
    expected_contract = (
...
def validate_inputs(
    *,
    ...
    if (
        ...
        or summary.get("schema_version") != AUDIT_SCHEMA_VERSION
...
def repository_metadata(summary: Mapping[str, Any]) -> dict[str, str]:
    ...
    authorized_head = str(
        summary["source_chain"]["main_task_b_source_authority"][
            "authorized_git_head"
        ]
    )
...
def figure_funnel(summary: Mapping[str, Any], output: Path) -> None:
    ...
    figure.text(
        0.01,
        0.01,
        "M1 is an operational screen; M2 is applied to every M1 flag, but only "
        "determinate rows enter PASS/FAIL. Neither confirms a cellular clone.",
        color=COLORS["muted"],
        fontsize=8.5,
    )
...
def figure_truth(summary: Mapping[str, Any], output: Path) -> None:
    ...
    figure.text(
        0.01,
        0.01,
        "Only one FP singleton is M2-determinate; these "
...
正式證據階梯: M1 operational screen；M2 measured-axis residual partition；G1/G2/R1 NOT_RUN；L1/L2未達
...
> **Task Type B 全量驗證；claim ceiling = M2 read-level residual epigenetic
> partition；G1/G2/R1 NOT_RUN，正式 L1/L2 均未達。**
...
4. M2 套用於全部 {counts['m1_flagged']:,} 個 M1 flags，其中只有
   **{m2_evaluable:,} = {counts['m2_pass']:,} PASS + {counts['m2_fail']:,}
   FAIL** 可完成八個 measured axes 的 determinate M2 判定；另有
   **{counts['m2_not_evaluable']:,} NOT_EVALUABLE**。因此 M2 PASS 是
   **{counts['m2_pass']:,} / {counts['singleton_sites']:,} =
   {pct(counts['m2_pass'], counts['singleton_sites'], 6)}**。這是 observed operational
   M2-PASS yield，不是 biological/subclone prevalence，也不是其
   lower bound。
   `{counts['m2_pass']}/{m2_evaluable}={pct(counts['m2_pass'], m2_evaluable, 1)}`
   的 Wilson 95% interval 為
   [{100 * m2_wilson[0]:.2f}%, {100 * m2_wilson[1]:.2f}%]，但只描述
   M2-determinate conditional subset，不可外推。
...
REF control。M2 + evaluable REF non-replication 的 observed joint operational yield 是
**{m2_control['ref_nonreplication']} / {counts['singleton_sites']:,} =
{pct(m2_control['ref_nonreplication'], counts['singleton_sites'], 6)}**；這是優先追蹤
候選率，不是 biological/subclone prevalence，也不是其下限。
...
- M2 套用於全部 {counts['m1_flagged']:,} 個 M1 flags；
  `{counts['m2_pass']}/{m2_evaluable}` 只是在 M2-determinate subset 內的條件比例。
...
- Tumor-REF non-replication 只增加 ALT specificity 支持；REF 不可測不能當
  non-replication。
- candidate-level locus matched-normal methylation 尚未執行；目前只有
  BAM/read-level extractability audit，不能視為 B1 背景控制。
- exact-locus CN、purity 與 CCF 尚未註記且目前不可用，因此 C1、L1、L2
  均未達。
    ...
    claim_table = html_table(
        ...
        [
            ...
            (
                ...
                "NOT_RUN / 未達",
                (
                    "candidate-level locus matched-normal 尚未執行，且 "
                    "exact-locus CN/purity/CCF 尚未註記"
                ),
            ),
            (
                "L1/L2",
                "未達",
                "沒有 cellular subclone 或 lineage/order 正交證據",
            ),
            (
                "細胞級正交確認",
                "無資料",
...
.tier-m1 {{ color: var(--teal); }}
.tier-m2 {{ color: var(--blue); }}
.tier-not-run {{ color: #865900; }}
...
@media print {{
  ...
  h2, h3, figure, .kpis, .assertion {{ break-inside: avoid; }}
  h2, h3 {{ break-after: avoid; }}
  .table-wrap {{ overflow: visible; border: 0; }}
  table {{ width: 100%; min-width: 0; table-layout: fixed; font-size: 10px; }}
...
<section id="answer"><div class="inner">
  ...
  <p><span class="tier tier-m1">M1 operational screen</span><span class="tier tier-m2">M2 residual partition</span><span class="tier tier-not-run">G1/G2/R1 NOT_RUN</span>；正式 L1/L2 均未達。</p>
  <div class="kpis">
  ...
  [{100 * m2_wilson[0]:.2f}%, {100 * m2_wilson[1]:.2f}%]；這只適用
  M2-determinate conditional subset。M2 套用於全部 {counts['m1_flagged']:,}
  個 M1 flags；只有 {m2_evaluable} 個為 determinate，其中
  {counts['m2_pass']} / {counts['singleton_sites']:,} =
  {pct(counts['m2_pass'], counts['singleton_sites'], 6)} 是 observed operational
  M2-PASS yield，不是 biological/subclone prevalence，也不是其 lower bound。
  其餘 {counts['m2_not_evaluable']:,} 個因 measured-axis
  ...
  <figure><img src="{image['funnel']}" alt="全體到 M2 的分母漏斗"><figcaption>橫軸為 dataset-site 數量（log scale）。M2 套用於全部 {counts['m1_flagged']:,} 個 M1 flags；只有 {m2_evaluable} 個具備八軸 determinate 結果。</figcaption></figure>
</div></section>
...
<section id="provenance"><div class="inner">
  ...
  <ul>
    ...
    <li>M1 是資料相依 screen yield；{counts['m2_pass']}/{m2_evaluable} 只是在 M2-determinate subset 內的條件比例。</li>
    ...
    <li>candidate-level locus matched-normal methylation 尚未執行；目前只有 BAM/read-level extractability audit，不能視為 B1 背景控制。</li>
    <li>exact-locus CN、purity 與 CCF 尚未註記且目前不可用，因此 C1、L1、L2 均未達。</li>
