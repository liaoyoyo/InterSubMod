...
import audit_stable_primary_artifacts as PRIMARY_AUDITOR
...
def embedded_artifact_matches(record: Any, path: Path) -> bool:
...
def validate_upstream_audit_input_binding(
    payload: Mapping[str, Any],
    *,
    schema_name: str,
    schema_version: str,
    expected_input_roles: set[str],
    assignments: Path,
    sites: Path,
) -> None:
    if (
        ...
        raise RuntimeError(f"{schema_name} schema/version/pass gate failed")
    inputs = payload.get("inputs")
    if not isinstance(inputs, Mapping) or set(inputs) != expected_input_roles:
        raise RuntimeError(f"{schema_name} input role set drifted")
    if not embedded_artifact_matches(inputs.get("stable_assignments"), assignments):
        raise RuntimeError(f"{schema_name} stable assignments identity drifted")
    if not embedded_artifact_matches(inputs.get("site_results"), sites):
        raise RuntimeError(f"{schema_name} site results identity drifted")


def validate_independent_m2_authority(payload: Mapping[str, Any]) -> None:
    inputs = payload.get("inputs")
    code = payload.get("code")
    if not isinstance(inputs, Mapping) or not isinstance(code, Mapping):
        raise RuntimeError("Independent M2 audit lacks inputs or code")
    expected_claim = SOURCE_AUTHORITY.artifact(
        SOURCE_AUTHORITY.CLAIM_CONTRACT_PATH
    )
    if inputs.get("claim_contract") != expected_claim:
        raise RuntimeError("Independent M2 audit claim-contract-v5 identity drifted")
    expected_code = artifact(
        SOURCE_AUTHORITY.EXPECTED_SOURCE_PATHS["independent_m2_auditor"]
    )
    if code != {"independent_recount": expected_code}:
        raise RuntimeError("Independent M2 auditor source identity drifted")


def validate_primary_audit_execution_authority(
    payload: Mapping[str, Any],
    *,
    site_results: Path,
    assignments: Path,
    ...
    if payload.get("source_authority") != source_authority:
        raise RuntimeError("Primary pre-audit source authority drifted")
    if payload.get("code") != expected_code:
        raise RuntimeError("Primary pre-audit source identity drifted")
    ...
    if (
        ...
        raise RuntimeError("Primary pre-audit source lock drifted")
    if payload.get("command") != expected_command:
        raise RuntimeError("Primary pre-audit command is not canonical")
    if payload.get("execution") != PRIMARY_AUDITOR.CANONICAL_PARAMETERS:
        raise RuntimeError("Primary pre-audit parameters are not canonical")
    if oct(output.stat().st_mode & 0o777) != "0o444":
        raise RuntimeError("Primary pre-audit receipt is not mode 0444")
...
def audit_raw_identity_task(task: dict[str, Any]) -> dict[str, Any]:
    ...
    screen_row = task["screen_row"]
    entry = task["entry"]
    vcf = ANALYZER.get_vcf(
        entry["all_ssnv_vcf"]["path"],
        entry["all_ssnv_vcf_index"]["path"],
    )
    focal, partners = ANALYZER.discover_focal_and_partners(
        vcf,
        str(assignment["chrom"]),
        int(assignment["pos"]),
    )
    if (
        focal.ref.upper() != str(screen_row["ref"]).upper()
        or focal.alt.upper() != str(screen_row["alt"]).upper()
    ):
        raise RuntimeError(
            f"Preflight focal allele drift at {assignment['sample']}:{focal.chrom}:{focal.pos}"
        )
    ...
    ANALYZER.validate_analysis_payload_audit(
        raw_audit,
    ...
    result = {
        ...
        "raw_identity_alignment_payload_sha256": str(
            raw_audit["alignment_payload_sha256"]
        ),
        "raw_identity_recovered_payload_sha256": str(
            raw_audit["recovered_payload_sha256"]
        ),
        "raw_identity_analysis_site_payload_sha256": str(
            raw_audit["analysis_site_payload_sha256"]
        ),
...
def runtime_statistical_api_probe() -> dict[str, Any]:
    return ANALYZER.runtime_statistical_api_probe()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    expected_command = ANALYZER.canonical_preflight_command(
    ...
    if parameters != ANALYZER.PREFLIGHT_CANONICAL_PARAMETERS:
        raise ValueError("Preflight parameters are not canonical for Task-B")
    ...
    actual_command = ANALYZER.resolve_release_command(
        argv=argv,
    ...
    runtime_identity_before = ANALYZER.capture_runtime_environment_identity()
    if set(source_modes_before.values()) != {"0o444"}:
        raise RuntimeError(f"Preflight sources are not mode 0444: {source_modes_before}")
    input_identity_before = ANALYZER.capture_primary_input_identity(
        manifest=args.manifest,
        assignments=args.assignments,
        sites=args.sites,
        primary_artifact_audit_pre=args.primary_artifact_audit_pre,
    ...
    validate_upstream_audit_input_binding(
        independent,
        schema_name="intersubmod.independent_m2_gate_recount",
        schema_version="2.0.0",
        expected_input_roles={
            "claim_contract",
            "production_gate_source_reference_only",
            "site_results",
            "stable_assignments",
        },
        assignments=args.assignments,
    ...
    validate_independent_m2_authority(independent)
    validate_upstream_audit_input_binding(
        primary_audit,
        schema_name="intersubmod.stable_primary_artifact_audit",
        schema_version="2.0.0",
        expected_input_roles={"site_results", "stable_assignments", "consumer_receipts"},
        assignments=args.assignments,
        sites=args.sites,
    )
    if primary_audit["inputs"]["consumer_receipts"] != []:
        raise RuntimeError("Primary pre-downstream artifact audit contains consumer receipts")
    validate_primary_audit_execution_authority(
        primary_audit,
        site_results=args.sites,
        assignments=args.assignments,
        output=args.primary_artifact_audit_pre,
        source_authority=source_authority,
    )
    ...
    runtime_identity_after = ANALYZER.capture_runtime_environment_identity()
    input_identity_after = ANALYZER.capture_primary_input_identity(
        manifest=args.manifest,
        assignments=args.assignments,
        sites=args.sites,
        primary_artifact_audit_pre=args.primary_artifact_audit_pre,
    ...
    checks = {
        ...
        "independent_audit_current_input_binding": True,
        "primary_audit_current_input_binding": True,
        "release_source_authority_verified": True,
        "runtime_python_isolated_mode": (
            runtime_identity_before["interpreter_flags"]["isolated"] == 1
            and runtime_identity_before["interpreter_flags"]["safe_path"] is True
        ),
        "runtime_python_user_site_disabled": (
            runtime_identity_before["interpreter_flags"]["no_user_site"] == 1
            and runtime_identity_before["environment"]["PYTHONNOUSERSITE"] == "1"
        ),
        "runtime_pythonpath_unset": (
            runtime_identity_before["environment"]["PYTHONPATH"] is None
        ),
    ...
    if set(checks) != ANALYZER.PREFLIGHT_CANONICAL_CHECK_KEYS:
        raise RuntimeError("Preflight implementation check-key contract drifted")
    ...
    payload = {
        ...
        "input_lock": {
            ...
            "identity_after": input_identity_after,
            "all_primary_inputs_unchanged": input_identity_before == input_identity_after,
        },
        "runtime_environment_lock": {
            "identity_before": runtime_identity_before,
            "identity_after": runtime_identity_after,
            "direct_runtime_unchanged": runtime_identity_before == runtime_identity_after,
        },
        "code": {
        ...
        "pass_semantics": ANALYZER.PREFLIGHT_PASS_SEMANTICS,
    }
