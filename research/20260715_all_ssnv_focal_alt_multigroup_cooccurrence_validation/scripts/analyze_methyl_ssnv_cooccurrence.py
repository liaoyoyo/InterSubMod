...
PREFLIGHT_SCHEMA_NAME = "intersubmod.cooccurrence_task_contract_preflight"
PREFLIGHT_SCHEMA_VERSION = "2.0.0"
EXPECTED_STABLE_SITE_TASKS = 102_842
CANONICAL_PYTHON_CACHE_DIRNAME = (
    ".python_cache_cooccurrence_v8_command_parity_bound_bootstrap"
)
PREFLIGHT_PASS_SEMANTICS = (
    "execution_and_task_contract_integrity_only_not_scientific_result"
)
PREFLIGHT_CANONICAL_PARAMETERS = {
    "workers": 40,
    "chunk_size": 8,
    "max_pending_chunks": 80,
    "progress_every": 1_000,
}
PRODUCER_CANONICAL_PARAMETERS = {
    "workers": 40,
    "chunk_size": 8,
    "max_pending_chunks": 80,
    "permutations": 999,
...
PREFLIGHT_CANONICAL_CHECK_KEYS = frozenset(
    {
        ...
        "single_group_limit_example",
        "group_limit_is_hcc1954_chr5_751076",
        "independent_counts_match",
        "independent_gate_constant_counts_match",
        "independent_audit_current_input_binding",
        "primary_audit_current_input_binding",
        "release_source_authority_verified",
        "runtime_python_isolated_mode",
        "runtime_python_user_site_disabled",
        "runtime_pythonpath_unset",
        "runtime_pearson_3x2_testable",
        "runtime_pearson_3x2_uses_chi_square",
        "runtime_pearson_p_matches_tuple_index",
        "runtime_library_marker_pearson_3x2_testable",
        "runtime_library_marker_pearson_uses_chi_square",
        "runtime_library_marker_pearson_p_matches_tuple_index",
        "runtime_fisher_2x2_testable",
        "runtime_fisher_2x2_uses_fisher_exact",
        "raw_identity_task_count",
        "raw_identity_all_samples",
        "raw_identity_nonempty_projection_population",
        ...
        "source_files_read_only_at_start",
        "source_files_read_only_at_finish",
        "input_identity_unchanged_during_preflight",
        "runtime_environment_unchanged_during_preflight",
    }
)
...
SITE_COLUMNS = [
    ...
    "raw_identity_alignment_payload_sha256",
    "raw_identity_recovered_payload_sha256",
...
def capture_primary_input_identity(
    *,
    manifest: Path,
    assignments: Path,
    sites: Path,
    primary_artifact_audit_pre: Path,
    independent_m2_audit: Path,
    ...
    paths = {
        ...
        "sites": sites,
        "primary_artifact_audit_pre": primary_artifact_audit_pre,
        "independent_m2_audit": independent_m2_audit,
    }
    if preflight_receipt is not None:
        paths["preflight_receipt"] = preflight_receipt
...
def capture_runtime_environment_identity() -> dict[str, Any]:
    implementation_modules = {
        ...
        "pysam_alignment_file": importlib.import_module("pysam.libcalignmentfile"),
        "pysam_variant_file": importlib.import_module("pysam.libcbcf"),
        "pysam_tabix_file": importlib.import_module("pysam.libctabix"),
        "pysam_htslib": importlib.import_module("pysam.libchtslib"),
        "scipy_special_ufuncs": importlib.import_module("scipy.special._ufuncs"),
        "scipy_stats_core": importlib.import_module("scipy.stats._stats"),
    }
    implementation_paths = {
        "scipy_chi2_contingency_callable": Path(inspect.getfile(chi2_contingency)).resolve(),
        "scipy_fisher_exact_callable": Path(inspect.getfile(fisher_exact)).resolve(),
        **{
    ...
    return {
        "contract": "python_executable_callable_sources_and_direct_extensions_v2",
        ...
        "packages": {
            ...
            "pysam": {
                ...
                "module": artifact(Path(pysam.__file__).resolve()),
            },
            "scipy": {
        ...
        "implementation_artifacts": {
            name: artifact(path) for name, path in sorted(implementation_paths.items())
        },
        "guardrail": (
            "The executable, imported package roots, statistical callable sources, and direct "
            "NumPy/pysam/SciPy extension modules used by this workflow are hash locked. This is "
            "not a bytewise snapshot of every transitive operating-system shared library."
        ),
    }
...
def runtime_statistical_api_probe() -> dict[str, Any]:
    ...
    pearson = categorical_association(labels_3x2, categories_3x2)
    raw_pearson = chi2_contingency(pearson["table"], correction=False)
    marker_pearson = LIB.methyl_group_marker_association(labels_3x2, categories_3x2)
    raw_marker_pearson = LIB.chi2_contingency(
        marker_pearson["table"], correction=False
    )
    labels_2x2 = ["g1"] * 10 + ["g2"] * 10
    categories_2x2 = ["R"] * 7 + ["A"] * 3 + ["R"] * 3 + ["A"] * 7
    fisher = categorical_association(labels_2x2, categories_2x2)
    return {
...
def canonical_preflight_command(
    *,
    manifest: Path,
    assignments: Path,
    sites: Path,
    ...
    parameters = PREFLIGHT_CANONICAL_PARAMETERS
    return [
        sys.executable,
        "-I",
        str((SCRIPT_DIR / "audit_cooccurrence_task_contract_preflight.py").resolve()),
        "--manifest",
        ...
        str(sites.resolve()),
        "--intersubmod-root",
        str(intersubmod_root.resolve()),
        "--independent-m2-audit",
        str(independent_m2_audit.resolve()),
        "--primary-artifact-audit-pre",
        str(primary_artifact_audit_pre.resolve()),
        "--output",
        str(output.resolve()),
        "--workers",
        ...
        "--max-pending-chunks",
        str(parameters["max_pending_chunks"]),
        "--progress-every",
        str(parameters["progress_every"]),
    ]
...
def capture_python_cache_contract(output_dir: Path) -> dict[str, Any]:
    expected = canonical_python_cache_root(output_dir)
    configured = sys.pycache_prefix
    xoption = sys._xoptions.get("pycache_prefix")
    if configured != str(expected) or xoption != str(expected):
        raise RuntimeError(
            "Formal Python cache prefix differs from the canonical Task-B path"
        )
    opened = expected.lstat()
    if (
        ...
        raise RuntimeError(
            "Formal Python cache root is not a canonical mode-0700 directory"
        )
    return {
        "path": str(expected),
        "mode": "0o700",
        "sys_pycache_prefix": configured,
        "xoption_pycache_prefix": xoption,
        "contract": "canonical_nonsymlink_mode0700_python_xoption_v1",
    }


def canonical_producer_command(
    *,
    manifest: Path,
    assignments: Path,
    sites: Path,
    ...
    parameters = PRODUCER_CANONICAL_PARAMETERS
    return [
        *canonical_python_prefix(output_dir),
        ...
        str(parameters["max_pending_chunks"]),
        "--permutations",
        str(parameters["permutations"]),
        "--top-markers",
        str(parameters["top_markers"]),
        "--exact-state-space-ceiling",
        str(parameters["exact_state_space_ceiling"]),
    ]
...
def observed_process_command() -> list[str]:
    try:
        ...
        raise RuntimeError("Cannot read formal process command from /proc/self/cmdline") from error
    if not raw or not raw.endswith(b"\0"):
        raise RuntimeError("Formal process command is empty or malformed")
    fields = raw[:-1].split(b"\0")
    if not fields or any(not field for field in fields):
        raise RuntimeError("Formal process command contains an empty argument")
...
def resolve_release_command(
    *,
    ...
    if source_authority.get("authority_id") == SOURCE_AUTHORITY.AUTHORITY_ID:
        if argv is not None:
            raise ValueError(f"Formal {role} execution is direct-CLI only")
    ...
    if actual_command != list(expected_command):
        raise ValueError(
            f"{role.capitalize()} process command must match the canonical Task-B command exactly"
        )
...
def _require_declared_artifact(
    record: Any,
    path: Path,
    label: str,
) -> None:
    if not isinstance(record, Mapping) or dict(record) != artifact(path):
        raise RuntimeError(f"Preflight {label} artifact identity does not match live input")
...
def validate_preflight_gate(
    ...
    *,
    ...
    independent_m2_audit: Path,
    primary_artifact_audit_pre: Path,
    current_code_identity: Mapping[str, Mapping[str, Any]],
    current_runtime_identity: Mapping[str, Any],
    current_runtime_probe: Mapping[str, Any],
    producer_started_at_utc: str,
) -> dict[str, Any]:
    ...
    if (
        payload.get("schema_name") != PREFLIGHT_SCHEMA_NAME
        ...
        or payload.get("raw_identity_release_contract")
        != RAW_IDENTITY_RELEASE_CONTRACT
        or payload.get("pass_semantics") != PREFLIGHT_PASS_SEMANTICS
    ):
        raise RuntimeError("Cooccurrence preflight gate schema/scope/contract did not pass")
    checks = payload.get("checks")
    if (
        not isinstance(checks, Mapping)
        or set(checks) != PREFLIGHT_CANONICAL_CHECK_KEYS
        or not all(value is True for value in checks.values())
    ):
        raise RuntimeError("Cooccurrence preflight contains a failed or malformed check")
    if payload.get("source_authority") != current_source_authority:
        raise RuntimeError("Cooccurrence preflight source authority differs from live authority")
    if payload.get("parameters") != PREFLIGHT_CANONICAL_PARAMETERS:
        raise RuntimeError("Cooccurrence preflight parameters are not canonical")
    expected_command = canonical_preflight_command(
        manifest=manifest,
        assignments=assignments,
        sites=sites,
    ...
    if payload.get("command") != expected_command:
        raise RuntimeError("Cooccurrence preflight command is not canonical")
    try:
        started = datetime.fromisoformat(str(payload["started_at_utc"]))
        finished = datetime.fromisoformat(str(payload["finished_at_utc"]))
        created = datetime.fromisoformat(str(payload["created_at_utc"]))
        producer_started = datetime.fromisoformat(producer_started_at_utc)
    except (KeyError, TypeError, ValueError) as error:
        raise RuntimeError("Cooccurrence preflight timestamps are malformed") from error
    if not started <= finished == created <= producer_started:
        raise RuntimeError("Cooccurrence preflight chronology is invalid")
    if oct(path.stat().st_mode & 0o777) != "0o444":
        raise RuntimeError("Cooccurrence preflight receipt is not mode 0444")
    observed = payload.get("observed")
    raw_identity = observed.get("raw_bam_identity_recovery") if isinstance(observed, Mapping) else None
    if (
        not isinstance(raw_identity, Mapping)
        or int(observed.get("task_count", -1)) != EXPECTED_STABLE_SITE_TASKS
        or int((raw_identity.get("aggregate") or {}).get("site_tasks", -1))
        != EXPECTED_STABLE_SITE_TASKS
        or raw_identity.get("all_result_rows_passed_invariant_validation") is not True
        or raw_identity.get("missing_projection_policy") != RAW_IDENTITY_MISSING_POLICY
        or raw_identity.get("conflicting_analysis_payload_policy")
        != RAW_IDENTITY_CONFLICT_POLICY
        or raw_identity.get("failure_counts_materialized") is not False
        or raw_identity.get("analysis_scope_identity_contract")
        != ANALYSIS_SCOPE_IDENTITY_CONTRACT
    ):
        raise RuntimeError("Cooccurrence preflight raw-identity gate is incomplete")

    inputs = payload.get("inputs")
    if not isinstance(inputs, Mapping):
        raise RuntimeError("Cooccurrence preflight lacks input identities")
    for name, input_path in (
        ...
        _require_declared_artifact(inputs.get(name), input_path, name)
    input_lock = payload.get("input_lock")
    if (
        not isinstance(input_lock, Mapping)
        or input_lock.get("identity_before") != inputs
        or input_lock.get("identity_after") != inputs
        or input_lock.get("all_primary_inputs_unchanged") is not True
    ):
        raise RuntimeError("Cooccurrence preflight input identity lock drifted")
    runtime_lock = payload.get("runtime_environment_lock")
    if (
        not isinstance(runtime_lock, Mapping)
        or runtime_lock.get("identity_before") != current_runtime_identity
        or runtime_lock.get("identity_after") != current_runtime_identity
        or runtime_lock.get("direct_runtime_unchanged") is not True
    ):
        raise RuntimeError("Cooccurrence preflight runtime environment lock drifted")
    if observed.get("runtime_statistical_api_probe") != current_runtime_probe:
        raise RuntimeError("Cooccurrence preflight statistical API probe drifted")

    code = payload.get("code")
    if not isinstance(code, Mapping):
        raise RuntimeError("Cooccurrence preflight lacks source identities")
    before = code.get("source_identity_before")
    after = code.get("source_identity_after")
    modes_before = code.get("source_modes_before")
    modes_after = code.get("source_modes_after")
    expected_preflight_roles = {"preflight", *current_code_identity.keys()}
    if (
        not isinstance(before, Mapping)
        or not isinstance(after, Mapping)
        or set(before) != expected_preflight_roles
        or dict(before) != dict(after)
    ):
        raise RuntimeError("Cooccurrence preflight source identity map drifted")
    if (
        not isinstance(modes_before, Mapping)
        or not isinstance(modes_after, Mapping)
        or set(modes_before) != expected_preflight_roles
        or dict(modes_before) != dict(modes_after)
        or set(modes_before.values()) != {"0o444"}
    ):
        raise RuntimeError("Cooccurrence preflight source mode map drifted")
    for role, identity in current_code_identity.items():
        if before.get(role) != identity:
            raise RuntimeError(f"Cooccurrence preflight {role} source differs from analyzer run")
    _require_declared_artifact(
...
def _canonical_hash_value(value: Any) -> Any:
    """Convert typed SAM values to an unambiguous JSON-hash representation."""
    ...
    if value is None or isinstance(value, (bool, int, str)):
        return value
    raise TypeError(f"Unsupported analysis-payload hash value: {type(value).__name__}")


def canonical_payload_sha256(value: Any) -> str:
    payload = json_text(_canonical_hash_value(value)).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()


def raw_alignment_analysis_payload(
    alignment: pysam.AlignedSegment,
) -> dict[str, Any]:
    return {
        "sam_core": raw_alignment_core_payload(alignment),
        "typed_auxiliary_tags": raw_alignment_aux_payload(
            alignment,
            excluded_tags=RAW_DUPLICATE_ALLOWED_DIFFERING_TAGS,
        ),
    }


def alignment_payload_digest(
    alignments: Mapping[TAGS.ProjectionKey, pysam.AlignedSegment],
) -> str:
    payload = [
        {
            "projection": list(projection),
            "alignment": raw_alignment_analysis_payload(alignment),
        }
        for projection, alignment in sorted(alignments.items())
    ]
    return canonical_payload_sha256(payload)


def recovered_read_analysis_payload(read: RecoveredRead) -> dict[str, Any]:
    return {
        "read_id": read.read_id,
        "read_name": read.read_name,
        "expected_focal_call": read.expected_focal_call,
        "focal_call": read.focal_call,
        "latest_hp": read.latest_hp,
        "latest_ps": read.latest_ps,
        "strand": read.strand,
        "assignment_label": read.label,
        "partner_calls": [
            [int(position), call]
            for position, call in sorted(read.partner_calls.items())
        ],
        "readset_identity": read.readset_identity,
    }


def finalize_analysis_payload_audit(
    audit: dict[str, Any],
    focal: LIB.Variant,
    ...
    recovered_payload = [
        recovered_read_analysis_payload(read)
        for read in sorted(recovered, key=lambda value: value.read_id)
    ]
    recovered_digest = canonical_payload_sha256(recovered_payload)
    audit.update(
        {
            "analysis_scope_identity_contract": ANALYSIS_SCOPE_IDENTITY_CONTRACT,
            "recovered_payload_sha256": recovered_digest,
            "analysis_site_payload_sha256": canonical_payload_sha256(
                {
                    "contract": ANALYSIS_SCOPE_IDENTITY_CONTRACT,
                    "focal": [focal.chrom, focal.pos, focal.ref, focal.alt],
                    "partners": [
                        [partner.chrom, partner.pos, partner.ref, partner.alt]
                    ...
                    "alignment_payload_sha256": audit.get(
...
def validate_raw_identity_audit(
    ...
    *,
    ...
    _require_sha256(
        audit["alignment_payload_sha256"], "alignment_payload_sha256"
    )
...
def validate_analysis_payload_audit(
    audit: Mapping[str, Any],
    *,
    expected_recovered_count: int | None = None,
) -> None:
    ...
    if audit.get("analysis_scope_identity_contract") != ANALYSIS_SCOPE_IDENTITY_CONTRACT:
        raise RuntimeError("Analysis-scope identity contract drifted")
    _require_sha256(audit.get("recovered_payload_sha256"), "recovered_payload_sha256")
    _require_sha256(
        audit.get("analysis_site_payload_sha256"),
        "analysis_site_payload_sha256",
    )
...
def resolve_unique_eligible_alignments(
    ...
    resolved = {key: values[0] for key, values in matches.items()}
    resolved_audit = {
        ...
        "alignment_payload_sha256": alignment_payload_digest(resolved),
        "duplicate_projection_examples": [
...
def recover_site_reads(
    ...
    if raw_identity_audit is not None:
        finalize_analysis_payload_audit(
            raw_identity_audit,
            focal,
            partners,
            recovered,
...
def analyze_site_task(task: dict[str, Any]) -> dict[str, Any]:
    ...
    validate_analysis_payload_audit(
        raw_identity_audit,
    ...
    site_row: dict[str, Any] = {
        ...
        "raw_identity_alignment_payload_sha256": str(
            raw_identity_audit["alignment_payload_sha256"]
        ),
        "raw_identity_recovered_payload_sha256": str(
            raw_identity_audit["recovered_payload_sha256"]
        ),
        "raw_identity_analysis_site_payload_sha256": str(
            raw_identity_audit["analysis_site_payload_sha256"]
        ),
...
M2_PRE_AXIS_SUMMARY_STATUS = "NOT_EVALUATED_M2_GATE_PRE_AXIS"
M2_PRE_AXIS_GATE_STATUSES = frozenset(
    {
...
def summarize_m2_axis_status_counts(
    site_rows: Sequence[Mapping[str, Any]],
) -> dict[str, dict[str, int]]:
    """Build a total per-axis census without inventing unmeasured axis evidence."""
    axes = tuple(prefix for prefix, _kind, _effect_suffix, _threshold in M2_GATE.AXIS_SPECS)
    axis_set = set(axes)
    counts = {axis: Counter() for axis in axes}
    for row in site_rows:
        label = f"{row.get('sample', '?')}:{row.get('chrom', '?')}:{row.get('pos', '?')}"
        statuses = row.get("m2_axis_statuses")
        if not isinstance(statuses, Mapping):
            raise RuntimeError(f"M2 axis statuses are not an object at {label}")
        observed_axes = set(statuses)
        gate_status = str(row.get("m2_screen_eligibility_status") or "")
        if not observed_axes:
            gate_status_name = gate_status.split(":", 1)[0]
            if gate_status_name not in M2_PRE_AXIS_GATE_STATUSES:
                raise RuntimeError(
                    f"Empty M2 axis statuses require an allowlisted pre-axis "
                    f"NOT_EVALUABLE gate at {label}: {gate_status!r}"
                )
            census_status = f"{M2_PRE_AXIS_SUMMARY_STATUS}:{gate_status}"
            for axis in axes:
                counts[axis][census_status] += 1
            continue
        missing = sorted(axis_set.difference(observed_axes))
        extra = sorted(observed_axes.difference(axis_set))
        if missing or extra:
            raise RuntimeError(
                f"M2 axis status schema drift at {label}: missing={missing} extra={extra}"
            )
        for axis in axes:
            evidence = statuses[axis]
            if not isinstance(evidence, Mapping):
                raise RuntimeError(f"M2 axis evidence is not an object for {axis} at {label}")
            status = evidence.get("status")
            if not isinstance(status, str) or not status:
                raise RuntimeError(f"M2 axis status is missing for {axis} at {label}")
            counts[axis][status] += 1
    result = {
        axis: dict(sorted(axis_counts.items()))
        for axis, axis_counts in counts.items()
    }
    expected = len(site_rows)
    if any(sum(axis_counts.values()) != expected for axis_counts in result.values()):
        raise RuntimeError("M2 axis status census does not conserve selected site rows")
    return result
...
def build_summary(
    ...
    raw_identity_summary = {
        "release_contract": RAW_IDENTITY_RELEASE_CONTRACT,
        "equivalence_contract": RAW_DUPLICATE_EQUIVALENCE_CONTRACT,
        "analysis_scope_identity_contract": ANALYSIS_SCOPE_IDENTITY_CONTRACT,
        "allowed_differing_auxiliary_tags": sorted(
            RAW_DUPLICATE_ALLOWED_DIFFERING_TAGS
        ...
        "analysis_scope_guardrail": (
            "The digest covers every selected focal REF/ALT alignment and every downstream "
            "read-level value used by this analysis; it is not a whole-file digest of unused "
            "BAM or sidecar records."
        ),
    }
    ...
    return {
        ...
        "global_exact_fdr_family": {
            ...
            "m2_axis_status_count_contract": (
                "Each axis census conserves every selected site. A site rejected before "
                "axis classification by one of the three allowlisted pre-axis gates is counted "
                "as NOT_EVALUATED_M2_GATE_PRE_AXIS plus its declared gate status; no axis "
                "effect or p-value is imputed."
            ),
            "m2_axis_status_counts": summarize_m2_axis_status_counts(site_rows),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.sample or args.allow_partial_scope:
        raise ValueError(
            "Task-B full preflight cannot authorize a partial producer execution"
        )
    expected_command = canonical_producer_command(
        ...
        intersubmod_root=args.intersubmod_root,
    ...
    actual_command = resolve_release_command(
        argv=argv,
    ...
    python_cache_before = capture_python_cache_contract(args.output_dir)
    ...
    runtime_identity_before = capture_runtime_environment_identity()
    runtime_probe_before = runtime_statistical_api_probe()
    input_identity_before = capture_primary_input_identity(
        manifest=args.manifest,
        assignments=args.assignments,
        sites=args.sites,
        primary_artifact_audit_pre=args.primary_artifact_audit_pre,
    ...
    preflight_gate = validate_preflight_gate(
        args.preflight_receipt,
        manifest=args.manifest,
        assignments=args.assignments,
        sites=args.sites,
        ...
        independent_m2_audit=args.independent_m2_audit,
        primary_artifact_audit_pre=args.primary_artifact_audit_pre,
        current_code_identity=source_identity_before,
        current_runtime_identity=runtime_identity_before,
        current_runtime_probe=runtime_probe_before,
        producer_started_at_utc=started_at_utc,
    )
    ...
    runtime_identity_after_compute = capture_runtime_environment_identity()
    if runtime_identity_before != runtime_identity_after_compute:
        raise RuntimeError("Runtime environment identity changed during analysis")
    runtime_probe_after_compute = runtime_statistical_api_probe()
    if runtime_probe_before != runtime_probe_after_compute:
        raise RuntimeError("Runtime statistical API probe changed during analysis")
    python_cache_after_compute = capture_python_cache_contract(args.output_dir)
    if python_cache_before != python_cache_after_compute:
        raise RuntimeError("Formal Python cache contract changed during analysis")
    input_identity_after_compute = capture_primary_input_identity(
        manifest=args.manifest,
        assignments=args.assignments,
        sites=args.sites,
        primary_artifact_audit_pre=args.primary_artifact_audit_pre,
    ...
    if input_identity_before != input_identity_after_compute:
        raise RuntimeError("Primary input identity changed during analysis")
    ...
    runtime_identity_after_output = capture_runtime_environment_identity()
    if runtime_identity_before != runtime_identity_after_output:
        raise RuntimeError("Runtime environment identity changed while outputs were written")
    runtime_probe_after_output = runtime_statistical_api_probe()
    if runtime_probe_before != runtime_probe_after_output:
        raise RuntimeError("Runtime statistical API probe changed while outputs were written")
    python_cache_after_output = capture_python_cache_contract(args.output_dir)
    if python_cache_before != python_cache_after_output:
        raise RuntimeError("Formal Python cache contract changed while outputs were written")
    input_identity_after_output = capture_primary_input_identity(
        manifest=args.manifest,
        assignments=args.assignments,
        sites=args.sites,
        primary_artifact_audit_pre=args.primary_artifact_audit_pre,
    ...
    if input_identity_before != input_identity_after_output:
        raise RuntimeError("Primary input identity changed while outputs were written")
    code_artifacts = source_identity_after_output
    input_artifacts = {
        **input_identity_after_output,
    ...
    receipt = {
        ...
        "pass_semantics": "execution_integrity_only_pending_release_reconciliation",
        "task_type": "B_comprehensive_validation" if full_scope else "A_partial_explicit_scope",
        "scope": "all_manifest_samples" if full_scope else "explicit_sample_subset",
        "full_scope": full_scope,
        "raw_identity_release_contract": RAW_IDENTITY_RELEASE_CONTRACT,
        "preflight_gate": {
            "schema_name": preflight_gate["schema_name"],
            "schema_version": preflight_gate["schema_version"],
            "task_count": preflight_gate["observed"]["task_count"],
            "pass": preflight_gate["pass"],
            "raw_identity_release_contract": preflight_gate[
                "raw_identity_release_contract"
            ],
        },
        ...
        "input_lock": {
            "identity_before": input_identity_before,
            "identity_after_compute": input_identity_after_compute,
            "identity_after_output": input_identity_after_output,
            "all_primary_inputs_unchanged": True,
        },
        "runtime_environment_lock": {
            "identity_before": runtime_identity_before,
            "identity_after_compute": runtime_identity_after_compute,
            "identity_after_output": runtime_identity_after_output,
            "statistical_api_probe_before": runtime_probe_before,
            "statistical_api_probe_after_compute": runtime_probe_after_compute,
            "statistical_api_probe_after_output": runtime_probe_after_output,
            "direct_runtime_unchanged": True,
        },
        "python_cache_lock": {
            "identity_before": python_cache_before,
            "identity_after_compute": python_cache_after_compute,
            "identity_after_output": python_cache_after_output,
            "canonical_cache_unchanged": True,
        },
        "inputs": input_artifacts,
        "parameters": dict(PRODUCER_CANONICAL_PARAMETERS),
