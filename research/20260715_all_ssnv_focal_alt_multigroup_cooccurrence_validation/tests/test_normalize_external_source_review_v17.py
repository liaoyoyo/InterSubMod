...
MODULE_PATH = TOPIC_ROOT / "audit_notes" / "normalize_external_source_review_v17.py"


def load_module() -> ModuleType:
    spec = importlib.util.spec_from_file_location(
        "normalize_external_source_review_v17", MODULE_PATH
    )
...
def valid_payload(module: ModuleType) -> dict[str, object]:
    return {
        ...
        "reviewer_label": "External Reviewer A v17",
        ...
        "reviewed_assembler": {
            "path": str(module.EXPECTED_ASSEMBLER_PATH.resolve()),
            "size_bytes": 42_000,
            "sha256": "a" * 64,
            "mode": "0o444",
        },
        "canonical_junit": {
            "artifact": {
                "path": str(module.EXPECTED_JUNIT_PATH.resolve()),
                "size_bytes": 120_000,
                "sha256": "b" * 64,
                "mode": "0o444",
            },
...
def test_wrong_assembler_path_is_rejected(
    module: ModuleType, valid_payload: dict[str, object]
) -> None:
    valid_payload["reviewed_assembler"]["path"] = "/tmp/wrong-assembler.py"
    with pytest.raises(module.ReviewValidationError, match="misbound"):
        module.validate_review(valid_payload, reviewer_token="A", require_approve=True)
...
def test_nonclean_junit_counts_are_rejected(
    module: ModuleType,
    valid_payload: dict[str, object],
    field: str,
    value: object,
) -> None:
    valid_payload["canonical_junit"]["counts"][field] = value
    with pytest.raises(module.ReviewValidationError, match="zero failures"):
        module.validate_review(valid_payload, reviewer_token="A", require_approve=True)


def test_extra_junit_identity_key_is_rejected(
    module: ModuleType, valid_payload: dict[str, object]
) -> None:
    valid_payload["canonical_junit"]["artifact"]["unexpected"] = True
    with pytest.raises(module.ReviewValidationError, match="exact artifact"):
        module.validate_review(valid_payload, reviewer_token="A", require_approve=True)
