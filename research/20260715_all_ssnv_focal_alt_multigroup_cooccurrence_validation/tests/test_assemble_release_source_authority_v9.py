...
MODULE_PATH = (
    TOPIC_ROOT / "audit_notes" / "assemble_release_source_authority_v9.py"
)


def load_module() -> ModuleType:
    spec = importlib.util.spec_from_file_location(
        "assemble_release_source_authority_v9", MODULE_PATH
    )
...
def valid_review_payload(normalizer: ModuleType, token: str = "A") -> dict[str, object]:
    return {
        ...
        "reviewer_label": f"External Reviewer {token} v16",
        "reviewer_id": (
...
def test_retired_v9_anchor_rejects_rotated_live_release_module(
    module: ModuleType,
) -> None:
    with module.BoundArtifactReader() as reader:
...
def test_bound_v5_signer_round_trips_exact_approval_identity(
    module: ModuleType,
    tmp_path: Path,
) -> None:
