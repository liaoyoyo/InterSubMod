...
MODULE_PATH = (
    TOPIC_ROOT / "audit_notes" / "assemble_release_source_authority_v7.py"
)


def load_module() -> ModuleType:
    spec = importlib.util.spec_from_file_location(
        "assemble_release_source_authority_v7", MODULE_PATH
    )
...
def valid_review_payload(normalizer: ModuleType, token: str = "A") -> dict[str, object]:
    return {
        ...
        "reviewer_label": f"External Reviewer {token} v14",
        "reviewer_id": (
...
def test_retired_v7_anchor_rejects_rotated_live_release_module(
    module: ModuleType,
) -> None:
    with module.BoundArtifactReader() as reader:
