...
def audit_raw_identity_task(task: dict[str, Any]) -> dict[str, Any]:
    ...
    missing = sorted(required.difference(raw_audit))
    if missing:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    source_modes_before = {
        name: oct(Path(record["path"]).stat().st_mode & 0o777)
        for name, record in source_identity_before.items()
    }
    ...
    checks = {
        ...
        > 0,
        "raw_identity_matched_record_reconciliation": int(
            raw_aggregate.get("matched_record_occurrences", -1)
        )
        == int(raw_aggregate.get("expected_projection_occurrences", -2))
        + int(raw_aggregate.get("duplicate_extra_record_occurrences_collapsed", -3)),
        "raw_identity_duplicate_classification_reconciliation": int(
            raw_aggregate.get("duplicate_projection_occurrences_collapsed", -1)
        )
        == int(raw_aggregate.get("exact_duplicate_projection_occurrences_collapsed", -2))
        + int(raw_aggregate.get("rg_only_duplicate_projection_occurrences_collapsed", -3)),
        "raw_identity_missing_zero": raw_identity["missing_projections"] == 0,
        "raw_identity_conflicting_payload_zero": (
            raw_identity["conflicting_analysis_payload_projections"] == 0
        ),
