...
def resolve_unique_eligible_alignments(
    ...
    """Resolve every reads.tsv projection to exactly one C++-eligible raw alignment."""
    ...
    ambiguous = sorted(key for key, values in matches.items() if len(values) != 1)
    if missing or ambiguous:
        raise RuntimeError(
            f"Raw BAM identity recovery failed at {chrom}:{pos1}: "
            f"missing={missing[:3]} ambiguous={ambiguous[:3]}"
        )
...
def recover_site_reads(
    ...
    alignments = resolve_unique_eligible_alignments(bam, focal.chrom, focal.pos, by_projection)
...
def analyze_site_task(task: dict[str, Any]) -> dict[str, Any]:
    ...
    recovered = recover_site_reads(assignment, focal, partners, entry)
...
def build_summary(
    ...
    return {
        ...
        "read_contract": {
            "raw_bam_unique_identity_recovery": True,
