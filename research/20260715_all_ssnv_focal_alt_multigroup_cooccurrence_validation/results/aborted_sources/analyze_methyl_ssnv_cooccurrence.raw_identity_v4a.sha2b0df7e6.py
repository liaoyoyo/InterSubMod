...
def analyze_site_task(task: dict[str, Any]) -> dict[str, Any]:
    ...
    site_row: dict[str, Any] = {
        ...
        "raw_identity_expected_projections": int(
            raw_identity_audit.get("expected_projections", len(recovered))
        ),
        "raw_identity_matched_records": int(
            raw_identity_audit.get("matched_records", len(recovered))
        ),
        "raw_identity_duplicate_projections_collapsed": int(
            raw_identity_audit.get("duplicate_projections_collapsed", 0)
        ),
        "raw_identity_duplicate_extra_records_collapsed": int(
            raw_identity_audit.get("duplicate_extra_records_collapsed", 0)
        ),
        "raw_identity_exact_duplicate_projections_collapsed": int(
            raw_identity_audit.get("exact_duplicate_projections_collapsed", 0)
        ),
        "raw_identity_rg_only_duplicate_projections_collapsed": int(
            raw_identity_audit.get("rg_only_duplicate_projections_collapsed", 0)
        ),
        "raw_identity_duplicate_projection_sha256": str(
            raw_identity_audit.get("duplicate_projection_sha256", projection_digest([]))
        ),
    ...
    return {"site": site_row, "pairs": pair_rows}
...
def build_summary(
    ...
    raw_identity_summary = {
        ...
        "conflicting_analysis_payload_projections": 0,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    code_artifacts = {
        "analyzer": artifact(Path(__file__).resolve()),
        "ssnv_cooccurrence_lib": artifact(Path(LIB.__file__).resolve()),
        "latest_tag_join": artifact(Path(TAGS.__file__).resolve()),
        "m2_screen_gate": artifact(Path(M2_GATE.__file__).resolve()),
    }
    input_artifacts = {
    ...
    receipt = {
        ...
        "counts": {
            ...
            "raw_identity_conflicting_analysis_payload_projections": 0,
            "raw_identity_missing_projections": 0,
        },
