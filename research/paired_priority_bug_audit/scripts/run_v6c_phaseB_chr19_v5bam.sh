#!/bin/bash
# V6-C Phase B (V5 BAM): chr19 子集 ISM × 2 flag (germline-hp-only on/off) region-level cross-tab
# 與 run_v6c_phaseB_chr19.sh 平行對照（V3F BAM vs V5 BAM 直接 head-to-head）
...
TUMOR_BAM=/big7_disk/liaoyoyo2001/longphase-to-mod/output/threshold_compare/v5_flag/tumor_tagged.bam
...
mkdir -p "$OUT_BASE"
echo "[V6-C Phase B V5 BAM] $(date) — chr19 ISM × 2 flag (V5 BAM head-to-head with V3F)"
echo "  TUMOR_BAM (V5):  $TUMOR_BAM"
echo "  VCF_TP:          $VCF_TP"
echo "  VCF_FP:          $VCF_FP"
echo "  REF:             $REF"
echo "  LOH BED (V5):    $LOH_BED"
...
echo
echo "=== Run 1/4: V5 BAM flag=off, TP ==="
run_ism "off" "" "$VCF_TP" "tp"

echo
echo "=== Run 2/4: V5 BAM flag=off, FP ==="
run_ism "off" "" "$VCF_FP" "fp"

echo
echo "=== Run 3/4: V5 BAM flag=on, TP ==="
run_ism "on" "--germline-hp-only" "$VCF_TP" "tp"

echo
echo "=== Run 4/4: V5 BAM flag=on, FP ==="
run_ism "on" "--germline-hp-only" "$VCF_FP" "fp"

echo
echo "[V6-C Phase B V5 BAM] $(date) — DONE"
