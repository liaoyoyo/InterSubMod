#!/usr/bin/env python3
"""Extract sparse molecule×sSNV calls with identity-safe duplicate collapse.

The extractor uses ``samtools view -u -M -L`` so one coordinate-sorted primary
alignment overlapping any target sSNV is normally emitted once. HCC1395 also
contains redundant BAM rows with the same canonical coordinate identity. The
manifest explicitly declares ``collapse_redundant_rows_with_identical_HP_PS``;
this version implements that policy and verifies that duplicate rows have the
same target calls, qualities, MAPQ, and authoritative HP/PS tags before keeping
one observation.
...
for import_dir in (SCRIPT_DIR, CONTRACT_SCRIPT_DIR):
...
def target_observation_digest(
    *,
    ...
    """Digest the complete target evidence used to validate duplicate collapse."""
    payload = {
        "hp": tags.hp,
        "ps": tags.ps,
...
def extract_one(
    ...
    *,
    ...
    duplicate_identity_policy = entry["read_tags"].get("duplicate_identity_policy")
    if duplicate_identity_policy != "collapse_redundant_rows_with_identical_HP_PS":
        raise RuntimeError(
            "unsupported read-tag duplicate identity policy: "
            f"{duplicate_identity_policy!r}"
        )
    ...
    samtools_executable = Path(samtools).expanduser()
    if not samtools_executable.is_absolute():
        resolved_samtools = shutil.which(str(samtools_executable))
        if resolved_samtools is None:
            raise FileNotFoundError(f"samtools executable not found: {samtools}")
        samtools_executable = Path(resolved_samtools)
    samtools_executable = samtools_executable.resolve(strict=True)
    if not samtools_executable.is_file() or not os.access(samtools_executable, os.X_OK):
        raise RuntimeError(f"samtools is not executable: {samtools_executable}")

    command = [
        str(samtools_executable),
        "view",
        "-u",
        "-M",
        "-L",
    ...
    seen_alignment_observation_digests: dict[bytes, bytes] = {}
            ...
            with pysam.AlignmentFile(process.stdout, "rb", check_sq=False) as stream, gzip.open(
                ...
                for alignment in stream.fetch(until_eof=True):
                    ...
                    if alignment.mapping_quality < mapq_min:
                        ...
                        continue
                    counts["canonical_eligible_alignments_pre_identity_collapse"] += 1
                    ...
                    observation_digest = target_observation_digest(
                        tags=tags,
                        indices=indices,
                        codes=codes,
                        qualities=qualities,
                    ...
                    previous_observation_digest = seen_alignment_observation_digests.get(aid_raw)
                    if previous_observation_digest is not None:
                        counts["duplicate_alignment_identities_seen"] += 1
                        if previous_observation_digest != observation_digest:
                            counts["duplicate_alignment_identity_conflicts"] += 1
                            raise RuntimeError(
                                "conflicting target evidence for duplicate alignment identity: "
                                f"{key}"
                            )
                        counts["duplicate_alignment_identities_collapsed"] += 1
                        continue
                    seen_alignment_observation_digests[aid_raw] = observation_digest
    ...
    receipt = {
        ...
        "parameters": {
            ...
            "samtools_executable": str(samtools_executable),
            ...
            "duplicate_alignment_identity_policy": duplicate_identity_policy,
            "duplicate_alignment_identity_verification": (
                "same canonical coordinate identity is collapsed only when target "
                "R/A/O/D/S/L/X calls, base qualities, MAPQ, HP, and PS are identical"
            ),
        },
        "command": command,
        "inputs": {
        ...
        "counts": {
            **dict(counts),
            "unique_alignment_ids": len(seen_alignment_observation_digests),
        ...
        "checks": {
            ...
            + counts["canonical_eligible_alignments_pre_identity_collapse"],
            "every_eligible_alignment_exact_sidecar_joined": joiner.matched
            == counts["canonical_eligible_alignments_pre_identity_collapse"]
            and joiner.missing == 0,
            "duplicate_identity_policy_matches_manifest": duplicate_identity_policy
            == "collapse_redundant_rows_with_identical_HP_PS",
            "duplicate_identity_collapse_conserved": (
                len(seen_alignment_observation_digests)
                + counts["duplicate_alignment_identities_collapsed"]
                == counts["canonical_eligible_alignments_pre_identity_collapse"]
                and counts["duplicate_alignment_identity_conflicts"] == 0
            ),
            "multi_region_alignment_unique": len(seen_alignment_observation_digests)
