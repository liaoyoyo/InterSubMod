#!/usr/bin/env python3
"""Deterministic size- and span-bounded ordered read-support partitioning.
...
exactly once, contain at most ``max_block_size`` sites, and optionally satisfy
``last_position - first_position <= max_block_span_bp``.  A singleton has
physical span zero and is therefore feasible for every valid span cap.
...
An edge whose ordered interval exceeds either hard block limit cannot fit in
any valid disjoint block.  Size-only, physical-span-only, and both-limit cases
are reported separately.  ``max_block_span_bp=None`` preserves the k-only
semantics of :mod:`read_support_partition`.
"""
...
UNAVOIDABLE_SPAN_CAP = "unavoidable_bp_span_gt_max_block_span_bp"
UNAVOIDABLE_BOTH = "unavoidable_span_gt_both_block_limits"
...
class PartitionResult:
    ...
    positions: Tuple[int, ...]
    max_block_size: int
    max_block_span_bp: Optional[int]
...
def partition_ordered_hypergraph(
    ...
    *,
    ...
    max_block_span_bp: Optional[int] = None,
) -> PartitionResult:
    """Partition ordered sites into optimal contiguous hard-bounded blocks.
    ...
    ``max_block_span_bp`` is inclusive, so a block whose endpoint difference
    equals the cap is feasible.
    """
    ...
    if max_block_span_bp is not None:
        if isinstance(max_block_span_bp, bool) or not isinstance(
            max_block_span_bp, int
        ):
            raise ValueError("max_block_span_bp must be an integer or None")
        if max_block_span_bp < 0:
            raise ValueError("max_block_span_bp must be non-negative")
    ...
    for edge in edges:
        ...
        physical_span_bp = (
            ordered_positions[indexed.hi] - ordered_positions[indexed.lo]
        )
        if (
            indexed.span_sites <= max_block_size
            and (
                max_block_span_bp is None
                or physical_span_bp <= max_block_span_bp
            )
        ):
    ...
    for start in range(n_sites):
        ...
        for end_inclusive in range(start, min(n_sites, start + max_block_size)):
            if (
                max_block_span_bp is not None
                and ordered_positions[end_inclusive] - ordered_positions[start]
                > max_block_span_bp
            ):
                # Positions are strictly increasing, so all later endpoints
                # for this start are also infeasible.
                break
    ...
    for end_exclusive in range(1, n_sites + 1):
        ...
        for start in range(first_start, end_exclusive):
            if (start, end_exclusive) not in block_gain:
                continue
    ...
    for indexed in indexed_edges:
        ...
        if block_index is not None:
            ...
            exceeds_size = indexed.span_sites > max_block_size
            exceeds_span_cap = (
                max_block_span_bp is not None
                and ordered_positions[indexed.hi] - ordered_positions[indexed.lo]
                > max_block_span_bp
            )
            if exceeds_size and exceeds_span_cap:
                status = UNAVOIDABLE_BOTH
                unavoidable_ids.append(indexed.edge.constraint_id)
            elif exceeds_size:
                status = UNAVOIDABLE
                unavoidable_ids.append(indexed.edge.constraint_id)
            elif exceeds_span_cap:
                status = UNAVOIDABLE_SPAN_CAP
                unavoidable_ids.append(indexed.edge.constraint_id)
            else:
                status = CUT
        dispositions.append(
    ...
    if max_block_span_bp is not None and any(
        block.span_bp > max_block_span_bp for block in blocks
    ):
        raise AssertionError("block-span contract was violated")
