#!/usr/bin/env python3
"""Build deterministic read-supported k<=8, span-capped sensitivity partitions.
...
This sensitivity variant adds an optional hard physical-span constraint:
``block_end - block_start <= max_block_span_bp``.  Omitting the cap preserves
the k-only partition semantics.  It is intentionally independent from the
production k-only scripts so an active full run cannot be changed in place.
"""
...
from read_support_partition_span_cap import (
    CUT,
    RETAINED,
    UNAVOIDABLE,
    UNAVOIDABLE_BOTH,
    UNAVOIDABLE_SPAN_CAP,
    Hyperedge,
    PartitionResult,
...
SCHEMA_NAME = "intersubmod.k_gt8_read_supported_segmentation_span_cap"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for component in components:
        ...
        raw_result = partition_ordered_hypergraph(
            ...
            max_block_span_bp=args.max_block_span_bp,
        )
            ...
            constraint_rows.append(
                {
                    ...
                    "span_bp": key.positions[-1] - key.positions[0],
        ...
        component_row = {
            ...
            "unavoidable_size_pattern_count": sum(
                row.status == UNAVOIDABLE
                for row in raw_result.dispositions
            ),
            "unavoidable_span_cap_pattern_count": sum(
                row.status == UNAVOIDABLE_SPAN_CAP
                for row in raw_result.dispositions
            ),
            "unavoidable_both_limits_pattern_count": sum(
                row.status == UNAVOIDABLE_BOTH
                for row in raw_result.dispositions
            ),
        ...
        global_counts["unavoidable_size_patterns"] += sum(
            row.status == UNAVOIDABLE
            for row in raw_result.dispositions
        )
        global_counts["unavoidable_span_cap_patterns"] += sum(
            row.status == UNAVOIDABLE_SPAN_CAP
            for row in raw_result.dispositions
        )
        global_counts["unavoidable_both_limits_patterns"] += sum(
            row.status == UNAVOIDABLE_BOTH
            for row in raw_result.dispositions
        )
    ...
    checks = {
        ...
        "every_block_span_lte_max": (
            args.max_block_span_bp is None
            or all(
                int(row["span_bp"]) <= args.max_block_span_bp
        ...
        "unavoidable_reason_counts_conserved": (
            global_counts["unavoidable_patterns"]
            == global_counts["unavoidable_size_patterns"]
            + global_counts["unavoidable_span_cap_patterns"]
            + global_counts["unavoidable_both_limits_patterns"]
        ),
    ...
    component_fields = list(component_rows[0]) if component_rows else [
        ...
        "unavoidable_size_pattern_count",
        "unavoidable_span_cap_pattern_count",
        "unavoidable_both_limits_pattern_count",
    ...
    receipt = {
        ...
        "claim_ceiling": (
            "optimal local disjoint size/span-bounded partition under observed "
            "read constraints; not a unique true evolutionary tree"
        ),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional inclusive hard cap on block end-start in bp; omit to "
            "preserve the k-only semantics"
        ),
    )
