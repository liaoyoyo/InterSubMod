#!/usr/bin/env python3
"""LOSO 5-fold CV using DecisionTreeClassifier (max_depth=5, min_samples_leaf=100).

Single-algorithm wrapper around the multi-algo grid in
`research/methyl_augmented_filter_phase2/phase2_completeness_audit/scripts/A4_multi_algo_LOSO.py`.
Reuses the framework from run_loso_cv.py (LR baseline). Single seed = 42 for parity
with cycle 4 LR baseline; multi-seed run lives in A4_multi_algo_LOSO.py.
"""
...
import logging
LOGGER = logging.getLogger("loso_DT")
...
OUT_TSV = LOSO_DIR / "data" / "loso_DT_results.tsv"
OUT_FIG = LOSO_DIR / "figures" / "loso_DT_5sample_dF1.png"
OUT_SUMMARY = LOSO_DIR / "intermediate" / "loso_DT_summary.json"
...
def loso_fold(test_sample: str, sample_dfs: dict) -> dict:
    ...
    clf = DecisionTreeClassifier(
        max_depth=5, min_samples_leaf=100, random_state=PRIMARY_SEED,
    )
...
def main():
    ...
    summary = {
        ...
        "hyperparameters": {"max_depth": 5, "min_samples_leaf": 100, "random_state": PRIMARY_SEED},
    ...
    ax.set_title("LOSO — DecisionTreeClassifier (max_depth=5, leaf=100, seed=42)")
    ax.set_ylabel("ΔF1")
