#!/usr/bin/env python3
"""Step 4 stage 1 — Build per-sample V6-only master TSV (4 samples: H1437/H2009/HCC1954/HCC1937).

Faster v2: uses shell `find` for path discovery (faster than os.walk for large trees),
then parallel processes via ProcessPoolExecutor for parsing.

Reads phaseD_v6_5sample/{sample}/{off,on}_{tp,fp}/filtered_snv_{tp,fp}/{chr}/{snv_pos}/{region_id}/reads/reads.tsv,
aggregates HP family counts per region, joins caller_af from VCFs and LOH/CN from
research/tpfp_loh_af_kde_discrimination/data/master.tsv.gz.
...
def discover_reads_tsv(run_dir: Path) -> list[str]:
    """Use shell `find` to discover reads.tsv paths fast."""
    if not run_dir.exists():
        return []
    try:
        proc = subprocess.run(
            ["find", str(run_dir), "-name", "reads.tsv", "-type", "f"],
            capture_output=True,
            text=True,
            check=True,
        )
        ...
        # fall back to os.walk if find fails
        out = []
...
def aggregate_run(run_dir: Path, n_workers: int = 32) -> dict[str, dict]:
    print(f"    discovering reads.tsv ...", flush=True)
    files = discover_reads_tsv(run_dir)
    print(f"    found {len(files):,} reads.tsv; parsing with {n_workers} workers ...", flush=True)
    out: dict[str, dict] = {}
    if not files:
        return out
...
def build_sample(sample: str) -> dict:
    import time
    t0 = time.time()
    print(f"\n[Step 4 build] {sample} START", flush=True)
    data: dict[str, dict[str, dict]] = {f: {} for f in FLAGS}
    for flag in FLAGS:
        for label in LABELS:
            run_dir = PHASED / sample / f"{flag}_{label}"
            print(f"  {sample}/{flag}_{label}: aggregating run_dir", flush=True)
            agg = aggregate_run(run_dir, n_workers=32)
            data[flag][label] = agg
            print(
...
def main() -> int:
    ...
    # Append mode if existing rows; otherwise write fresh header
    existing = []
    if out.exists():
        with out.open() as fh:
    ...
    with out.open("w", newline="") as fh:
        ...
        for s_name, row in sorted(keep.items()):
            w.writerow(row)
    print(f"\n[Step 4 build] wrote summary → {out}", flush=True)
    return 0
