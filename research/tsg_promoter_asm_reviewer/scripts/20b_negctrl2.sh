...
mkdir -p "$WORK/negctrl2"
MASTER="$WORK/asm_dualaxis_germline_negctrl2.tsv"
rm -f "$MASTER"
...
for CHR in $CHROMS; do
  VCF="$WORK/negctrl2/${CHR}_germ.vcf"
  cp "$CLAIRS_HDR" "$VCF"
  ...
  OUT="$WORK/negctrl2/msa_$CHR"; rm -rf "$OUT"; mkdir -p "$OUT"
  "$MSA" -v "$VCF.gz" -r "$REF" -t "$TUMOR" -n "$NORMAL" \
    ...
    > "$WORK/negctrl2/${CHR}_msa.log" 2>&1
  L1=$(find "$OUT" -name "level1_raw_methylation_details.tsv.gz" | head -1)
  if [ -n "$L1" ] && [ -f "$L1" ]; then
    python3 "$PIVOT" "$L1" "$MASTER" "$LOH" 0.5 2>> "$WORK/negctrl2/${CHR}_pivot.log"
    rm -f "$L1"
  fi
done
