...
int main() {
    ...
    try {
        ...
        std::cout << "✓ Matrix dimensions: " << matrix.size() << " reads × " << (matrix.empty() ? 0 : matrix[0].size())
                  << " CpGs" << std::endl;
        if (!cpg_pos.empty()) {
        ...
        writer.write_region(test_snv, test_chr,
                            0,  // region_id
                            region_start, region_end, matrix_builder.get_reads(), cpg_pos, matrix, elapsed_ms,
