...
bool SomaticSnvTable::save_to_tsv(const std::string& path, const ChromIndex& chrom_index) const {
    ...
    for (const auto& snv : snvs_) {
        out << snv.snv_id << "\t" << chrom_index.get_name(snv.chr_id) << "\t" << snv.pos << "\t" << snv.ref_base << "\t"
            << snv.alt_base << "\t" << snv.qual << "\t" << (snv.is_pass_filter ? "PASS" : "FAIL") << "\t"
            << snv.somatic_conf << "\n";
    }
...
bool SomaticSnvTable::load_from_vcf(const std::string& vcf_path, ChromIndex& chrom_index) {
    ...
    Utils::Logger::info("Finished loading VCF. Loaded: " + std::to_string(snv_count) +
