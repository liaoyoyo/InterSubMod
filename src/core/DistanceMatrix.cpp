 ...
 */
static double calculate_nhd(
    const Eigen::VectorXi& binary_row_i,
    const Eigen::VectorXi& binary_row_j,
    int min_cov,
    int& common_count
) {
    common_count = 0;
 ...
 */
static double calculate_l1(
    const Eigen::VectorXd& raw_row_i,
    const Eigen::VectorXd& raw_row_j,
    int min_cov,
    int& common_count
) {
    common_count = 0;
 ...
 */
static double calculate_distance_impl(
    const MethylationMatrix& mat,
    int row_i,
    int row_j,
    ...
    switch (config.metric) {
        ...
        case DistanceMetricType::JACCARD: {
            Eigen::VectorXi vec_i = mat.binary_matrix.row(row_i);
            Eigen::VectorXi vec_j = mat.binary_matrix.row(row_j);
            return calculate_jaccard(vec_i, vec_j, config.min_common_coverage, common_count, config.jaccard_include_unmeth);
        }
...
void DistanceMatrix::compute_from_methylation(
...
void DistanceMatrix::compute_subset(
    const MethylationMatrix& methyl_mat, 
...
void DistanceMatrix::compute_subset(
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            int common_count = 0;
            double dist = calculate_distance_impl(
                methyl_mat, row_indices[i], row_indices[j], config, common_count
            );
    ...
    if (this->num_valid_pairs > 0) {
        this->avg_common_coverage = 
            static_cast<double>(total_common_coverage.load()) / this->num_valid_pairs;
    }
}
...
DistanceMatrix DistanceCalculator::compute(
    const MethylationMatrix& methyl_mat,
...
std::string DistanceCalculator::metric_to_string(DistanceMetricType type) {
    switch (type) {
        case DistanceMetricType::NHD: return "NHD";
        case DistanceMetricType::L1: return "L1";
        case DistanceMetricType::L2: return "L2";
        case DistanceMetricType::CORR: return "CORR";
        case DistanceMetricType::JACCARD: return "JACCARD";
