...
bool TreeWriter::write_linkage_matrix(const Tree& tree, const std::string& filepath) const {
    ...
    for (const auto& record : records) {
        ofs << record.cluster_i << "\t" << record.cluster_j << "\t" << std::fixed
            << std::setprecision(options_.precision) << record.distance << "\t" << record.new_cluster_id << "\t"
            << record.size << "\n";
    }
...
bool TreeWriter::write_tree_stats(const Tree& tree, const std::string& filepath) const {
    ...
    ofs << "Tree height (root): " << std::fixed << std::setprecision(6) << root->height << "\n\n";
    ...
    if (!bootstrap_values.empty()) {
        ...
        int high_support =
            std::count_if(bootstrap_values.begin(), bootstrap_values.end(), [](double bs) { return bs >= 95.0; });
