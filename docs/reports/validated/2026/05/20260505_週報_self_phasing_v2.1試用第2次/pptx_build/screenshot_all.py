#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""screenshot_all.py — Render PIL wireframe PNGs for the 18-slide deck.

Adapted from v3 24-slide screenshot_all.py (verified working). LibreOffice not
required: render directly from .pptx shape geometry using Pillow + per-character
Latin/CJK font fallback.

Per-slide checks:
    1. notes non-empty AND >= 300 chars
    2. shapes within slide bounding box
    3. title presence
    4. wireframe PNG renders all shapes & text

Outputs:
    wireframes/slide_NN.png       (one per slide)
    wireframe_report.txt          (issue summary)
"""
...
WIREFRAME_DIR = HERE / "wireframes"
WIREFRAME_DIR.mkdir(parents=True, exist_ok=True)

PPTX_PATH = HERE / "output.pptx"
REPORT_PATH = HERE / "wireframe_report.txt"
...
# 1 inch = 96 px → 16:9 slide = 1280 x 720
SCALE_PX_PER_EMU = 96.0 / 914400.0
...
def check_slide_structure(idx, slide):
    ...
    if not notes_text:
        ...
        issues.append(f"slide {idx}: speaker note too short ({len(notes_text)} < {MIN_NOTES_CHARS} chars)")
...
def main():
    ...
    if all_issues:
        summary_lines.append(f"[ISSUES] {len(all_issues)} total ({len(hard_fails)} HARD FAILS):")
    ...
    if all_issues:
        ...
        if len(all_issues) > 20:
            print(f"  ... ({len(all_issues) - 20} more, see {REPORT_PATH})")
