...
**caveat（關鍵·誠實）**：**subclone/copy-partition**（非 CN-dosage；**HP1-1 是 longphase-S 的 somatic subclone tag〔germline-H1+somatic-ALT, HaplotagStrategy.cpp:505-516〕非 copy**）仍在最強訊號上 confound HP-axis magnitude——HP-axis cis 在 amplified 區 conflate subclone/copy 與 allele。816 HP-axis loci 經 Bonferroni+copy-test 後**唯一乾淨 cis = chr17:79,991,120 (TBC1D16)**（within d=0.142 > HP-axis 0.122, perm p=0.001）；**BRCA2 (chr13) + chr5 = subclone/copy-confounded**（BRCA2 HP-axis −0.122 = subclone-vs-germline 差 d_copy −0.11 + 邊際 focal d_within −0.023, perm p=0.024, n=19/19；**% split 不 robust**）。**6 個最強訊號 untestable**（pure-ALT tag, CGI-desert；copy-test 只能算 leaky-tag 7/60）。partition 未 anchor SEQC2 CN ground-truth。⇒ **勿寫「BRCA2 真 cis-driven」、勿寫「copy number」、勿寫精確 %；BRCA2 = subclone/copy 主導例（焦點 cis 單樣本不可分離），chr17/TBC1D16 = 唯一乾淨 cis exemplar（稀有但真實，單 locus 單樣本 n=16/14）**。⚠ 故 **Martin-Trujillo (CN-dosage 82-92%) 不 corroborate**——dosage 已 REFUTED（MW p=0.6183），confound 是 subclone 非 CN-dosage（category error）。

**§N2b 延伸（2026-06-10）— 連 clusterability 也非 coverage/CN-dosage 假象 + LOH-抑制機制（正向）**：dosage falsification 從「分群 **magnitude**」延伸到「**clusterability**（位點能否分群）」：(1) **coverage**：18 sample×class cells ρ(clustered,coverage) 全 |ρ|<0.25，power floor(<50×)後 plateau，多變量控制後 coverage OR=0.91 **轉負** → 非 coverage-driven；(2) **CN**：clustered-rate neutral 0.222 > gain 0.179 ≫ **loh 0.022**，**LOH logistic OR=0.07**，gain 固定覆蓋亦不增 → 非 dosage 灌大；(3) **真 driver = n_CpG**（OR=1.20；cov+nCpG+CN 僅解釋 8% 變異, pseudo-R²=0.083）；(4) **LOH-抑制機制 6/6 樣本 OR<1**（0.01–0.107, 3 癌種）= **正向 mechanistic 證據**：read-clustering 需 ≥2 haplotype 等位甲基差，LOH 移除第二 allele 故分不了 → **分群是真等位生物學非技術假象**。⚠ 區分：NGroups（HPFineNGroups）與 coverage 強相關 ρ=0.51 **但屬 HP-tag occupancy 取樣非甲基**（HD-4）。[src: `tsg.../genome_survey_v2/catalog/clusterability_vs_cov_cn.json` + `clusterability_cov_cn_extended.json`；scripts 101/102；完整報告 `InterSubMod/docs/experiments/in_progress/2026/06/20260610_methylation_clusterability_vs_coverage_CN_characterization_01.md`；ledger 97/98]
