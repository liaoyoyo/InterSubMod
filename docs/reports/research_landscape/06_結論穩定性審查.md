...
| **驗證狀態** | 原驗證：20260406 肉眼觀察報告 Section 8-11；**B.1-2（20260417）**：飽和假說否定 — NR-matched 6/6 bins Fisher p<0.05 且 Δ=+5 至 +14pp；**B.1-1（20260417）**：多變數 residualized AUC 控制 NR+AF+Cov+LOH 後 pooled TO NonLOH Δ=+0.025 ROBUST（M3 full AUC=0.641，吻合原聲稱 0.617）；**B.1-4**：LOH NGroups=4 僅 n=11，HPFineNGroups **僅適用 NonLOH** |
| **隱含假設** | ⚠️ (1) NGroups 上限=4 為 HP1/HP1-1/HP2/HP2-1 分群的合理代理；(2) TO mode self-phasing 修正後（P0-2）訊號保留；(3) 跨 NR bin 訊號外推合理 — NR-matched 證實 |
| **如果前提改變** | P0-2 haplotag 修正後 NGroups 分布可能變化；跨樣本外推至 baseline TP rate <0.85 的臨床樣本可能強化訊號（H2009 目前 ceiling 壓扁 Δ 至 +2.84pp）；Paired mode tp=0.99 ceiling 下 B.1-1 Δ=+0.002 完全失效 |
| **需要額外驗證** | (a) ~~B.1-1~~ **已完成 20260417**；(b) P0-2 後 haplotag 乾淨重測；(c) 臨床 cohort 在 baseline TP rate <0.85 下的 Δ；(d) B.2-2 Coverage_Multiple 是否能代理真實 CN（B.1-1 M4 加 Cov 後 Δ 未變化，可能 Cov 與 NR/AF 共線） |

**新增限制（20260417 B.1-2 + B.1-1 + B.1-3 驗證）**：
1. **LOH 不適用** — LOH 區域 NGroups 被單側 HP 壓縮至 1-2（NGroups=4 僅 n=11）
2. **Ceiling effect（H2009）** — overall TP rate=0.911 時 NGroups 訊號被壓扁（Δ=+2.84pp 弱）；Paired mode TP=0.99 完全 ceiling（B.1-1 Δ=+0.002 無效）
3. **小樣本 artifact（COLO829）** — TO NonLOH NR≥80 僅 143 rows；原「7/7 一致」需用 NR-bin weighted 分析重算才成立（weighted 後 7/7 POS）
4. **Per-sample ΔAUC 層級僅 1/7 ROBUST（B.1-1 新發現）** — 7 樣本 TO NonLOH 中僅 HCC1954 M3 Δ=+0.022 達 ROBUST 門檻，其餘 6 樣本 Δ<0.02；「7/7 方向一致」在 AUC 層級應精確化為「7/7 Δ>0、1/7 ROBUST」
5. **Per-sample Effect Size 5/7 POS + 2/7 特殊（B.1-3 新發現 20260417）** — TO NonLOH NR≥40 的 NGroups=4 vs <4 TP rate 差異：3/7 medium (d=+0.57~+0.60: HCC1395_DORADO/HCC1937/HCC1954)、2/7 small (HCC1395/H1437)、**1/7 negligible (H2009 d=+0.12, ceiling)、1/7 NEGATIVE (COLO829 d=-0.17, 小樣本 n_N4=206)**；「7/7 方向一致」應再精確化為「5/7 Cohen's d POS + 2/7 特殊情況」
