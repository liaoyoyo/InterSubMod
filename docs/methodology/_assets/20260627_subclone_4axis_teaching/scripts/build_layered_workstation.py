...
function dash(){
 ...
 (function(){
  ...
  h+='<div><h2>⑥ mutation-bearing 分母 + clone 數 c</h2><table><tr><th>類</th><th class="n">區數</th><th class="n">比例</th></tr>'
   ...
   +'</table><div class="note">🔴 本表 root-only = <b>region 有 mlhp positions 但全位點 0 實測 ALT</b>（保守下界·此樣本 '+detRoot.toLocaleString()+' 個 all-determined 屬此）→ all-determined '+rd.all_determined.toLocaleString()+' 扣此後 mutation-bearing determined '+detMB.toLocaleString()+'（'+pct(detMB,rd.all_determined)+'）。⚠ 另 07-10 `funnel_census` 用<b>更嚴的 tree-structure 定義</b>(determined 但樹為 root-only·含純REF)→ root-only 計數大得多(HCC1395 ~1739·chr1-22·<b>尚未併進本工作站</b>)→ 兩定義不同不可直接對照。c≥2「subclonal 候選」<b>≠已確認 subclone</b>(需排除 CN/single-bulk·⭐3)。分母(有 positions 的區)='+wpos.toLocaleString()+';noPos '+noPos.toLocaleString()+' 區無 mlhp。</div></div></div>';
 })();
