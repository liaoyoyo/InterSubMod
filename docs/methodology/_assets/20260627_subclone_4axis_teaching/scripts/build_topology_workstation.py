...
def _load_sample(dr):
    ...
    _ctj = json.load(open(ctp, encoding="utf-8")) if os.path.exists(ctp) else {}
    rec["candtrees"] = {x["region"]: x for x in _ctj.get("candidate_trees", [])}
    rec["ct_summary"] = _ctj.get("summary", {})  # gap#2: identifiability_table 等聚合
...
const TT={'linear(全直系)':'t_linear','branched(直系+姊妹)':'t_branched','star(全姊妹)':'t_star','single':'t_single','germline_only':'t_single','branched(需隱藏祖先)':'t_branched','branched(順序未定,需隱藏祖先)':'t_branched','linear(需隱藏祖先)':'t_linear','incompatible':'t_single'};
...
function regSit(r){ if(r.determinacy=='incompatible')return['衝突(成環)','#f03e3e'];
 var _d=r.determinacy||'';
 if(_d.indexOf('E_subcube_recovered')>=0)return['subcube救回(partial建樹)','#20c997'];
 if(_d.indexOf('recurrence_artifact')>=0)return['recurrence→artifact(m>1棄)','#e8590c'];
 if(_d.indexOf('recurrence_candidate')>=0)return['recurrence候選(m=1留)','#2f9e44'];
 if(_d.indexOf('recurrence_LOH')>=0)return['recurrence·LOH(m未定)','#e8590c'];
 if(_d.indexOf('recurrence')>=0)return['recurrence(m通道不可用)','#e8590c'];
 ...
 return['已確定','#37b24d']; }
// gap#3: recurrence 家族(artifact/candidate/LOH/不可用)在圖例聚合為單一計數
function recSum(m){var t=0;for(var k in (m||{})){if(k.indexOf('recurrence')>=0)t+=m[k];}return t;}
...
// topology_type 顯示正名(③):既有 solve_topology(row-laminar)建樹只能「多根 lineage」(各自從 germline 分出);
// 2026-07-04 修:pairwise 四型乾淨(nic==0)但 row-laminar 誤殺的區 → build_hidden_node_tree 建「深分支(推斷祖先)」(可畫 subclone-of-subclone)
const TTLABEL={'branched(直系+姊妹)':'多根 lineage','linear(全直系)':'linear 直系','star(全姊妹)':'star 多根','single':'single','germline_only':'germline','branched(需隱藏祖先)':'深分支(推斷祖先)','branched(順序未定,需隱藏祖先)':'深分支(推斷祖先·順序未定)','linear(需隱藏祖先)':'linear(推斷祖先)'};
...
function renderIdeo(){var ID=D.ideogram;var host=el('ideogram');if(!host)return;if(!ID){host.innerHTML='';return;}
 ...
 chroms.forEach(function(c,i){var pc=ID.per_chrom[c],y=14+i*24,w=PXW*pc.len/maxlen,bin=pc.bin;
   ...
   else if(mode=='situation'){(sitByChrom[c]||[]).sort(function(a,b){var o={'已確定':0,'pairwise 拼接':1,'多樹相容(欠定)':2,'順序 2-3 順位待定':3,'跨HP(兩棵樹)':4,'recurrence(Model A;需m通道拆)':5,'衝突(成環)':6};return (o[a[1]]||0)-(o[b[1]]||0);}).forEach(function(g){var x=X(g[0]);s+='<line x1="'+x+'" y1="'+(y-9)+'" x2="'+x+'" y2="'+(y-1)+'" stroke="'+g[2]+'" stroke-width="1.5" style="cursor:pointer" data-i="'+g[3]+'"><title>'+g[4]+' · '+sitDisp(g[1])+(g[5]&&g[5]!='unknown'?' · CN '+g[5]:'')+'</title></line>';});}
...
(function(){var sc=el('scorecard');if(!sc)return;var dt=D.detail||[];var tot=dt.length||1;
 ...
 sc.innerHTML='<div style="display:flex;gap:6px;flex-wrap:wrap;margin:8px 0">'
  +card('已確定',cnt['已確定']||0,'#37b24d')+card('pairwise',cnt['pairwise 拼接']||0,'#74c0fc')+card('單群欠定',cnt['多樹相容(欠定)']||0,'#adb5bd')+card('跨HP',cnt['跨HP(兩棵樹)']||0,'#f59f00')+card('順序待定',cnt['順序 2-3 順位待定']||0,'#ffd43b')+card('recurrence',recSum(cnt),'#e8590c')+card('衝突',cnt['衝突(成環)']||0,'#f03e3e')+card('subcube救回',cnt['subcube救回(partial建樹)']||0,'#20c997')+card('截斷>8',trunc,'#e67700')+card('LOH 區',lohN,'#9775fa')+card('癌基因命中',geneN,'#e64980')
...
(function(){var host=el('threeaxis');if(!host)return;var dt=D.detail||[];
 ...
 dt.forEach(function(r){var c=r.n_clusters<=3?String(r.n_clusters):'4+';if(!joint[c])return;var t=(r.topology_type||'').split('(')[0];joint[c][t]=(joint[c][t]||0)+1;});
 ...
 var consist='<div style="background:#fff9db;border:1px solid #ffe08a;border-radius:5px;padding:4px 9px;margin-bottom:6px;font-size:10.5px;color:#7a5c00">📐 <b>枚舉完整</b> ✅：此樣本 <b>'+Object.keys(shapeSet).length+'</b> 種 canonical 樹形（含 2026-07-04 隱藏祖先深分支樹形·樹同構式）。 <b>有效性</b>：incompatible（真 artifact:CN-gain multiplicity/pairwise 成環）<b>'+incpct+'%</b>（'+ninc+'/'+dt.length+'）— 跨樣本 0.0%–6.2%。🔴 <b>2026-07-04 修</b>:改用 pairwise 四型(n_independent_clean)判真衝突,舊 incompatible 拆為 救(隱藏祖先深分支樹)+recurrence-required(Model A)+真artifact,故 incompatible% 大降（原 0.4%–19.1% 含 row-laminar 誤殺）。樹空間小=經驗淺薄（深度多 1-2）,非壓縮 n^n。</div>';
...
const STAT_DICT={topology_type:{title:'拓樸型態',desc:'每區 read 群在系統發生樹上的形狀(只計 n_sSNV≥2 區)。🔴 <b>既有 row-laminar 建樹的表達力上限</b>:solve_topology 用 row-laminar(每 read 的 ALT 集合兩兩巢狀/互斥),多數「branched」只在 germline 根分支。<b>2026-07-04 修正</b>:pairwise 四型乾淨(n_independent_clean==0)但被 row-laminar 誤殺的區,改用 <b>build_hidden_node_tree 建「深分支(推斷祖先)」</b>—可畫 subclone-of-subclone,代價是 posits 未觀測祖先 clone(誠實標「推斷祖先」非觀測)。見 20260704_incompatible_reclassification_hidden_node_finding_01.md。',items:{'single':'單群:reads 全塌成一個基因型,無分支','linear(全直系)':'全直系鏈 germline→A→AB→…','branched(直系+姊妹)':'<b>多根 lineage</b>:多條從 germline 各自分出(root-laminar)','branched(需隱藏祖先)':'<b>深分支(推斷祖先)</b>:pairwise 四型乾淨、含 1+ 未觀測祖先 clone 的 subclone-of-subclone 樹(2026-07-04 救)','branched(順序未定,需隱藏祖先)':'深分支(推斷祖先)+ 同-support 字元順序未定','linear(需隱藏祖先)':'linear 鏈 + 推斷中間祖先','star(全姊妹)':'全姊妹:多條從 germline 各自分出','germline_only':'只有 germline'}},n_clusters:{title:'群數 c（=觀測到的不同 ALT 細胞群數）',desc:'c = 該區內「含 ≥1 個 somatic 突變的不同 genotype 向量」數 = 觀測到的不同 ALT 細胞群 / lineage 狀態（germline 全-R 向量不計）。c=1 即只觀測到單一 ALT 群（無第二群可比較定序→易落 single/欠定）；c=2 是最乾淨的兩 lineage 分離；c 越大分支越複雜。perfect-phylogeny 下 c ≤ k（觀察到 germline root 時，root 不計；k=該區 sSNV 數），實測 c 多為 1-2，故拓樸搜尋空間極小。',items:{}},determinacy:{title:'determinacy 可辨識性',desc:'樹「存在」≠「能辨識是哪棵」。🔴 2026-07-04 修:pairwise 四型(n_independent_clean)取代 altset-row-laminar 為真衝突判準 → 舊 incompatible 118 拆為 救30(建隱藏節點樹)+70 recurrence-required+18 真artifact(gain-multiplicity/cycle)。',items:{'A_determined(單分子向量)':'單分子向量唯一可辨識(含 2026-07-04 救回的隱藏祖先深分支樹)','A_ambiguous_order(缺中間群)':'缺中間群→累積順序未定','B_pairwise_structure':'pairwise 拼接,非單分子整跨','C_underdetermined':'單ALT群·缺連接read(只觀測到單一 ALT 群、無第二群可定序→需深覆蓋;非「多棵樹相容」之意)','recurrence_artifact(m>1;CN-amp)':'<b>Model A→artifact(棄)</b>:整數 CN≥3(m>1) → CN-multiplicity artifact(粗 categorical gate 漏掉的 GAINLOH;HCC1395 11 區)','recurrence_candidate(m=1)':'<b>Model A→真 recurrence 候選(留)</b>:diploid CN2/single-copy(m=1) → 某位元真翻兩次的候選(HCC1395 9 區;需 orthogonal 佐證非甲基)','recurrence_LOH_unresolved':'<b>Model A→未解</b>:copy-neutral LOH total=2、allele-specific CN 無 → m∈{1,2} CN 不能定(HCC1395 50 區;VAF L3 軟旗標:高VAF→likely artifact / 低VAF→likely recurrence,非硬判)','recurrence_required(m通道不可用)':'cn=unknown(非 HCC1395 6 樣本)→ 無 SEQC2 整數 CN,m 通道不可用,recurrence 未拆','incompatible':'真 artifact:CN-gain multiplicity(四型+gain) 或 pairwise 成環(has_cycle)','other':'單群無分支'}},n_roots:{title:'HP 根數',desc:'somatic 事件散在幾條 germline 單倍型。≥2 = 跨 HP(allelic,非 subclone)。',items:{}}};
...
(function(){let u=el('unlocatable');if(!u)return;let sm=(D.scoring&&D.scoring.summary)||{},sd0=sm.situation_dist;if(!sd0){u.innerHTML='';return;}
 let tot=Object.values(sd0).reduce((a,b)=>a+b,0)||1;
 let sd=Object.assign({},sd0); sd['recurrence(m-通道拆)']=recSum(sd0);  // gap#3: 聚合 4 個 recurrence 子類供位置圖/清單顯示
 let order=[['已確定','#37b24d'],['pairwise 拼接','#74c0fc'],['多樹相容(欠定)','#adb5bd'],['跨HP(兩棵樹)','#f59f00'],['順序 2-3 順位待定','#ffd43b'],['recurrence(m-通道拆)','#e8590c'],['衝突(成環)','#f03e3e']];
 ...
 let unloc=[['衝突(成環)','#f03e3e','四配子違反 → 無單一樹(多為 CN-gain 假環)'],['recurrence(m-通道拆)','#e8590c','非-gain pairwise 全四型→Model A recurrence;整數 CN m-通道拆(HCC1395):CN≥3=artifact棄 / diploid=候選留 / copy-neutral LOH=m未定(VAF L3)'],['跨HP(兩棵樹)','#f59f00','突變散在 H1/H2 → 非單一譜系、是兩棵樹(allelic)'],['多樹相容(欠定)','#adb5bd','只觀測到單一 ALT 群(無第二群可定序;非「多棵樹相容」之意)→需深覆蓋'],['順序 2-3 順位待定','#ffd43b','缺中間群 → 累積順序未定']];
 ...
 let nunloc=(sd['衝突(成環)']||0)+(sd['recurrence(m-通道拆)']||0)+(sd['跨HP(兩棵樹)']||0)+(sd['多樹相容(欠定)']||0)+(sd['順序 2-3 順位待定']||0);
 u.innerHTML=`<div class="zone" style="background:#fff0f6;color:#c2255c;border-left-color:#f06595">🔍 無法定位群偵測（草圖）—「群存在但定不出位置/順序」(此樣本 ${nunloc} 區)</div>
  ...
  <div class="note" style="margin-bottom:6px">合計 ${tot} 區｜🟢已確定 ${sd['已確定']||0}｜🔵pairwise 拼接 ${sd['pairwise 拼接']||0}(可建樹非單分子)｜<b style="color:#c2255c">↓ 5 種無法定位</b></div>${rows}
  ...
  <div class="note" style="margin-top:5px;color:#a37200">⚠ 「不同的樹」全列舉(替代整樹)= 上游 <b>enumerate_candidate_trees</b>(已實作;B1 缺中間群區 detail 有候選樹 carousel·等機率);成環區 detail 另有「可能固定樹」carousel(假環示意)。此面板誠實顯示「什麼定不出來」,不假裝在列舉答案。</div>
  ${(function(){var cs=(D.ct_summary||{});var it=cs.identifiability_table;if(!it)return'';var ord=['determined','ambiguous','recurrence(m通道;非乾淨可辨識)','conflict(真artifact/cycle)','nonenumerable(B_pairwise)','nonenumerable(C_underdetermined)','other'];var col={determined:'#37b24d',ambiguous:'#7048e8'};var tot2=Object.values(it).reduce((a,b)=>a+b,0)||1;var cells=ord.filter(k=>it[k]).map(k=>`<span style="color:${col[k]||'#868e96'}">▏${k.replace('nonenumerable','non-enum').replace('(m通道;非乾淨可辨識)','').replace('(真artifact/cycle)','')} <b>${it[k]}</b></span>`).join(' ');return `<div style="margin-top:8px;border-top:1px solid #f1f3f5;padding-top:7px;font-size:11.5px"><b>🌲 gap#2 identifiability（全 ${tot2} with-vector 區完整分區）</b>：${cells}<div class="note" style="margin-top:3px">B1 A_ambiguous 全枚舉 → <b>${cs.B1_determined||0} 收斂 determined + ${cs.B1_ambiguous||0} 真歧義</b>（含 parent-choice·修正舊「0 歧義」假象）·n_minimal_trees dedup·🔴絕不用甲基排序</div></div>`;})()}</div>`;})();
...
function show(i,row){el('list').querySelectorAll('.row').forEach(x=>x.classList.remove('sel'));if(row)row.classList.add('sel');let r=det[i];
 ...
 el('detail').innerHTML=`<h3>${r.region} <span class="tag ${TT[r.topology_type]||'t_single'}">${r.topology_type}</span> <span class="tag ctx_${r.genome_ctx}">${r.genome_ctx}</span></h3>
  ...
  ${(r.determinacy||'').indexOf('recurrence')>=0?`<div style="background:#fff4e6;border:1px solid #ffd8a8;border-radius:6px;padding:7px 10px;margin:6px 0;font-size:11.5px">🔶 <b>Model A recurrence — m-通道拆分</b>：${r.m_channel?({artifact_drop:'整數 CN='+(r.m_channel.cn_int!=null?r.m_channel.cn_int:'?')+'（m>1）→ <b>CN-multiplicity artifact，棄</b>（粗 categorical gate 漏掉的 GAINLOH）',candidate_keep:'CN='+(r.m_channel.cn_int!=null?r.m_channel.cn_int:2)+'（m=1）→ <b>真 recurrence 候選，留</b>（需 orthogonal 佐證，非甲基）',LOH_unresolved:'copy-neutral LOH total=2、allele-specific CN 無 → <b>m∈{1,2} 未解</b>；VAF L3 軟旗標：'+(r.m_channel.vaf_L3||'—')+'（max-VAF '+(r.m_channel.max_vaf!=null?r.m_channel.max_vaf:'—')+'，非硬判）',unavailable:'cn=unknown → 無 SEQC2 整數 CN，<b>m 通道不可用</b>'}[r.m_channel.verdict]||r.m_channel.verdict):'—'}<br><span class="note">非-gain pairwise 全四型（某位元須翻≥2次）= Model A recurrence-required。HCC1395 70 區 → <b>11 artifact棄 + 9 候選留 + 50 LOH未解</b>（誠實上限：LOH 無 allele-specific CN 不能定 m，不硬丟）。</span></div>`:''}
  ...
  ${(ctr&&ctr.candidate_set&&ctr.candidate_set.length)?(function(){var nmt=(ctr.n_minimal_trees!=null?ctr.n_minimal_trees:ctr.true_count);var ident=ctr.identifiability||(nmt<=1?'determined':'ambiguous');var single=(ident=='determined');var bg=single?'#eef9f0':'#f8f0fc',bd=single?'#b2dfc0':'#d0bfff';var pc=(ctr.resolution=='PARENT_CHOICE');var badge=single?`<b style="color:#2b8a3e">✅ determined（唯一最小樹）</b> <span class="note">(gap#2: n_minimal_trees=1·identifiability=determined·resolution=${ctr.resolution||'—'}·枚舉後收斂,非多候選)</span>`:`<b style="color:#7048e8">⚖ ambiguous：${nmt} 種最小樹（左右滑動）</b> <span class="note">(gap#2: identifiability=ambiguous·${pc?'<b>parent-choice 拓樸歧義</b>(節點多個等大觀測父·舊碼曾 silently 丟)':'缺中間群→插虛擬中間節點·'+(ctr.honest_note||'')}·<b>非給答案</b>·🔴甲基無法裁決)</span>`;return `<div style="background:${bg};border:1px solid ${bd};border-radius:6px;padding:9px;margin:8px 0">${badge}<div id="cttreebox" style="margin-top:6px">${candTreeCard(ctr,0,r)}</div></div>`;})():''}
