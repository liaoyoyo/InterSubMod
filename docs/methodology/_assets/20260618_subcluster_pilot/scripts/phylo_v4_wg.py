#!/usr/bin/env python3
"""全基因組 phylo-v4.1(modal coarse K=5 + fine + other + instability + hidden-het). RNULL=40 一致.
原 v3.1 版見 phylo_v3_wg.py.
"""
_OLD="""全基因組 phylo-v3.1 — TP+FP, big7 本機, 平行。對照 4-gate 31.3% 的『有結構』比例。
...
輸出 phylo_v4_wg_records.json + summary。用法: python3 phylo_v3_wg.py [CHRS]  e.g. 22(smoke) / 留空=1-22。"""
...
NPROC = 24; MINSZ = 3; SEP_MIN = 1.3; RNULL = 40; KSEED = 5
import phylo_v4 as V4; V4.RNULL = 40
...
def proc_locus(arg):
    ...
    try:
        ...
        a = V4.analyze_v4(sub, P, K=KSEED)
        lab = a["coarse_lab"]; ng = a["coarse_ng"]; nout = a["n_outlier"]; nother = a["n_other"]
        nonres = [l for l in lab if l not in ("outlier", "other")]
        maxdepth = max((l.count("-") for l in set(nonres)), default=0)
        aligned = False; Vhp = Val = 0.0
        if ng >= 2:
            idx = [i for i in range(n) if lab[i] not in ("outlier", "other")]
            hp = [reads[ids[i]]["hp"] for i in idx]; al = [reads[ids[i]]["alt_support"] for i in idx]; tl = [lab[i] for i in idx]
        ...
        return {"pos": rd.split("/")[-1].split("_")[1] if "_" in rd.split("/")[-1] else None, "set": setlab, "n": n, "C": nC,
                "ngroups": ng, "fine_ng": a["fine_ng"], "n_other": nother, "modal_frac": a["modal_frac"],
                "unstable": a["unstable"], "hidden_het": a["hidden_het_warn"], "maxdepth": maxdepth, "n_outlier": nout,
                "aligned": aligned, "V_hp": round(Vhp, 2), "V_allele": round(Val, 2)}
...
def run():
    ...
    for chrom in CHRS:
        for setlab, pref in (("TP", "filtered_snv_tp"), ("FP", "filtered_snv_fp")):
            ...
            od = f"{WT}/output/_pv4wg_{chrom}_{setlab}"; shutil.rmtree(od, ignore_errors=True); os.makedirs(od, exist_ok=True)
            ...
            json.dump(out, open(f"{A}/phylo_v4_wg_records.json", "w"))
    ...
    def summ(g):
        ...
        return {"n": len(g), "structure_pct": round(100 * len(multi) / len(g), 2),
                ...
                "fine_multi_pct": round(100 * sum(1 for r in g if r["fine_ng"] > r["ngroups"]) / len(g), 2),
    ...
    S = {"TP": summ(TP), "FP": summ(FP), "elapsed_s": int(time.time() - t0),
         "params": {"RNULL": RNULL, "K": KSEED, "SEP_MIN": SEP_MIN, "chrs": CHRS}}
    json.dump(S, open(f"{A}/phylo_v4_wg_summary.json", "w"), indent=2)
