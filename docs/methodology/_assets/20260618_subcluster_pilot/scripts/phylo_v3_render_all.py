...
每位點: UPGMA 樹(分支依 v3 演化群著色) + 演化群側欄 + HP + ALT + strand + 甲基 read×CpG + 距離塊 + 各群對齊圖例。
...
FIGS = f"{A}/figs_phylo_v3"; os.makedirs(FIGS, exist_ok=True)
MINSZ = 3; SEP_MIN = 1.3; RNULL = 40
# 每個終端標籤一個獨立類別色（修：原 family+depth 上色讓同家族多群撞色、肉眼分不出 4 群）
PALETTE = H.CLUSTER_COL


def make_colmap(labs):
    terms = sorted(set(l for l in labs if l not in (None, "outlier", "-")))
    return {t: PALETTE[i % len(PALETTE)] for i, t in enumerate(terms)}


def labcol(lab, cmap):
    if lab is None or lab in ("outlier", "-"): return "#cfcfcf"
    return cmap.get(lab, "#888888")
...
def phylo_v2(sub, P, rng):
    """v3.1: descend quarantine(物有所值) + per-subgroup null。函式名留 phylo_v2 以最小改 render。"""
    ...
    def rec(node, label):
        ...
        ok = test_split(bnode) if bnode is not None else False
        if ok:
...
for it in items:
    ...
    cmap = make_colmap(lab)  # 每終端標籤一獨立色（4 群=4 明顯色，不再撞紅）
    sb = [("演化群", [labcol(l, cmap) for l in lab_o]), ("HP", [HPC.get(h, "#ddd") for h in hp_o]),
    ...
    dendrogram(Z, orientation="left", link_color_func=lambda k: labcol(nodelab.get(k), cmap) if nodelab.get(k) else "#cccccc", ax=axdn, no_labels=True)
    axdn.set_xticks([]); axdn.set_yticks([]); axdn.set_title("UPGMA 樹(分支=v3演化群)", fontsize=8); [axdn.spines[s2].set_visible(False) for s2 in axdn.spines]
    ...
    axl.text(0.0, 0.7, f"v3 切割: 穩定群 {nterm} 個 + 離群 {nout}（標籤 {sorted(labct)}）", fontsize=8.5, fontweight="bold", transform=axl.transAxes)
    axl.text(0.0, 0.25, txt, fontsize=7.5, transform=axl.transAxes, wrap=True)
    fig.suptitle(f"{key}  n={n} CpG={len(cpgs)}  fine_class={it['class']}  →  v3 演化群 {nterm} 個 離群 {nout}", fontsize=9.5, y=1.0)
    fn = f"{FIGS}/v3_{key}.png"; fig.savefig(fn, dpi=104, bbox_inches="tight"); plt.close(fig)
    out.append({"key": key, "n": n, "C": len(cpgs), "fine_class": it["class"], "v2_ngroups": nterm,
                "labels": dict(labct), "align": galign, "n_outlier": nout, "png": f"figs_phylo_v3/v3_{key}.png"})
    print(f"  {key} n={n} C={len(cpgs)} → v2 群{nterm} {sorted(labct)} 離群{nout}", flush=True)
json.dump(out, open(f"{A}/phylo_v3_render.json", "w"), indent=1)
print(f"\nDONE {len(out)} 位點 · v2 多群 {sum(1 for o in out if o['v2_ngroups']>=2)} · 圖在 figs_phylo_v3/")
