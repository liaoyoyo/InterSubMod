...
**🔴 甲基雙峰 × SAVANA CN 因果驗證（07-06，`InterSubMod/docs/references/20260706_methyl_cn_line_handoff_sot_01.md` + `docs/experiments/in_progress/2026/07/20260706_methyl_vs_ssnv_lineage_01.md`；commit 1a23d74）**：補了 **SAVANA cna-only CN（4/7 可靠：HCC1395 SEQC2 + H1437 0.95/H2009 0.95/HCC1954 0.66；COLO829/HCC1937 purity mis-fit + DORADO = cn=unknown）**，用它把 methyl_auxiliary 的群內雙峰做 **CN-分層率檢定**（`methyl_bimodality_cn_rate.py` 8-chunk）。**因果結論：雙峰率跨 CN 平坦**（H2009 gain **0.170** ≈ neutral **0.163** = **1.04x**，最有 power；HCC1954 1.25x；H1437 2.41x 但 neutral n=49 不可靠）→ **CN-gain 未提高群內甲基雙峰率、雙峰率 ~15% 與 CN 無關**。🔴 **自我修正**：中間曾誤讀「77-86% residual 在 gain = CN 假象」= **base-rate 假象**（gain 區佔基因組大→分母效應，非因果）；教訓 count 富集≠因果、必看帶分母的 rate。**修正裁決**：群內甲基雙峰是 **CN-獨立的真實現象**（甲基有超越 CN 的獨立內容），**但仍屬 within-lineage 隱藏子結構 L3**（cis-ASM/HP-殘留/技術未排除），**bulk 不能證 subclone** → **維持 bounded-auxiliary**（現有 CN 定量佐證）。**Q5 難建樹區輔助（用戶想法）**：`C_underdetermined` 區 ALT-cluster cn-clean 雙峰 = 7 候選（H2009 6+HCC1954 1，Δβ 0.2-0.3）= L3 甲基輔助標記，需 normal-cis/single-cell 確認。觀察 HTML `docs/experiments/in_progress/2026/07/_assets/methyl_lineage/`。
