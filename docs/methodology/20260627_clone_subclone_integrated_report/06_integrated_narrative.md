...
| **L4 甲基** | **弱** corroborate（已重抽）| 既有輸出覆蓋 0.19% → **從 BAM 重抽 740 區**：只 **49（6.6%）corroborate，全 subclone-specific（0% cis）**；93% 不區分；🔴 非 detect | L1/L2 |
...
5. **甲基（已重抽 genome-wide）**：corroborate 非 detect；既有輸出覆蓋 0.19% → 從 BAM 重抽 740 區，僅 **6.6%（49）區域甲基區分遺傳 subclone（全 subclone-specific）**，93% 不區分 → **弱 corroborator**，符合「characterize 非 detect」。
