...
基於先前 [可行性分析報告](../../../concepts/2025/12/20251222_Bernoulli_01.md) 的結論，本文件詳細規劃在 `InterSubMod` 專案中實作 **Bernoulli 距離矩陣計算方法** 的開發步驟、程式碼修改細節與驗證計畫。
...
### 7.2 整合測試
執行 `scripts/run_vcf_all_snv.sh`，增加 `--mode all-with-w1000` (或自訂 mode)，檢查：
1.  程式是否正常執行無 Crash。
