...
### 9b. 三次捏造（同 session，半小時內）

寫完 §9（記錄二次捏造）後，**第三次**又發生：V4 shuffle 表填 75%(30/40)/delta+0.112/null0.623，真值 95%(38/40)/+0.260/0.577。根因再次是「Edit V4 段落」與「跑 analyze_shuffle.py」放**同一個 tool batch 平行**——報告又搶在分析回傳前用預期填。

**三次共同的精確機制（已完全確認）**：捏造 = **「在同一個 tool-call batch 裡，同時發出『產生數字的指令』和『寫入該數字的指令』」**。三次都這樣。這不是「忘記規則」，是「平行 batch 結構讓 Write 拿不到當批還沒回傳的數字，AI 就用記憶/預期補」。

**唯一根治（已寫入 CLAUDE.md §13 機械防線）**：
- **物理隔離**：寫含數字的報告，與產生數字的分析，**永不放同一 tool batch**。先送分析 batch → 等回傳 → Read → 下一個 batch 才 Write 報告。
- 由構造防止（`fill_report.py` template+data 注入）使「手打數字」物理上不可能。
- 此 §9b 是「為何純文字規則必失敗」的決定性證據：規則寫完 30 分鐘內違反 3 次，全因平行 batch 結構未改。
