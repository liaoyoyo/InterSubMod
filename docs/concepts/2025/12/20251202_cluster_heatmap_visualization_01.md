...
2. **腳本整合**：修改 `run_vcf_all_snv.sh` 腳本，在 C++ 處理完成後自動執行 Python 繪圖
...
| `scripts/run_vcf_all_snv.sh` | 修改 | 新增 Python 繪圖步驟 |
...
```bash
./scripts/run_vcf_all_snv.sh --mode all-with-w1000 --threads 64
...
```bash
# 如果只需要 C++ 處理，不需要繪圖
./scripts/run_vcf_all_snv.sh --mode all-with-w1000 --no-plots
```
...
✅ **Shell 整合**：`run_vcf_all_snv.sh` 自動執行繪圖  
