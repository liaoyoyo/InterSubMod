...
[`PUBLIC_SAFETY_RECEIPT.json`](longlineage_public_safety_receipt.json). Source and dependency
limitations are in [`THIRD_PARTY_NOTICES.md`](longlineage_third_party_notices.md) and
[`SBOM.spdx.json`](longlineage_SBOM.spdx.json).
...
production validation. See [`CAPABILITY_MATRIX.md`](longlineage_capability_matrix.md).
