...
### k_ISM × CN/SV per-read 整合（甲基分群是否受 CN 影響）全基因組 — ✅ 2026-06-21（HCC1395 ⭐3）
用已驗證 SAVANA CN 參考（LOH Jaccard 0.962）+ 現有 ISM per-region 樹（零重跑）回答「甲基分群 k 是否與 CN 數相關 / 多少受 CN 影響 / SV 有用嗎 / read 屬哪群」。**Q1 k_ISM vs k_CN 無相關**（Spearman ρ=−0.038；k_eff 中位數 LOH/het 皆 1）。**Q3 極少受 CN 影響**：9.5%(2,827/29,754) 區有顯著 cluster 結構（PERMANOVA gate；90% 無結構），其中**僅 0.64% 對齊 germline-HP(CN_explained_HP)**、het 區 **85% unaligned**、LOH 區 71.7% candidate_beyond_CN → **甲基分群非 CN 鏡子但也多不對齊任何遺傳軸=來源未定 epigenetic 異質**。**Q2 SV 稀疏弱**（SV-informative 1,292 區，對齊僅 5.1%）。**Q4 per-read 指派可行**（345,714 read，n_clusters==optimal_k 98.2%；ARI(cluster,hp)≈0）。FP 負對照結構率 3.8%(< TP 9.5%)、同 0% CN_explained。context：88.8% allele-PERMANOVA 顯著(ASM label-level≠cluster 對齊)。🔴 cluster 重建改 scipy 重算 UPGMA(C++ linkage 不相容 fcluster)；bulk 下 candidate≠subclone(cis-ASM 天花板)；⭐3 跨樣本需 COLO829。[報告](in_progress/2026/06/20260621_kism_vs_cn_perread/20260621_kism_vs_cn_perread_result_01.md) · script `scripts/analysis/analyze_kism_vs_cn_perread.py` · `_assets/{aggregate_tp,aggregate_fp}.json` · memory `project_ont_cnv_sv_subclone_verification_feasibility`

### 位點甲基切群判別流程 + per-CpG 多軸歸屬 + ISM vs modkit 對照（全基因組）— ⏳ 2026-06-20（HCC1395 ⭐2-3，4 觀察 HTML）
一串：**判別流程 5 態**（N=30,490 tumor/merged；②/③/④/⑤）→ **BRCA2 per-CpG 多軸定位**（carrier 18 sig CpG 焦點 −600~−169，符真值 −0.122）→ **ISM joint vs modkit-equiv 全資料 Venn**（both 19,637 / **ISM-only 1,593(5.22%)** / modkit-only 878；③切不出但 joint 顯著 15.6% per-CpG=0）→ **7 案 dual-panel**（modkit 漏/ISM 抓，用戶判皆 label-consistent 真結構）。**結論口徑**：per-CpG 定位 modkit/DSS 夠用（非 ISM 創新）；**ISM 差異化 = read-level 結構偵測（diffuse location + heterogeneity/dispersion + 無監督群發現，modkit 皆無對應）**；訊號弱、單樣本、結構≠subclone。**🗂️ 觀察入口（4 HTML+16 圖+3 doc 全索引）：[OBSERVATION_INDEX](../methodology/_assets/20260618_subcluster_pilot/OBSERVATION_INDEX.md)**。doc：[decisionflow](in_progress/2026/06/20260620_decisionflow_5state_classification_wg_01.md) · [BRCA2 per-CpG](in_progress/2026/06/20260620_per_cpg_multiaxis_attribution_brca2_pilot_01.md) · [ism_vs_modkit](in_progress/2026/06/20260620_ism_vs_modkit_percpg_comparison_wg_01.md)。🔗 姊妹線（真 modkit/DSS）：`20260620_subclone_label_methods_pilot/`。memory `project_ism_vs_external_methylation_tools_comparison` / `project_decisionflow_5state_classification_wg`。

### SAVANA ONT CNV/SV 可行性確認 + 全基因組固定 CN 參考層（HCC1395 vs SEQC2）— ✅ 2026-06-20（工具可行性 / GREEN，全基因組執行中，⭐3 單樣本）
SAVANA 1.3.7 在 ONT HCC1395（R10 5kHz Dorado）**跑通且 CN/LOH 與 SEQC2 truth 高度一致**：**chr8 LOH Jaccard 0.977（98.4% vs 96.3%）、chr2 0.920（54.5% vs 50.8%）**，purity **0.96**（純細胞系命中）/ploidy 2.68；`savana run` 14min 出 12,886 breakpoints + `read_support.tsv`（read-level SV 標籤）。補上內部 Coverage_Multiple（只連續 depth ratio）缺的 integer/allele/haplotype CN。**整合框架（用戶提案，非循環）**：CN 當固定觀察參考層比較 **k_ISM vs k_CN**（k<k_CN=甲基有界 / k≈=CN-mirror / k>=candidate subclone）→ 用 CN 分流各區甲基詮釋，補無監督 k 無外部錨的死角。**先前 refute 的「CN↔甲基對齊=subclone」仍成立**，此框架不同（CN 當尺規非 validator）。全基因組固定參考層過夜執行中（job btr4kbdrw）。env=cnvtools（bioconda libmamba；SAVANA/Wakhan 不在 PyPI）；Wakhan 待移獨立 env。[結果報告](in_progress/2026/06/20260620_savana_hcc1395_cnv_sv_feasibility_result_01.md) · plan `../plans/20260620_ont_cnv_sv_subclone_verification_feasibility_01.md`

### 三個甲基↔標籤關聯方法 BRCA2 pilot（想法1 per-CpG / 想法2 modkit-DSS / 想法3 per-label Δβ）— 🟡 2026-06-20（method comparison pilot / PARTIAL 單樣本 L2）
三法 = 甲基↔標籤關聯 granularity ladder，pilot 證**偵測不同層次非同義**。**想法1（per-CpG 三軸歸屬）可行 ✓**：22305 抓 18 subclone-sig CpG，HP-sig 平均 Δβ=**−0.106**（符 bespoke 真值 −0.122）；🔴 normal reads 全 hp=0 unphased→**normal-ASM-control 無法從 ISM 輸出做**。**想法2（modkit/DSS vs read-distance）**：強訊號 modkit-Fisher≈我方 MWU（22305 Jaccard **0.76**），但**二值化丟漸進訊號**（22306 我方 MWU 65 vs modkit 20）、**DSS 無重複過 liberal**（52 vs 18）；真 modkit 07 區域 −0.159 vs ISM −0.122 同向。**想法3（per-label Δβ）**：region-mean **稀釋局部訊號**（22305 三軸全 n.s.，漏 BRCA2 ASM）=偵測更少；全 scope（29,754）**想法3 ⊂ 距離法**（label-PERMANOVA 27,304=91.8% 近飽和，A−B=0~2），對 GlobalP 集則雙向差集；18.4% Δβ-sig 是純 LEVEL-shift；🔴 cross-check 揪出 CSV `HPFineD`≠想法3 字面定義(差10×)。三法**零重跑**(Python on per-region 矩陣)。⚠ allele germline-het null + 跨樣本(COLO829) deferred；characterization 非判別。[報告](in_progress/2026/06/20260620_subclone_label_methods_pilot/20260620_subclone_label_methods_pilot_report_01.md) · scripts `_assets/scripts/{lib_region,run_idea1,run_idea2,run_idea3,run_idea2_compare}.py` · data `_assets/data/*.json` · 推論 SoT `docs/plans/20260620_subclone_situation_verification_reasoning_01.md`

### TP vs FP × 結構-標籤關聯狀況刻畫（HCC1395）— 🟡 2026-06-19（characterization / PARTIAL 單樣本，⭐3）
首次按 TP/FP 拆 verdict/結構軸/Δβ（既有 inventory/FN-audit 皆 pool）。**核心**：TP/FP 在甲基結構的「軸」上有清楚且**覆蓋穩健**的質性差異 — **TP 偏 allele 軸（51.6% vs 27.8%，配對後 OR_fp 0.35 / CMH 0.44），FP 偏 germline-HP 軸（hp_only 1.96% vs 8.45%，配對後 OR 4.71 / CMH 4.10，Breslow-Day 同質）**；FP 亦富集大 HP-Δβ(≥0.2: 7.4×)、SampleASM(≥0.2: 1.9×)、neither(2.1×)、Noise verdict、within-HP 多群(1.28×)。🔴 **無排他性**（最偏 TP 的 allele_only 在 FP 仍 27.8%；FP=0 格 rule-of-three 上界 0.48%=抽樣不足）→ **精煉非推翻 filter-DEAD**（富集非可分離，兩類 90%+ no_cluster）。覆蓋是 #1 confound（FP CpG 中位 58<TP 76）已用配對(KS p≈1)+CMH 控制。🔴 限制：PERMDISP 退化(無法分離 dispersion)、VerificationClass gate 截斷、HP_Residual≈HPMerged 待驗、單樣本封頂⭐3、FN 維度未含(Phase 2 需重跑 ISM)。L3 詮釋(TP=allele-驅動 somatic / FP=germline-ASM)待 cis-control+跨樣本。[報告](in_progress/2026/06/20260619_tp_fp_structure_association_HCC1395/20260619_tp_fp_structure_label_association_HCC1395_01.md) · scripts `tp_fp_structure_label_association.py`+`_figures.py` · 3 figs · `_assets/{crosstabs,confound_control,run_meta}.json`
...
### ISM joint/cluster vs modkit-equiv per-CpG 全基因組對照 — ⏳ 2026-06-20（HCC1395 ⭐2-3）
回答「modkit 式逐-CpG 能涵蓋多少、ISM read 層 joint 多抓多少」。N=30,490；modkit=per-CpG-by-label equiv(Fisher,=modkit dmr marginal 本質；binary 缺)、ISM=joint PERMANOVA。**整體 any-軸**：both **19,637**、**ISM-only 1,593(5.22%)**(joint 顯著但 per-CpG 貧乏=分散弱訊號)、modkit-only 878(2.88%)→ISM-only/modkit-only **1.81×**。**最清楚優勢證據：③切不出但 joint 顯著有 15.61% per-CpG=0**(ISM 說有結構、modkit 一無所獲；正是 Phase B 核心問)；⚠ **④未對齊 33.41% per-CpG=0=雜訊嫌疑**；⑤ per-CpG=0 僅 0.16%(強 ASM modkit 也抓)。🔴 口徑：per-CpG 定位非 ISM 創新(modkit 也做)，ISM 賣點=read-level joint(多抓~5%)+無監督 clustering(modkit 無對應，未計入 Venn)。modkit carrier/allele 軸須先 retag(非原生)。[報告](in_progress/2026/06/20260620_ism_vs_modkit_percpg_comparison_wg_01.md) · HTML `_assets/20260618_subcluster_pilot/20260620_ism_vs_modkit_percpg_comparison_01.standalone.html` · scripts percpg_{wg,compare}+build_cmp_report · BRCA2 pilot [報告](in_progress/2026/06/20260620_per_cpg_multiaxis_attribution_brca2_pilot_01.md)

### 判別流程 5-態分類（位點甲基能否切成不同群）全基因組 — ⏳ 2026-06-20（HCC1395 單樣本 ⭐2-3）
