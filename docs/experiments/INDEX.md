...
### Paired Mode HP1-1/HP2-1 Priority Bug Audit — ✅ NEGATIVE 2026-05-09
用戶 5/9 提問：「paired tag 是否也會出現 HP1-1/HP2-1 同位點共現偏向 HP1-1（類似 TO 17.3:1 priority bug）」。**答案：否**。**核心釐清**：paired mode 用 `longphase-s somatic_haplotag` 是不同 binary、不同 codebase（vs TO mode `longphase-to`），HP tag 編碼 `HP:Z:` 字串（1/2/1-1/2-1/3）vs TO 的 `HP:i:` 整數。**Step A** chr19 整體 HP1-1:HP2-1 = 12,401:14,504 = 1:1.169（近 1:1，對比 TO baseline 17.3:1）。**Step C** 1Mb window som_ratio mean **0.462** / median 0.494 / stdev 0.332，16/57 windows 近 1:1 共現，跨 chr 變化 0-1 全範圍 → 反映**真實 sub-clone signal**（LOH / sub-clone heterogeneity）而非 systematic bias。**SP1 (chr19:17M)** som_ratio 0.500 完美對稱（vs TO baseline 113:0 失衡）。**Step B** (binary patch) skip（成本不對稱，結論預期一致）。HP1-1/HP2-1 共現可作為 **亞克隆 marker 候選**（F-paired-1/2/3 follow-up）。[audit 報告](../../research/paired_priority_bug_audit/00_audit_report.md)

### PI Report 4-29 Errata Companion — ✅ 2026-05-09
F1 PI 報告 errata patch（用戶 5/8 整合報告 §9.2 4 條訊息變更後續）。**主結論不撤回**，僅修訂 4 處表述：(E1) chr19 SP1/2/3 從「主要 hotspot」→「可重現案例」（全基因組占比 2.16%, rank 19）/ (E2) V5 working tree → 已 commit（5 commits, d0bcd8c+938f0df 4-30）/ (E3) priority bug 機制證據升級至 read-level 34,855 全基因組鐵證 / (E4) V5 §6 數值為 Pass 1 only，主要功勞 V3F + Layer 1.5。erratum 獨立 companion + 原 PI 報告頂部 banner。[erratum 檔](../reports/validated/2026/05/20260509_PI_Report_4_29_Errata_01.md) [原 PI 報告（含 banner）](../reports/validated/2026/04/20260429_longphase_TO_vs_V5_Somatic_Fallback_技術報告_01.md)

### Self-Phasing 完整觀察整合報告（observation-first synthesis） — ✅ 2026-05-08 / verified 2026-05-09
單一 PI / lab meeting 入口（10 段 + 3 附錄 + 5 figures，~1000 行）。**整合 5 處先前散落觀察**：PI 報告 17.3:1 + chr19 SP1/2/3、V5 audit Pass 1 only 揭露、T1.2 chr19 752 victims、T1.2-F1 全基因組 34,855 victims、5 commits 修補演進（PON-only / V3F two-layer / V5 Layer 1.5 + ploidy fix + threshold）。**機制因果確立**（17.3:1 → 100% V3F/V5 修正、6 cell × 4 metric caller F1 三版完全相同 0.7166 / 0.6273）。**§8.4 V5 vs V3F zero-sum 已釐清** = `d0bcd8c` Pass 2 reclassify 104,457 germline het → reads shift Layer 1 → 1.5。**§8.5 20+9 指標 no regression**：PI §6 全表持平或好（6 顯著改善）+ purity 0.6 6 caller F1 + 9 結構指標 0 critical regression + LOH bed 三版完全相同。**§8.5.3 三路徑算法不依賴 purity**（Pass 1 永遠執行）。**HEAD `938f0df` = 最新有效演算法**（origin/main 後續 10 commits 全 doc / dead-code）。對 PI 報告 4-29 4 處 errata 候選（chr19 SP1/2/3 解讀降級、chr8 hotspot 改寫、§5.2 機制升級、§6.4/6.5 V5 歸因 V3F + Layer 1.5 而非 Pass 2）。[報告](../reports/validated/2026/05/20260508_Self_Phasing_完整觀察整合報告_01.md)
