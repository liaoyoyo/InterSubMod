...
data_sources:
  ...
  - /big7_disk/liaoyoyo2001/external_validation/_landscape/05_context_cards_index.md (69 源索引)
...
> **入口**：`REGISTRY.md`（主索引）→ `_landscape/05_context_cards_index.md`（69 源卡索引）+ `_landscape/08_paper_literature_map.md`（整體論文文獻地圖, Ch1-Ch5 一站式導航）+ `_landscape/06_source_read_corrections.md`（源碼校正與佐證）+ `_landscape/07_cis_basis_synthesis.md`（cis-test 科學依據）。
...
## 庫狀態（2026-06-14）
- **69 源**；tier **A×13 · B×34 · C×22**；39 repo clone + 36 PDF（repo/PDF 數不變）。**61/69 已親讀**（primary/source/fulltext）；8 cited_secondary（deamination + mcf7-subline + 6 why-hard L3-abstract）。2026-06-18 +PDclust/scMelody（axis5）；2026-06-19 +8 why-hard 問題陳述卡（axis1，SNV-only/multiplicity/CNV 困難一手源；ASCAT=B，其餘 C；doc `20260619_subclone_analysis_interpretation_full_framework_01.md`）。
