...
- ✅ Phase 1A paired-pure ceiling +0.0112（鎖定不投入）
- ✅ CL-008 Beyond-AUC ≤0.58（純 methylation ceiling ⭐5）
- ✅ CL-024 Z3 amplicon blacklist NEGATIVE-for-canonical（⭐5）
- ✅ Phase 2 Cycle 1 ⭐3 strong（HCC1395-internal ΔF1=+0.02236 = 9.24× v1.0）
- ✅ Phase 2 Cycle 2 cross-sample DIRECTION_NEGATIVE（5/19，caller-F1-headroom-bounded）
- ✅ H_C1_6 V3F/V5/V6 cross-binary PASS（V6 production zero F1 regression）
- 🔴 methyl_filter_phase2 cycle 3 blocked（4 must-fix + 48hr cooling-off）
- 🤔 G5 path (a) 真正關閉 vs cycle 3 productive failure escalation 待 5/21 後決定

**缺口**：caller F1<0.80 low-F1 sample panel 不存在；mechanism 視覺化 figures 缺
...
| 7 | F1 提升不限樣本 ceiling 假設 | Phase 1A paired-pure +0.0112 ceiling | 已鎖定不投入 |
