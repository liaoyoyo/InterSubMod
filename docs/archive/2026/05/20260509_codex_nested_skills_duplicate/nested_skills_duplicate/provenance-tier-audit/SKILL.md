...
- 本 skill 是從 `.claude/skills/provenance-tier-audit` 遷移到 `.agents/skills/provenance-tier-audit` 的 Codex 版本；`.claude` 版本保留為 legacy source，不在本 skill 內修改。
...
- repo hooks 或手動同步提醒只作輔助；本 skill 做深度盤點
...
- **上游觸發**: weekly-report 流程開始 / 用戶手動「週報 evidence」「跨 cycle 審計」 / 使用者明確要求的週級別例行檢查 / pivot-direction 後（檢查歷史是否誤判）
...
| **Used by** (誰會觸發本 skill) | `weekly-report` Layer 1 / 用戶手動「結論盤點」 / 使用者明確要求的週級別檢查 / `pivot-direction` 換方向前的歷史檢查 |
| **Reads** | `research/autoresearch/hypothesis_queue.json`、`research/autoresearch/evidence_ledger.jsonl`（全 jsonl）、`state/cycles/*/state.json`（每 cycle）、`state/cycles_archived/*/state.json`、`docs/experiments/INDEX.md`（grep ⭐ markers）、legacy Claude memory（若存在）與 repo-local 研究索引 |
| **Writes** | Codex 版本預設 **read-only**；輸出文字報告（5 段：orphan / over-claim / stale / 跨 artifact 不一致 / tier 分佈異常）給呼叫者（weekly-report 或 stdout）。只有使用者明確要求記錄 audit warning 時，才可 append 到 `state/invalidation/audit_warnings.jsonl`（新建，與 stale_marks.jsonl / tier_overrides.jsonl 並列） |
