...
- 本 skill 是從 `.claude/skills/run-evaluator` 遷移到 `.agents/skills/run-evaluator` 的 Codex 版本；`.claude` 版本保留為 legacy source，不在本 skill 內修改。
...
讀 `.agents/skills/run-evaluator/pitfalls_table.json`（6 條 P-01..P-06）。對每條：
