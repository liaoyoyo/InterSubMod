...
- 本 skill 是從 `.claude/skills/init-research` 遷移到 `.agents/skills/init-research` 的 Codex 版本；`.claude` 版本保留為 legacy source，不在本 skill 內修改。
...
**必須先讀取 AGENTS.md 與 `.claude/CLAUDE.md` 中的研究必讀清單**，確認：
...
7-phase 設計來源為 legacy Claude plan；Codex 版本以 `.agents/skills/{cycle-init,research-loop,check-staleness,run-evaluator}` 的現行說明為準。
