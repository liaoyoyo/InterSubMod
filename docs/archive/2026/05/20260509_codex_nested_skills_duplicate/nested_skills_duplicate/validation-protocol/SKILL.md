...
- 本 skill 是從 `.claude/skills/validation-protocol` 遷移到 `.agents/skills/validation-protocol` 的 Codex 版本；`.claude` 版本保留為 legacy source，不在本 skill 內修改。
...
| 系統 | 代號 | 回答什麼 | 定義位置 |
|------|------|----------|---------|
| 修改層級 | **ML1/ML2/ML3** | 用什麼語言/方式修改程式碼 | `.agents/skills/research-loop/references/SCALE_LADDER.md` |
| 規模梯度 | **S1/S2/S3** | 用多少資料測試 | `.agents/skills/research-loop/references/SCALE_LADDER.md` |
| 驗證層級 | **L1/L2/L3/L4** | 信號是否可信（本文件） | `.agents/skills/validation-protocol/SKILL.md` |
...
| 層次 | 欄位 | 可選值 | 寫入位置 | 定義文件 |
|------|------|--------|---------|---------|
| **單輪測試** | `verdict` | `positive_pilot` / `negative` / `dataset_specific` / `annotation_only` | evidence_ledger.jsonl | `.agents/skills/research-loop/references/TEST_DISCIPLINE.md` |
| **假說最終判定** | `Verdict` | `positive` / `negative` / `NO-GO` / `conditional` | Hypothesis Tracker | 本文件 + `.agents/skills/research-loop/references/HYPOTHESIS_TRACKER_TEMPLATE.md` |
...
在 `.agents/skills/research-loop/SKILL.md`「假說類型與優先分數」表中：
1. 新增類型行，含基礎分和加分條件
...
模板路徑：`.agents/skills/research-loop/references/HYPOTHESIS_TRACKER_TEMPLATE.md`
NO-GO 報告模板：`.agents/skills/research-loop/references/NOGO_REPORT_TEMPLATE.md`
