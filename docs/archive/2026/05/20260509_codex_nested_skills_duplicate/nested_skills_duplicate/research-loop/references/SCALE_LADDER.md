...
> `.agents/skills/validation-protocol/SKILL.md` 的「L1/L2/L3/L4」指**信號信任層級**，也是不同系統。
...
**S2 平行模板**（Codex subagent 平行調度；僅在使用者明確要求平行化時使用）：
```
Subtask A: 執行 HCC1395_5kHz_TO benchmark，輸出到獨立目錄。
Subtask B: 執行 HCC1395_DORADO_TO benchmark，輸出到獨立目錄。
```

**S3 平行模板**（Codex subagent 平行調度；輸出目錄必須互不重疊）：
```
Task: 平行執行 S3 full benchmark
Datasets: [6 datasets]
Output root: cycles/${CYCLE_ID}/
Constraint: 每個 dataset 使用獨立子目錄，避免覆寫。
```
...
2. 在 `.agents/skills/validation-protocol/SKILL.md` L3 固定順序清單中新增樣本名稱
