...
- 本 skill 是從 `.claude/skills/research-loop` 遷移到 `.agents/skills/research-loop` 的 Codex 版本；`.claude` 版本保留為 legacy source，不在本 skill 內修改。
...
**新增假說類型**：在上表新增行（類型名 + 基礎分 + 加分條件），並同步在 `.agents/skills/validation-protocol/SKILL.md`「假說類型 → 驗證路徑建議」表新增對應的 L1-L4 路徑。
