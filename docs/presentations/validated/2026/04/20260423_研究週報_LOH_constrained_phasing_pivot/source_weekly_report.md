...
| **S3 Diploid Het TP rate** (HCC1395 TO) | **95.5%** ⭐ (n=380, FP reduction 99.85%) | baseline 71.1% | canonical het somatic 最純白名單 |
| **S5 combo TP rate / FP reduction** | **91.8% / 99.37%** (n=886, TP recall 2.85%) | — | 高 precision 白名單子集 |
...
**本週實測**：(1) 93.2-99.0% cross 6/6 TO 樣本 ✓；(2) gap +0.37 median cross 6/6 ✓；(3) HCC1395 TO 全量 flag=on 下 NG≥3=0（Thread C 獨立觀察）✓；(4) 需跨樣本 paired 驗證（下週 R12）。
...
- **判決**：**4 假說全 POSITIVE**
  ...
  - **H-D4**（negative control）✅：Thread C 獨立觀察 HCC1395 TO flag=on 全 40k sites NG≥3=0，somatic HP demote 消除 fine-group 人工結構
- **穩定度**：⭐5（C++ 原始碼 + 6/6 obs18 跨樣本一致 + Thread C 獨立 negative control 三重證實）
...
|---------|------|:-------:|:-------:|---------|
| **CL-LCP-001** (新) | **LOH-constrained phasing signatures distinguish somatic vs germline-het in TO mode** | — | **⭐5** | C++ 原始碼 + 6/6 obs18 + Thread C 獨立 negative control 三重證實 |
