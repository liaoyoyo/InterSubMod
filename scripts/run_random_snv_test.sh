#!/bin/bash
# Script to run random SNV tests from a real VCF file

# Defaults
VCF_PATH="/big8_disk/liaoyoyo2001/InterSubMod/data/vcf/HCC1395/pileup/filtered_snv_tp.vcf.gz"
OUTPUT_DIR="/big8_disk/liaoyoyo2001/InterSubMod/output"
NUM_SITES=32
THREADS=4
LOG_FILE="${OUTPUT_DIR}/execution_analysis.log"

# Parse arguments
while [[ $# -gt 0 ]]; do
    case $1 in
        -v|--vcf)
            VCF_PATH="$2"
            shift 2
            ;;
        -n|--num)
            NUM_SITES="$2"
            shift 2
            ;;
        -t|--threads)
            THREADS="$2"
            shift 2
            ;;
        -o|--out)
            OUTPUT_DIR="$2"
            shift 2
            ;;
        *)
            echo "Unknown argument: $1"
            exit 1
            ;;
    esac
done

echo "=== InterSubMod Random SNV Test ==="
echo "VCF: ${VCF_PATH}"
echo "Output Dir: ${OUTPUT_DIR}"
echo "Sites: ${NUM_SITES}"
echo "Threads: ${THREADS}"
echo "Log File: ${LOG_FILE}"
echo "-----------------------------------"

# Create output directory
mkdir -p "${OUTPUT_DIR}"

# Step 1: Generate Random SNV Table
echo "[1] Generating random SNV table..."
TEMP_TSV="${OUTPUT_DIR}/random_snvs_${NUM_SITES}.tsv"

# Write Header
echo -e "chr\tpos\tref\talt\tqual" > "${TEMP_TSV}"

# Extract random lines from VCF
# VCF columns: CHROM(1) POS(2) ID(3) REF(4) ALT(5) QUAL(6)
# We need: chr pos ref alt qual
if [ -f "${VCF_PATH}" ]; then
    # Check if file is gzipped
    if [[ "${VCF_PATH}" == *.gz ]]; then
        CAT_CMD="zgrep"
    else
        CAT_CMD="grep"
    fi
    
    # Extract, Shuffle, Format
    ${CAT_CMD} -v "^#" "${VCF_PATH}" | shuf -n "${NUM_SITES}" | awk '{print $1, $2, $4, $5, $6}' >> "${TEMP_TSV}"
    
    echo "    Generated: ${TEMP_TSV}"
    echo "    Sample lines:"
    head -n 3 "${TEMP_TSV}" | sed 's/^/      /'
else
    echo "Error: VCF file not found at ${VCF_PATH}"
    exit 1
fi

# Step 2: Run Execution
echo ""
echo "[2] Running InterSubMod..."
EXECUTABLE="/big8_disk/liaoyoyo2001/InterSubMod/build/bin/test_phase4_5"

if [ ! -f "${EXECUTABLE}" ]; then
    echo "Error: Executable not found at ${EXECUTABLE}"
    echo "Please build the project first."
    exit 1
fi

# Use /usr/bin/time for resource monitoring
# -v: verbose (gives memory, time, etc.)
echo "    Starting execution with ${THREADS} threads..."
{
    /usr/bin/time -v "${EXECUTABLE}" "${TEMP_TSV}" "${OUTPUT_DIR}" "${THREADS}"
} 2>&1 | tee "${LOG_FILE}"

# Step 3: Verify Results
echo ""
echo "[3] Verifying results..."
REGION_COUNT=$(ls -d "${OUTPUT_DIR}/region_"* 2>/dev/null | wc -l)

echo "    Expected regions: ${NUM_SITES}"
echo "    Found regions:    ${REGION_COUNT}"

if [ "${REGION_COUNT}" -eq "${NUM_SITES}" ]; then
    echo "    ✓ Region count matches."
else
    echo "    ⚠ Region count mismatch! Check logs."
fi

echo ""
echo "=== Analysis Log Saved to: ${LOG_FILE} ==="
echo "Done."

