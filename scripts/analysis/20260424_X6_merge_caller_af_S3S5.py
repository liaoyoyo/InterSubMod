...
def load_and_merge(sample: str, archive: str):
    ...
    for tsv_path, vcf_path, lbl in [(tp_tsv, tp_vcf, 1), (fp_tsv, fp_vcf, 0)]:
        tsv = pd.read_csv(tsv_path, low_memory=False,
                          usecols=["Chr", "Pos", "Ref", "Alt", "HPFineNGroups",
...
def main():
    ...
    for sample, archive in SAMPLES.items():
        print(f"=== {sample} ===")
        df, missing = load_and_merge(sample, archive)
