...
3. 🔴 **BRCA2 不再是 cis 錨點 = copy-artifact（06-07）** —— 唯一乾淨 cis exemplar 是 **chr17:79991120 (TBC1D16)**；BRCA2 hero 敘事退役。
...
| **T3 cis-test / copy-partition** | 支撐（per-locus + falsification）| copy-dosage 乾淨 falsified；normal-anchored cis-test 方法；copy-partition 分解；**chr17/TBC1D16 = 唯一乾淨 cis** (perm p=0.001, within>HP) | 🔴 **BRCA2 + chr5 = copy-artifact**（推翻）；chr18 mechanical CpG-create；「60 T3」塌成 1 | 6 strongest UNTESTABLE (pureALT CGI-desert)；chr17 單 locus/單樣本；未 anchor SEQC2 CN | **READY-AS-NEGATIVE / methods** |
...
🔴 **HD-3（真矛盾 #1 — 必改 2 份 validated 文件）：把 BRCA2 改成 06-07 狀態。** `master_draft.md` line 122 仍寫「BRCA2 真 cis-driven (d_cis=−0.142/−0.152)…cis-test 雙軸通過」；`07_asm_cis_cancer_impact.md` lines 27/33/50/53/66（含 `@06-03` snapshot tag）仍寫「cis-driven ASM 真…60 T3+4 survivor」。兩者被 06-07 `fast_cnv_validation.json`（BRCA2 = copy-artifact；只 chr17 clean）矛盾。⚠ **honest 口徑（critic #9）**：BRCA2 不是純 copy-artifact，是「**~80% copy + 邊際未確立 ~20% 真 cis (d_within=−0.023, perm p=0.022)**」——改時勿從一個 overclaim 換成相反 overclaim。**改 validated 文件是 governance 決定 → 你核准，不可默默覆寫。**（Martin-Trujillo 現在反而 corroborate copy-artifact reclass = 「吃掉 BRCA2 的 confound」，比 ρ=−0.055 pilot 更強。）
...
- **FT-1 characterization-set vs filter**（最高頻）｜**FT-2 cis-anchor vs copy-artifact**（BRCA2 ~80% copy；且 hypo≠canonical hyper，勿當 TSG-silencing 證據）｜**FT-3 private vs underpowered**（0/38）｜**FT-4 phenomenon-recurrence vs cis-recurrence**（6/6 是 allelic-meth-diff 復現非 cis-ASM 復現）｜**FT-5 held-out simulation vs true-unphase rescue**（umtag 88.5% 是 held-out 模擬非真救援）。
