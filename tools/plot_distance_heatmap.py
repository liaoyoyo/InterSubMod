...
def load_distance_matrix(region_dir: str, metric: str = "NHD", strand: str = "all") -> Tuple[Optional[pd.DataFrame], Optional[List[str]]]:
    """
...
def compute_linkage_from_distance(dist_df: pd.DataFrame, method: str = "average") -> np.ndarray:
    """
...
def create_annotation_colors(reads_df: pd.DataFrame, read_ids: List[str]) -> Tuple[pd.DataFrame, Dict]:
    """
...
def process_single_region(
    ...
    success = plot_distance_heatmap(
        dist_df, read_ids, reads_df, Z, output_path,
        region_info, metric_name=distance_metric, linkage_method=linkage_display,
