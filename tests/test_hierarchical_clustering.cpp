...
protected:
    void SetUp() override {
        ...
        dist_6x6 << 0.0, 0.2, 0.4, 0.8, 0.9, 1.0,
                    0.2, 0.0, 0.3, 0.7, 0.8, 0.9,
                    0.4, 0.3, 0.0, 0.6, 0.7, 0.8,
                    0.8, 0.7, 0.6, 0.0, 0.2, 0.3,
                    0.9, 0.8, 0.7, 0.2, 0.0, 0.2,
                    1.0, 0.9, 0.8, 0.3, 0.2, 0.0;
...
TEST_F(HierarchicalClusteringTest, AllMethodsProduceTrees) {
    std::vector<LinkageMethod> methods = {
        LinkageMethod::UPGMA,
        LinkageMethod::WARD,
    ...
    for (LinkageMethod method : methods) {
        ...
        EXPECT_FALSE(tree.empty()) 
            << "Method " << HierarchicalClustering::method_to_string(method) 
...
TEST_F(HierarchicalClusteringTest, BootstrapAnnotation) {
    ...
    EXPECT_TRUE(newick_with_bs.find("95") != std::string::npos ||
                newick_with_bs.find("80") != std::string::npos ||
